...
//	self.attachEvent("onblur",function(){WebFX_PopUpcss.style.display="block"})
}
...
ContextMenu.fixSize=function()
{
	...
	WebFX_PopUpcss.style.height =  h + "px"//"25px";
	WebFX_PopUpcss.style.width = w + "px"//"70px";
...
ContextMenu.populatePopup=function(arr,win)
{
	...
	if (doc.getElementsByTagName("LINK").length == 0) {
		doc.open();
		doc.write('<html><head><link rel="StyleSheet" type="text/css" href="'+systemroot+'/css/contextmenu.css"></head><body></body></html>');
		doc.close();
	}
	for(i=0;i<alen;i++)
	{
