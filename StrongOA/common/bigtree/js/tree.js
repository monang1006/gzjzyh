...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if (nd.hasChildren && (!dfop.displayDeep || deep < dfop.displayDeep - 1)) {//增加显示级别限制
            	...
            	if(nid.indexOf("u") == 0){//用户图标要显示成 renyuan.gif yanjian 2012-07-07 16:00
            ...
            if (dfop.showcheck && nd.showcheck) {
                ...
                if(nid.indexOf("u") == 0){//用户图标要显示成 renyuan.gif yanjian 2012-07-07 16:00
                	ht.push("<img src='"+scriptroot+"/workflow/images/tree/renyuan.gif'/>");
         ...
         * 点击单选框时节点选中事件
         * @param item -树节点
         * @param state -树节点选中状态，在单选情况下一直是1
         */
        ...
        /**
         * 区分当前选中节点的半选和全选状态，使用情况：
         * 1.当树节点选中不需要进行回溯和遍历时，该节点视子节点是否全部选中来决定是否使用半选状态
         * 2.当树节点初始化时，该节点视子节点是否全部选中来决定是否使用半选状态
         * 3.当item为父级节点并且当前节点的选中状态为0时，则item为半选状态
         * @param item -树节点
         * @param state -选择状态
         * @param type -选择类型：1.item为当前选中节点 0.item为父级节点
         * @return 
         */
         ...
         * @param c -返回结果集合
         * @param fn -返回处理函数
         * @param deep -目前树的层次深度，主要在处理级别设置时使用
         * @return
         */
         ...
         * 初始化选中的树节点
         * @param items -树节点集
         * @param parentNode -items的父节点
         * @param type -初始化类型；type=0 树初始化时的节点选中；type=1 树延迟加载时的节点选中
         * @return 
         */
        ...
        function getAllChildNodes(item,nodes) {
        	if(!nodes){
        		nodes = new Array();
        	}
        	if (item.ChildNodes != null && item.ChildNodes.length > 0) {
        ...
        me[0].t = {
        	...
        	/**
        	 * 得到指定节点的所有子节点
        	 * @return 指定节点的所有子节点（需要递归实现）
        	 */
        	getChildNodes:function(item){
        		return getAllChildNodes(item);
        	},
        	/**
        	 * 得到树的所有节点
    ...
    //获取指定节点的所有子节点
    $.fn.getChildNodes = function(item){
    	if (this[0].t) {
            return this[0].t.getChildNodes(item);
        }
        return null;
    }
    // 获取所有选中的节点的Ids数组
