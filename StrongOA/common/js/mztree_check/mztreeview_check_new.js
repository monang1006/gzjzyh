/**
 * <p>Description: Web TreeView 目录树 </p>
 * <p>Copyright: Copyright (c) 2005</p>
 * <p>CreateData: 2005-12-25 </p>
 * @author xuzm
 * @version 1.0
 */
//Web TreeView , 在实例化的时候请把实例名作参数传递进来
...
function MzTreeView(Tname)
{
  ...
  this.icons    = {
    ...
    L1        : 'join.gif',  //┣line_notlast
    L2        : 'joinbottom.gif',  //┗line_last
    L3        : 'L3.gif',  //━
    ...
    root      : 'base.gif',   //缺省的根节点图标 root
    folder    : 'folder_closed.gif', //缺省的文件夹图标
    file      : 'ico2.gif',   //缺省的文件图标
    ...
    xxgx      : 'xxgx.gif',
    zdgl      : 'zdgl.gif',
    folder2  : 'folder_closed-2.gif',   
    ico3     : 'ico3-2.gif',
    folder3  : 'folder_closed_3.gif',   
    ico4     : 'ico2_3.gif',
    exit      : 'exit.gif'
  };
  this.iconsExpand = {  //存放节点图片在展开时的对应图片
    ...
    PM1       : 'minus.gif',     //－┣
    PM2       : 'minusbottom.gif',     //－┗
    PM3       : 'minus_notlast.gif',     //－━
    folder    : 'folderopen.gif',
	folder2  : 'folder_open-2.gif',
	folder3  : 'folderopen_3.gif',
    exit      : 'exit.gif'
  };
...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var valueId = this.getAttribute(source, "valueId");
  var onclick = this.getAttribute(source, "onclick");
  var isChecked =this.getAttribute(source, "isChecked");
  var chooseType=this.getAttribute(source, "chooseType");
  var showTitle=this.getAttribute(source,"showTitle");
  //**
  var checkAble = this.getAttribute(source,"checkAble");
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  var chooseType= node.chooseType;
  //**
  var checkAble = node.checkAble;
  var showTitle = node.showTitle;
  ...
  else if(node.icon=="folder2")
  	node.icon = HCN ? "folder2" : "ico3";
  else if(node.icon=="folder3")
  	node.icon = HCN ? "folder3" : "ico4";
  ...
  HTML += "<IMG "+
    ...
    if(node.icon.indexOf(".")!=-1){
     ...
     checkBoxHtml="<input type='checkbox' style='position:absolute;clip: rect(auto auto auto auto)' id='chk_" + id +
     "' name='chkTreeNode' checktype='add'";
     if(chooseType=="signle"){
     	checkBoxHtml="<input type='radio' style='position:absolute;clip: rect(auto auto auto auto)' id='chk_" + id +
     "' name='chkTreeNode' checktype='add'";
     }else if(chooseType=="chooseOne"){
     	
     }else{
     	checkBoxHtml+="onclick='checkNodeClick(this.checked,this.value,"+id+")' "
     }
     if(isChecked=="true"){
     ...
     checkBoxHtml+=" value='"+valueId+"'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
     if(isChecked!="no") HTML += checkBoxHtml;
     if(showTitle=="true"){
	    HTML +="<A "+
    ...
    if(onclick!=null&&onclick!=""&&onclick!="null")
    	HTML +=onclick+";";
    HTML +="return "+ this.name +".nodeClick(\""+ id +"\")'>"+ node.text +
  "</A></NOBR></DIV>";
...
MzTreeView.prototype.expand   = function(id, sureExpand)
{
  ...
  if (area)
  {
    ...
    //if (img!=undefined&&img)  img.src = !Bool ? icon.src :typeof(iconE)=="undefined" ? icon.src : iconE.src;
...
{
	  if(this.node[id].onclick!=null&&this.node[id].onclick.length!=0)
	  {
         eval(this.node[id].onclick);
		 return;
	  }
	  //模块编码
	  var sysCode=this.node[id].valueId;
	  //alert(sysCode);
	  //模块名称
	  var title=this.node[id].text;
	  var url="/theme/theme!RefreshPriviModule.action?priviparent="+sysCode;
	  //alert(this.node[id].onclick);
	  //打开新的窗口
	  navigates(url,title);
};
...
MzTreeView.prototype.clickHandle = function(e)
{
  ...
  switch(e.tagName)
  {
    ...
    case "A" :
      ...
      if(id!=1&&this.node[id].hasChild) this.expand(id);
      break;
    case "SPAN" :
      if(e.className=="pm")
        this.expand(e.id.substr(e.id.lastIndexOf("_") + 1));
...
MzTreeView.prototype.expandAll = function()
{
  ...
  if(this.totalNode>500){ 
  	...
  	setTimeout(this.name+".expandAllTest()",1);
  	var node  = this.node[e.id];
        var temp = this.getElementById(this.name +"_tree_"+ e.id).childNodes;
        temp[temp.length-1].innerHTML = "<DIV noWrap><NOBR><SPAN>"+ (this.wordLine ?
        node.childAppend +"?" : this.word2image(node.childAppend +"?")) +"</SPAN>"+
        "<IMG border='0' height='16' align='absmiddle' src='"+this.icons["file"].src+"'>"+
        "<A style='background-Color: "+ this.colors.highLight +"; color: "+
        this.colors.highLightText +"; font-size: 9pt'>正在加载数据，请稍候...</A></NOBR></DIV>";
 		temp[temp.length-1].style.display="";
  }else 
  	this.expandAllTest(e);
  
 
};
MzTreeView.prototype.expandAllTest = function()
{
  if(this.node["0"].childNodes.length==0) return;
  var e = this.node["0"].childNodes[0];
  var isdo = t = false;
