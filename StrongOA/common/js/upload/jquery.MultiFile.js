...
/*# AVOID COLLISIONS #*/
 ...
 $.extend($.MultiFile, {
  options: {
   ...
   STRING: {
    remove:'<SPAN style="color: blue">删除</span>',
    denied:'不能选择 $ext 文件.\n请重试...',
    selected:'File selected: $file',
    duplicate:'您已经选择了这个文件:\n$file'
   }
  }
 });
    ...
    return $(this).each(function(group_count){
     if(this._MultiFile) return; this._MultiFile = true;
       ...
       if(String(MF.accept).length>1){
        MF.rxAccept0 = new RegExp('\\.('+(MF.accept.split("/")[0]?MF.accept.split("/")[0]:'')+')$','gi');
        MF.rxAccept1 = new RegExp('\\.('+(MF.accept.split("/")[1]?MF.accept.split("/")[1]:'')+')$','gi');
        //alert(MF.rxAccept0);
       // alert(MF.rxAccept1);
       };
       ...
       MF.addSlave = function( slave, slave_count ){
        ...
        $(slave).change(function(){
          ...
          // check extension
          if(MF.accept && v && !v.match(MF.rxAccept0)&& !v.match(MF.rxAccept1))//{
     ...
     /**if(MF.slaves.length==1){
        	if($("#msgTitle").val()==""){
        		$("#msgTitle").val($(MF.slaves[0]).val().match(/[^\/\\]+$/gi)[0]);
        	}
        }*/
        try{
        	doInterface(MF);
      ...
      //  alert($(MF.slaves[0]).val().match(/[^\/\\]+$/gi)[0]);
        /** if($("#msgTitle").val()==""){
        	 $("#msgTitle").val(a.html());
         }*/
