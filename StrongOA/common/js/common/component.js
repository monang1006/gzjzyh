...
(function($) {
	...
	$.fn.contextmenu = function(option) {
		...
		option = $.extend({
			...
			width : 150,
			theme : 'b-m-mpanel',
			//增加鼠标移开后是否自动隐藏菜单功能设置，默认为否
			//注意：该功能只能适用于菜单组件只包含一个Group的情况，如果有多个Group，则启用该功能会出问题
			autoHidden : false
		}, option);
		...
		var outItem = function(e) {
			...
			//如果需要在鼠标移开菜单后自动隐藏菜单，则增加菜单项mouseleave事件的响应
			if(option.autoHidden){
				if(groups[option.alias] !== e.relatedTarget && !$.contains(groups[option.alias], e.relatedTarget)){
					root.hideMenuPane();
				}
			}
		...
		// CollectGarbage();
		// return me;
		//如果需要在鼠标移开后自动隐藏菜单，则增加菜单hover事件
		if(option.autoHidden){
			$(groups[option.alias]).mouseleave(
					function(event){
						root.hideMenuPane();
					}
			);
...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            ht.push("<div id='", id, "_", nid, "' tpath='", path, "' unselectable='on' title='", nd.tipText, "'");
            var cs = [];
        ...
        function nodeclick(e) {
            ...
            if (et.tagName == "IMG") {
                ...
                var nid = item.id.replace(/[^\w]/gi, "_");
                var et = $("#" + id + "_" + nid + "_cb");
                if ($(et).hasClass("bbit-tree-node-ra")) // 点击了radio
                {
                    var s = 1;
                    var r = true;
                    if (dfop.oncheckboxclick) {
        ...
        function initSelectedNodes(items, parentNode, type){
        	if(dfop.aHasSelected || dfop.aHasSelected != ""){
	        	if(dfop.aNeedLazy){
	        		if(dfop.showradio){
     ...
     * @returns
     */
    $.fn.config = function(settings){
    	$.extend(this[0].t.dfop, settings);
    }
})(jQuery);
...
$.extend($.validator, {
	...
	normalizeRules: function(rules, element) {
		...
		$.each(['rangelength', 'range'], function() {
			if (rules[this]) {
			    //为了兼容<input range="1,99"/>、<input rangelength="1,99"/>这种情况
			    //所以当rules[this]是字符串时，需要根据","进行分割
...
//非负整数验证
$.validator.addMethod("integerAndZero", function(value, element) {
	var Intzero=/^[1-9]\d*|0$/;
	return this.optional(element) ||Intzero.test(value);
}, "errors_IntegerAndZero");


//输入框英文双引号验证
$.validator.addMethod("doubleQuotes", function(value, element) {
	var pattern = /\"+/;
		...
		this.tabHeaddown.click(function(e) {
			...
			if(hidetabssize > 0) {
				for(i = 0; i < hidetabssize; i += 1) {
					...
					menu[i] = {text:hidetabtext, alias:'menuoption'+hidetabid, icon: imgPath + "/contextmenu/ico4-1.gif", action:menuclick};
				}
				
				mm = {alias:'aaaaa' + new Date(), items:menu, autoHidden:true};
	...
	HTabPanel.prototype = {
		...
		add : function(config) {
			var that	 = this, 
				...
				menuOption 		= {alias:"menu"+id+(new Date()).toString(), items: [{text:"关闭当前",icon: imgPath + "/contextmenu/ico4-1.gif", alias:"menuOptiona", action:close},
			         		                        	  {text:"关闭其他",icon:imgPath + "/contextmenu/ico4-1.gif",alias:"menuOptionb",action:closeOthers},
			         		                              {text:"关闭全部",icon:imgPath + "/contextmenu/ico4-1.gif",alias:"menuOptionc",action:closeAll}], autoHidden:true},
...
// //tabBodyContainer.append(tabBodyWraper);
		...
		activeId : function(id) {
			var curActiveTab = this.tabHeadWraper.find('.menubg');
			this.reset();
			var thetab = this.tabHeadWraper.find("#"+id);
			if(thetab.css("display") === "none"){
				if(curActiveTab.length == 0) {
					this.tabHeadWraper.find('li:visible').first().hide();
				}else{
					curActiveTab.hide();
				}
				thetab.show();
		 ...
		 * 高亮并刷新传入id的tab页
		 */
		refreshId : function(id, url) {
			this.tabBodyRenderer.find("#_" + id).find('iframe')[0].src = url;
			this.activeId(id);
		},
