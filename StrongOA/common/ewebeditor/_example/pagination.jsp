...
static String[] eWebEditorPagination(String s_Content, String s_CurrPage){
	...
	// <style>...</style>
	String s_Style = "";
	p = Pattern.compile("<style[^>]*>((?:.|[^\\x00-\\xff])+?)</style>", flags);
	...
	// 使用正则表达式对分页进行处理
	p = Pattern.compile("<!--ewebeditor:page title=\"([^\"|[^\\x00-\\xff]]*)\"-->((?:.|[^\\x00-\\xff])+?)<!--/ewebeditor:page-->", flags);
	m = p.matcher(s_CurrContent);
	while (m.find()){
	...
	if (n_PageCount == 0){
		...
		for (int i=1; i<=n_PageCount; i++) {
			if (i == n_CurrPage){
				...
				s_Titles = s_Titles + "<li>" + i + ") <a href='?page=" + i + "'>" + a_PageTitle.get(i).toString() + "</a>";
			}
		}
