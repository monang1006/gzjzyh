...
EWEBTools.DisableSelection = function( element ){
	if ( EWEBBrowser.IsGecko ){
		...
		element.style.MozUserSelect		= '-moz-none' ;	// Gecko only.
...
EWEBSelection.GetType = function(){
	...
	if ( sel && sel.rangeCount == 1 ){
		...
		if ( range.startContainer == range.endContainer
			...
			&& StyleObjectElements[ range.startContainer.childNodes[ range.startOffset ].nodeName.toLowerCase() ] )
		{
			type = 'Control' ;
		}
	}
...
EWEBSelection.SelectNode = function( element ){
