...
function myEditor_Init() {
	...
	myEditor.BaseHref = "";
	if (config.BaseHref != "") {
		myEditor.BaseHref = "<base href='" + document.location.protocol + "//"
				+ document.location.host + config.BaseHref + "'>";
	}
	if (myParam.ExtCSS) {
		myEditor.ExtCSS = "<link href='" + relative2fullpath(myParam.ExtCSS)
...
window.onload = function() {
	...
	if ((!myEditor.LinkField) && (config.AppExt == "aspx")) {
...
function getFixWidthHTML(html) {
	if (config.FixWidth) {
		...
		if (!re.test(html)) {
			return "<div id=eWebEditor_FixWidth_DIV style='width:"
					+ config.FixWidth + ";height:100%' unselectable>" + html
					+ "</div>";
		}
	}
	return html;
...
function setLinkedField() {
	...
	if (!oForm.submitEditor)
		oForm.submitEditor = new Array();
	...
	if (!oForm.resetEditor)
		oForm.resetEditor = new Array();
...
function onPaste() {
	...
	if (myEditor.CurrMode == "EDIT") {
		...
		getDoc().selection.createRange().pasteHTML(
				HTMLEncode(clipboardData.getData("Text")));
		saveHistory();
		return false;
	}
}
function PasteWord() {
	...
	var arr = showModalDialog("dialog/importword.htm?action=paste", window,
...
function onMouseUp() {
	if (oResizing.El) {
		if ((oResizing.El.style.width != oResizing.W)
				|| (oResizing.El.style.height != oResizing.H)) {
			saveHistory();
		}
	}
}
...
function setHTML(html, b_NotSaveHistory) {
	...
	case "EDIT":
		...
		getDoc().open();
		getDoc().write(
				getStyleEditorHeader() + getFixWidthHTML(html) + "</body>");
	...
	getDoc().oncontextmenu = new Function(
			"return showContextMenu(eWebEditor.event);");
	...
	for ( var i = 0; i < arr.length; i++) {
		...
		if ((o_Parent.tagName == "FONT") || (o_Parent.tagName == "SPAN")) {
			if (o_Parent.innerText == o_Font.innerText) {
				o_Parent.style.cssText = o_Parent.style.cssText + ";"
						+ o_Font.style.cssText;
...
function setStyleInFont(obj, what, v) {
	...
	var o_Children = obj.children;
	for ( var j = 0; j < o_Children.length; j++) {
		if ((o_Children[j].tagName == "SPAN")
				|| (o_Children[j].tagName == "FONT")) {
...
function delEmptyNodeInFont(obj) {
	...
	for ( var j = 0; j < o_Children.length; j++) {
		...
		if ((o_Children[j].tagName == "FONT")
				|| (o_Children[j].tagName == "SPAN")) {
			if ((o_Children[j].style.cssText == "")
					|| (o_Children[j].innerHTML == "")) {
...
function showDialog(url, optValidate) {
	...
	var arr = showModalDialog(url, window,
			"dialogWidth:0px;dialogHeight:0px;help:no;scroll:no;status:no");
	saveHistory();
	FocusEditarea();
}
function Maximize() {
	...
	window
			.open(
					"dialog/fullscreen.htm?style=" + myParam.StyleName,
					'FullScreen' + myParam.LinkField,
					'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,fullscreen=yes');
}
function createLink() {
	if (!validateMode()) {
		return;
		...
		if (sel.type == "Control") {
			...
			if (el.tagName.toUpperCase() == tag) {
				if ((attrName) && (attrValue)) {
					if (el.getAttribute(attrName, 2).toLowerCase() == attrValue
							.toLowerCase()) {
						return true;
					}
				} else {
...
function mapEdit() {
	...
	window
			.open(
					"dialog/map.htm",
					"mapEdit" + myParam.LinkField,
					"toolbar=no,location=no,directories=no,status=not,menubar=no,scrollbars=no,resizable=yes,width=450,height=300");
}
	...
	if (sel.type != "Control") {
		...
		for ( var i = 0; i < aAllEl.length; i++) {
			...
			if (sel.inRange(oRngTemp)) {
				...
				if (((sel.compareEndPoints("StartToEnd", oRngTemp) < 0) && (sel
						.compareEndPoints("StartToStart", oRngTemp) > 0))
						|| ((sel.compareEndPoints("EndToStart", oRngTemp) > 0) && (sel
								.compareEndPoints("EndToEnd", oRngTemp) < 0))) {
					aSelEl[aSelEl.length] = aAllEl[i];
				}
			}
		}
...
function splitTextField(objField, html) {
	...
	if (html.length > FormLimit) {
		...
		while (html.length > 0) {
			var objTEXTAREA = objDocument.createElement("<TEXTAREA NAME='"
					+ strFieldName + "'></TEXTAREA>");
...
function localUpload() {
	...
	var s_PostUrl = getSitePath() + myEditor.RootPath + "/" + config.ServerExt
			+ "/upload." + config.ServerExt + "?action=local&type=local&style="
			+ config.StyleName + "&cusdir=" + config.CusDir;
	var s_HTML = getHTML();
	...
	if (s_OriginalFiles) {
		...
		for ( var i = 0; i < a_Original.length; i++) {
			if (a_Saved[i]) {
				...
				var s_SaveFileName = a_Saved[i].substr(a_Saved[i]
						.lastIndexOf("/") + 1);
				var s_SavePathFileName = a_Saved[i];
				addUploadFile(s_OriginalFileName, s_SaveFileName,
						s_SavePathFileName);
			}
		}
	}
...
function saveHistory() {
	...
	if (myHistory.data[myHistory.position] != html) {
		...
		if (getDoc().selection.type != "Control") {
			try {
				myHistory.bookmark[myHistory.bookmark.length] = getDoc().selection
			...
			var el = oRng.item(0);
			myHistory.bookmark[myHistory.bookmark.length] = "[object]|"
					+ el.tagName + "|" + getElementTagIndex(el);
		}
		if (nBeginLen != 0) {
			myHistory.position++;
		}
...
function getStyleEditorHeader() {
	var s_Header = "<head>";
	s_Header += "<link href='" + myEditor.RootPath + "/skin/" + config.Skin
			+ "/editorarea.css' type='text/css' rel='stylesheet'>";
	s_Header += "<base target='_blank'>";
	switch (myEditor.CurrMode) {
	case "CODE":
		s_Header += "<link href='" + myEditor.RootPath + "/skin/" + config.Skin
				+ "/editorarea.code.css' type='text/css' rel='stylesheet'>";
		break;
	case "TEXT":
		s_Header += "<link href='" + myEditor.RootPath + "/skin/" + config.Skin
				+ "/editorarea.text.css' type='text/css' rel='stylesheet'>";
		break;
	case "EDIT":
	case "VIEW":
		if (config.AreaCssMode != "1") {
			s_Header += "<link href='" + myEditor.RootPath + "/skin/"
					+ config.Skin
					+ "/editorarea.edit.css' type='text/css' rel='stylesheet'>";
		}
...
function getMenuRow(s_Disabled, s_Event, s_Image, s_Html) {
	var s_MenuRow = "";
	if (s_Disabled == "") {
		s_MenuRow += "<tr><td class='Menu_Item'><table border=0 cellpadding=0 cellspacing=0 width='100%'><tr><td valign=middle class=MouseOut onMouseOver=\"this.className='MouseOver'\" onMouseOut=\"this.className='MouseOut'\" onclick=\"parent."
				+ s_Event + ";parent.oPopupMenu.hide();\">";
	} else {
	...
	if (typeof (s_Image) == "number") {
		...
		s_MenuRow += "<div class='Menu_Image" + s_Disabled + "'><img src='"
				+ s_Img + "' style='top:" + n_Top + "px'></div>";
	} else if (s_Image != "") {
		var s_Img = "skin/" + config.Skin + "/" + s_Image;
		s_MenuRow += "<img class='Menu_Image" + s_Disabled + "' src='" + s_Img
				+ "'>";
	}
	s_MenuRow += "</td><td class='Menu_Label" + s_Disabled + "'>" + s_Html
			+ "</td></tr></table>";
...
function showContextMenu(event) {
	...
	if (isControlSelected("OBJECT", "classid",
...
function showToolMenu(menu) {
	...
	case "paragraph":
		...
		sMenu += sMenuHr;
		sMenu += getStandardMenuRow((tagInSelection("P")) ? "" : "disabled",
				"ParagraphAttr", "CMenuParagraph");
		break;
	case "edit":
		var s_Disabled = "";
	...
	case "zoom":
		for ( var i = 0; i < aZoomSize.length; i++) {
			if (aZoomSize[i] == nCurrZoomSize) {
				sMenu += getMenuRow("", "doZoom(" + aZoomSize[i] + ")", 120,
						aZoomSize[i] + "%");
			} else {
				sMenu += getMenuRow("", "doZoom(" + aZoomSize[i] + ")", 119,
						aZoomSize[i] + "%");
			}
		}
		break;
	case "fontsize":
		...
		for ( var i = 0; i < lang["FontSizeItem"].length; i++) {
			if (lang["FontSizeItem"][i][0] == v) {
				sMenu += getMenuRow("", "formatFont('size','"
						+ lang["FontSizeItem"][i][0] + "')", 120,
						lang["FontSizeItem"][i][1]);
			} else {
				sMenu += getMenuRow("", "formatFont('size','"
						+ lang["FontSizeItem"][i][0] + "')", 119,
						lang["FontSizeItem"][i][1]);
			}
		}
		break;
	case "fontname":
		...
		for ( var i = 0; i < lang["FontNameItem"].length; i++) {
			if (lang["FontNameItem"][i] == v) {
				sMenu += getMenuRow("", "formatFont('face','"
						+ lang["FontNameItem"][i] + "')", 120,
						lang["FontNameItem"][i]);
			} else {
				sMenu += getMenuRow("", "formatFont('face','"
						+ lang["FontNameItem"][i] + "')", 119,
						lang["FontNameItem"][i]);
			}
		}
		break;
	case "formatblock":
		...
		for ( var i = 0; i < lang["FormatBlockItem"].length; i++) {
			if (lang["FormatBlockItem"][i][0].toLowerCase() == v) {
				sMenu += getMenuRow("", "format('FormatBlock','"
...
function TableRowMerge() {
	...
	if (selectedTR.rowIndex + 1 != allRows.length) {
		var allCellsInNextRow = allRows[selectedTR.rowIndex
				+ selectedTD.rowSpan].cells;
		var addRowSpan = allCellsInNextRow[selectedTD.cellIndex]
	...
	for ( var n = 0; n < addRowsNoSpan; n++) {
		...
		for ( var j = 0; j < selectedTR.rowIndex; j++) {
			for ( var k = 0; k < allRows[j].cells.length; k++) {
				if ((allRows[j].cells[k].rowSpan > 1)
						&& (allRows[j].cells[k].rowSpan >= selectedTR.rowIndex
								- allRows[j].rowIndex + 1)) {
...
function TableColMerge() {
	...
	if (selectedTD.cellIndex + 1 != selectedTR.cells.length) {
		var addColspan = allCells[selectedTD.cellIndex + 1]
...
function showEditorBody() {
	document
			.write("<table id=eWebEditor_Layout border=0 cellpadding=0 cellspacing=0 width='100%' height='100%'>");
	...
	document
			.write("	<iframe class='Composition' ID='eWebEditor' style='width:100%;height:100%' marginwidth=1 marginheight=1 scrolling='yes' frameborder='0' src='dialog/blank.htm'></iframe>");
	document.write("</td></tr>");
	if (config.StateFlag) {
		document.write("<tr><td class=SB>");
		document
				.write("	<TABLE border='0' cellPadding='0' cellSpacing='0' width='100%' class=SB>");
		document.write("	<TR valign=middle>");
		document.write("	<td>");
		document
				.write("		<table border=0 cellpadding=0 cellspacing=0 class=SB_Mode>");
		document.write("		<tr>");
		document.write("		<td class=SB_Mode_Left></td>");
		if (config.SBCode) {
			document
					.write("		<td class=SB_Mode_BtnOff id=eWebEditor_CODE onclick=\"setMode('CODE')\" unselectable=on><table border=0 cellpadding=0 cellspacing=0><tr><td class=SB_Mode_Btn_Img>"
							+ getBtnImgHTML("ModeCode")
		...
		if (config.SBEdit) {
			document
					.write("		<td class=SB_Mode_BtnOff id=eWebEditor_EDIT onclick=\"setMode('EDIT')\" unselectable=on><table border=0 cellpadding=0 cellspacing=0><tr><td class=SB_Mode_Btn_Img>"
							+ getBtnImgHTML("ModeEdit")
							+ "</td><td class=SB_Mode_Btn_Text>"
							+ lang["StatusModeEdit"]
							+ "</td></tr></table></td>");
			document.write("		<td class=SB_Mode_Sep></td>");
		...
		if (config.SBText) {
			document
					.write("		<td class=SB_Mode_BtnOff id=eWebEditor_TEXT onclick=\"setMode('TEXT')\" unselectable=on><table border=0 cellpadding=0 cellspacing=0><tr><td class=SB_Mode_Btn_Img>"
							+ getBtnImgHTML("ModeText")
							+ "</td><td class=SB_Mode_Btn_Text>"
							+ lang["StatusModeText"]
							+ "</td></tr></table></td>");
			document.write("		<td class=SB_Mode_Sep></td>");
		...
		if (config.SBView) {
			document
					.write("		<td class=SB_Mode_BtnOff id=eWebEditor_VIEW onclick=\"setMode('VIEW')\" unselectable=on><table border=0 cellpadding=0 cellspacing=0><tr><td class=SB_Mode_Btn_Img>"
							+ getBtnImgHTML("ModeView")
							+ "</td><td class=SB_Mode_Btn_Text>"
							+ lang["StatusModeView"]
							+ "</td></tr></table></td>");
		}
		...
		if (myParam.FullScreen != "1") {
			...
			document
					.write("		<table border=0 cellpadding=0 cellspacing=0 class=SB_Size>");
			document.write("		<tr>");
			document
					.write("		<td class=SB_Size_Btn onclick='sizeChange(300)' title='"
							+ lang["SizePlus"]
							+ "'>"
							+ getBtnImgHTML("SizePlus") + "</td>");
			document.write("		<td class=SB_Size_Sep></td>");
			document
					.write("		<td class=SB_Size_Btn onclick='sizeChange(-300)' title='"
							+ lang["SizeMinus"]
							+ "'>"
							+ getBtnImgHTML("SizeMinus") + "</td>");
	...
	document.write("</table>");
	document
			.write("<div id='eWebEditor_Temp_HTML' style='VISIBILITY: hidden; OVERFLOW: hidden; POSITION: absolute; WIDTH: 1px; HEIGHT: 1px'></div>");
	document.write("<div style='position:absolute;display:none'>");
	document.write("<form id='eWebEditor_UploadForm' action='"
			+ config.ServerExt + "/upload." + config.ServerExt
			+ "?action=remote&type=remote&style=" + myParam.StyleName
			+ "&cusdir=" + myParam.CusDir
			+ "' method='post' target='eWebEditor_UploadTarget'>");
	document.write("<input type='hidden' name='eWebEditor_UploadText'>");
	document.write("</form>");
	document
			.write("<iframe name='eWebEditor_UploadTarget' width=0 height=0 src='dialog/blank.htm'></iframe>");
	document.write("</div>");
	document
			.write("<div id=divProcessing style='width:200px;height:30px;position:absolute;display:none'>");
	document
			.write("<table border=0 cellpadding=0 cellspacing=1 bgcolor='#000000' width='100%' height='100%'><tr><td bgcolor=#3A6EA5><marquee id='msgProcessing' align='middle' behavior='alternate' scrollamount='5' style='font-size:9pt;color:#ffffff'></marquee></td></tr></table>");
	document.write("</div>");
}
	...
	for ( var i = 0; i < config.Toolbars.length; i++) {
		...
		for ( var j = 0; j < tb.length; j++) {
			...
			if (s_Code == "TBSep") {
				...
				result += "<td class=TB_Btn_Padding><div class='TB_Btn' title=\""
						+ lang[s_Code] + "\" onclick=\"" + a_Button[1] + "\">";
				if (typeof (a_Button[0]) == "number") {
					var s_Img = "skin/" + config.Skin + "/buttons.gif";
					var n_Top = 16 - a_Button[0] * 16;
					result += "<div class='TB_Btn_Image'><img src='" + s_Img
							+ "' style='top:" + n_Top + "px'></div>";
				} else {
				...
				case "FontSize":
					...
					for ( var k = 0; k < lang[s_Code + "Item"].length; k++) {
						s_Options += "<option value='"
								+ lang[s_Code + "Item"][k][0] + "'>"
								+ lang[s_Code + "Item"][k][1] + "</option>";
					}
					break;
				case "FormatBlock":
				...
				case "ZoomSelect":
					...
					for ( var k = 0; k < aZoomSize.length; k++) {
						s_Options += "<option value='" + aZoomSize[k] + "'>"
								+ aZoomSize[k] + "%</option>";
					}
					break;
				}
				result += "<td class=TB_Btn_Padding><select onchange=\""
						+ a_Button[1] + "\" size=1 " + s_FixedWidth
						+ "><option selected>" + lang[s_Code] + "</option>"
						+ s_Options + "</select></td>";
			}
		}
