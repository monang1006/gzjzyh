                         ...
                         ,aNeedIndex,aBaseIndex,aNeedCheckbox,aCheckBoxn,aSelectCheckn,aNeedLazy )
{
   ...
   this.HTOnClick		= HTOnClick
   this.HTVOnClick		=HTVOnClick
...
//for JTableTreeClass
function fJTTInitTreeData(aThis, ofid)
{
	var i
	var fid;
	var lNode;
	...
	**/
	...
	if(ofid == 0){
		...
		oNode = fJTTFindTreeNode(ofid, aThis.mNodes);
	}
	...
	}//  end of for(i =0;i<aThis.mNodes.items.length;i++)
	
	/**lazy edit**/
	if(aThis.aNeedLazy){
		for(i=0; i<oNode.subtree.items.length; i++){
			if(oNode.subtree.items[i].id in aThis.childNodes){
...
//for HelpTreeClass by LiuXi 20111215
function HTOnClick()
{
   var e;
   var lID, i;
   	...
   	case "SPAN":
        ...
        if( lGrandFather == null) return;
   	   	...
   	   	if(lFather.tagName == "TD" && lGrandFather.tagName == "TR")
   	   	{
				...
				//alert(lID);
				var lNode = this.SelectNode(lID);
				parent.HelpContent.location=scriptroot+"/helpedit/helpedit!input.action?helpTreeId="+lID;
   	   	   	}else
   	   	   	{
   	   	   	   return ;
   	   	   	}   	   	   
   	...
   	case "SPAN":
        ...
        if( lGrandFather == null) return;
   	   	...
   	   	if(lFather.tagName == "TD" && lGrandFather.tagName == "TR")
   	   	{
				...
				//alert(lID);
				var lNode = this.SelectNode(lID);
				parent.HelpContent.location=scriptroot+"/helpedit/helpedit!view.action?helpTreeId="+lID;	   	 
   	   	   	}else
   	   	   	{
   	   	   	   return ;
   	   	   	}   	   	   
...
function fJTTLazyBuildFromNode(aThis, fNode, aNode, aIsLast, aDisplay, rowIndex)
{
	...
	for(i = 0; i < aNode.TextArray.length; i ++){
		if( i == 0){
			...
			for(var j=0; j< fNode.imgBars.split(",").length-1; j++)
			{
...
function JTTLazyExpandOrClose(aThis, aBegin, aExpandOrClose,isFilter)
{
   ...
   //lazy initial data
   fJTTInitTreeData(aThis, aBegin.id);
