...
function CheckForm()
{
	...
	if(START_DATE>END_DATE||NOW_DATE>END_DATE)
	{
	   alert('请选择正确的时间！');
		return false;
	}
...
<INPUT type=hidden value="${vote.type}" name="vote.type">
<INPUT type=hidden value="${vote.vid}" name="vote.vid">
...
<table  border="0" cellpadding="0" cellspacing="0" class="table1" width="100%" >
  ...
  <TR>
    ...
    <INPUT class=text maxlength="50" size="50" name=vote.title value="${vote.title}">&nbsp;&nbsp;</TD></TR>
  <TR>
    <TD  class="biao_bg1" align="right"><span class="wz"><font color="red">*</font>&nbsp;显示样式：</span></TD>
    <TD class="td1" style="padding-left:5px;"> 
    <s:if test="vote.styleId!=null">
      	<s:select list="styleList" style="width:460"  listKey="styleId" listValue="styleName" id="vote.styleId" headerValue="请选择样式"  name="vote.styleId" value="vote.styleId" />
    </s:if>
    <s:else>
   ...
   <TR>
    ...
    <TD class="td1" style="padding-left:5px;"> 
      <script>document.write(showType(${vote.type}));</script>
    </TD></TR> 
  ...
  <TR>
    ...
    <TD class="td1" style="padding-left:5px;">
<c:choose>
   <c:when test="${vote.isRepeated eq 'Y'}"> 
   			          <input type="radio" name="vote.isRepeated" value="N">允许重复
     	  &nbsp;&nbsp;<input type="radio" name="vote.isRepeated" checked value="Y">不允许重复
   </c:when>
   <c:otherwise>  
     		 <input type="radio" name="vote.isRepeated" checked value="N">允许重复
  ...
  <TR>
    ...
    <TD class="td1" style="padding-left:5px;">
<c:choose>
   <c:when test="${vote.isPrivate eq 'Y'}"> 
   			 <input type="radio" name="vote.isPrivate" checked value="Y">限制查看
     	  &nbsp;&nbsp;<input type="radio" name="vote.isPrivate" value="N">不限制查看
   </c:when>
   <c:otherwise>  
     		 <input type="radio" name="vote.isPrivate" value="Y">限制查看
     	  &nbsp;&nbsp;<input type="radio" name="vote.isPrivate"  checked value="N">不限制查看
   </c:otherwise>
</c:choose>
    	</select>
      </TD>  </TR>
...
<INPUT class=button onclick="clearConten('SELECT_USER','SELECT_USER_NAME')" type=button value=清空 name=""> 
  ...
  <TR>
    ...
    <TD class="td1" style="padding-left:5px;"><TEXTAREA class=textarea name=vote.memo rows=5 wrap=yes cols=58 >${vote.memo}</TEXTAREA> 
    </TD></TR>
    
  <TR>
    <TD class="biao_bg1" align="right"><span class="wz"><font color="red">*</font>&nbsp;开始时间：</span></TD>
    <TD class="td1" style="padding-left:5px;">
    <strong:newdate name="vote.startDate" id="search1" width="30%" skin="whyGreen" isicon="true"  dateform="yyyy-MM-dd" nowvalue="${startDate}" ></strong:newdate>
    </TD></TR>
  <TR>
    <TD class="biao_bg1" align="right"><span class="wz"><font color="red">*</font>&nbsp;结束时间：</span></TD>
    <TD class="td1" style="padding-left:5px;">
    <strong:newdate name="vote.endDate" id="search2" width="30%" skin="whyGreen" isicon="true" dateform="yyyy-MM-dd"  nowvalue="${endDate}" ></strong:newdate>  
    </TD></TR>
