...
$(function(){
	...
	Cnrgb.mouseover(function(){
	  ...
	  $(".cnrgbl01 dd div.cn01ds").eq(i).show().siblings("div.cn01ds").hide();
	  if(i==0){
	  	$("#doMore1").attr("href","refreshWorkByTitle('<%=path%>/work/work!listtodo.action?type=sign','办公门户')");
	  }else if(i==1){
	  	$("#doMore1").attr("href","refreshWorkByTitle('<%=path%>/senddoc/sendDoc!todo.action?workflowType=3&type=notsign&formId=t_oarecvdoc','办公门户')");
	  }else if(i==2){
	  	$("#doMore1").attr("href","refreshWorkByTitle('<%=path%>/senddocLeader/sendDocLeader!listtodo.action?type=sign','办公门户')");
	  }else if(i==3){
	  	$("#doMore1").attr("href","refreshWorkByTitle('<%=path%>/senddocLeader/sendDocLeader!listprocessed.action?state=0&excludeWorkflowTypeName=收文办件登记','办公门户')");
	  }
  ...
  $(function(){
    var Cnrgb = $("#cnrgbl02 dt b");
	Cnrgb.mouseover(function(){
	  var i = Cnrgb.index(this);
	  $(this).addClass("cnr01bing").siblings("b").removeClass("cnr01bing");
	  $("#cnrgbl02 dd div.cn02ds").eq(i).show().siblings("div.cn02ds").hide();
	  if(i==0){
	  	$("#doMore2").attr("href","refreshWorkByTitle('<%=path%>/work/work!listprocessed.action?state=0','办公门户')");
	  }else if(i==1){
	  	$("#doMore2").attr("href","refreshWorkByTitle('<%=path%>/traceDoc/traceDoc.action','办公门户')");
	  }else if(i==2){
	  	$("#doMore2").attr("href","refreshWorkByTitle('<%=path%>/work/work!listprocessed.action?state=1&workflowType=2','办公门户') ");
	  }else if(i==3){
	  	$("#doMore2").attr("href","refreshWorkByTitle('<%=path%>/receives/recvDoc!todo.action','待处理文件')");
	  }else if(i==4){
	  	$("#doMore2").attr("href","refreshWorkByTitle('<%=path%>/sends/docSend!doclist.action','待发公文')");
	  }
	});	   
  });
		...
		//厅内文件检索
		function hallFilesearch(){
		
				var hallFileName = encodeURI(encodeURI($("#hallFileName").val()));
				//workflowDesign/action/processMonitor!monitorList.action?mytype=<%=request.getParameter("mytype") %>&state=<%=request.getParameter("state") %>&workflowType=<%=request.getParameter("workflowType") %>&excludeWorkflowType=<%=request.getParameter("excludeWorkflowType") %>
				navigate('<%=path%>/workflowDesign/action/processMonitor!orgFileSearchList.action?mytype=org&processName='+hallFileName,'厅内文件');
		}
		
		//发文检索
		function senddocFilesearch(){
		
		     var senddocFileName=encodeURI(encodeURI($("#senddocFileName").val()));
		     navigate('<%=path%>/workflowDesign/action/processMonitor!orgFileSearchList.action?mytype=org&workflowType=2&processName='+senddocFileName,'发文检索');
...
//			return html.replace("更多</a>","</a>");
    ...
    <script language="javascript">//切换我的文件、厅内文件
		   $(function(){
		     $("#wjqh dt span").mouseover(function(){
					  var i = $("#wjqh dt span").index(this);	
					  $(this).addClass("tit1").siblings("#wjqh dt span").removeClass("tit1");
					  $("#wjqh dd").eq(i).show().siblings("#wjqh dd").hide();
					});
		   });
   </script>
	</head>
	...
	<div id="kpc">
	  <div id="cont" class="clearfix">
	    <div class="conlef fle">
	      <div class="conld02">
	        <dl class="conldlist">
	          <dt class="tit"><span class="dtmore"><a style="font-size:14px; font-weight:normal; padding-right:12px;" href="#" onclick="refreshWorkByTitle('<%=path%>/notify/notify.action','办公门户')">更多>></a></span>通知公告</dt>
	          <dd style="height:177px; overflow:hidden;">
	                    ...
	                    <script type="text/javascript">
									...
									function ref_dbnf(){
										$.ajax({ type:"post",
											...
											data:{showNum:"7",subLength:"12",sectionFontSize:"14"},
	      ...
	      <div class="conld03" id="drag_402882a031c2e8ce0131c2ee583f0023">
	        <dl class="conldlist">
	         <dt class="tit"><span class="dtmore"><a style="font-size:12px; font-weight:normal; padding-right:12px;" href="#" onclick="refreshWorkByTitle('<%=path%>/infopub/articles/columnArticles.action?columnId=ff8080813002b86d0130030a7636023e&showtype=0','每周动态')" >更多>></a></span>每周动态</dt>
				<dd style="height:199px; overflow:hidden;">
				<input type="hidden" id="calDeskTopID" value="402882a031c2e8ce0131c2ee583f0023" />
	            <div class="cnrglists fle" type="1" id="mzdtcontent"> 
                    <script type="text/javascript">
								var mzdtcontent = document.getElementById("mzdtcontent");
								function ref_mzdt(){
									$.ajax({ type:"post",
										url:'<%=path%>/infopub/articles/articles!showTableDesk.action?'+new Date(),
										data:{ type:"sign",showDate:"0",showType:"0",showCreator:"0",showNum:"7",subLength:"13",sectionFontSize:"14",columnId:"ff8080813002b86d0130030a7636023e" },
										success:function(response){
											if(response==""){
											}else{
						                		mzdtcontent.innerHTML=replaceMore(response);
											}
										},
										error:function(data){
											//alert("对不起，操作异常"+data);
	    ...
	    <div class="conrig fri">
	      <div class="conrigdb clearfix">
	        <div class="cnrgblef fle">
             <div class="cnrgbl01" style="margin-top:0;">
	            <dl class="cnrgblist">
	              <dt class="tit">
          				<span class="dtmore"><a id="doMore1" href="javascript:refreshWorkByTitle('<%=path%>/work/work!listtodo.action?workflowType=&handleKind=&type=sign','办公门户')" >更多&gt;&gt;</a></span>
          				<b class="cnr01bing" id="bdtx"  style="width: 140px;"><a href="javascript:refreshWorkByTitle('<%=path%>/work/work!listtodo.action?type=sign','办公门户')">待办提醒<span><strong id="dbcount">0</strong></span></a></b>
          				<b style="width: 150px;"><a href="javascript:refreshWorkByTitle('<%=path%>/senddoc/sendDoc!todo.action?workflowType=3&type=notsign&formId=t_oarecvdoc','办公门户')" >待签收文件<span><strong id="dqcount">0</strong></span></a></b>
          				<b id="Lddbtx" style="width: 130px; display: none"><a href="javascript:refreshWorkByTitle('<%=path%>/senddocLeader/sendDocLeader!listtodo.action?type=sign','办公门户')">领导的待办提醒</a></b>
          				<b id="Ldybsy" style="width: 130px; display: none"><a href="javascript:refreshWorkByTitle('<%=path%>/senddocLeader/sendDocLeader!listprocessed.action?state=0&excludeWorkflowTypeName=收文办件登记','办公门户')">领导的已办事宜</a></b>
           		  </dt>
		           
		              <dd style="height:170px; overflow:hidden;">
		             <div class="content" id="dbsycontent"> 
							...
							function ref_dbsy(){
								$.ajax({ type:"post",
									...
									data:{ type:"sign",showDate:"1",showCreator:"1",showNum:"7",subLength:length1,sectionFontSize:"14"},	
									success:function(response){
										if(response==""){
										}else{
					                	   ...
					                	   if(dbsySize != null){
											   $("#dbcount").html("(共"+dbsySize+"条)");//待处理
											   $("#dbsySize").html(dbsySize);//待处理
					                	   }
										}
									},
									error:function(data){
				  ...
				  <!--  <div class="cnrglists" type="1" id="dbsycontent"> 
	                    <script type="text/javascript">
									...
									function ref_dbsy(){
										$.ajax({ type:"post",
											...
											data:{ showDate:"1",showCreator:"1",showNum:"7",subLength:length1,sectionFontSize:"14"},
											success:function(response){
												if(response==""){
												}else{
							                		dbsycontent.innerHTML=replaceMore(response);
		              	...
		              	<div id="dqscontent" style="display: none">
		              	  <script type="text/javascript">
							var dqscontent = document.getElementById("dqscontent");
							var width = screen.availWidth;
									var length1 = 12;
									if(width > 1024){
										length1 = 22;
							...
							function ref_dqs(){
								$.ajax({ type:"post",
									url:'<%=path%>/senddoc/sendDoc!getTodoByRedType.action?'+new Date(),
									data:{ type:"notsign",showNum:"7",showDate:"1",showCreator:"1",subLength:length1,sectionFontSize:"14" },
									success:function(response){
										if(response==""){
										}else{
					                		dqscontent.innerHTML=replaceMore(response);	
							                var dqwjSize =  $("#dqscontent").children(".listsize").val();
							                if(dqwjSize != null){
											   $("#dqcount").html("(共"+dqwjSize+"条)");//待签收
							                }
										}
									},
									error:function(data){
						 ...
						 <div style="display: none" id="lddbtxcontent"> 
	                        <script type="text/javascript">
							var lddbtxcontent = document.getElementById("lddbtxcontent");
							var width = screen.availWidth;
									var length1 = 12;
									if(width > 1024){
										length1 = 22;
							...
							function ref_lddbtx(){
								$.ajax({ type:"post",
									url:'<%=path%>/senddocLeader/sendDocLeader!getLdTodoByRedType.action?'+new Date(),
									data:{ type:"sign",showDate:"1",showCreator:"1",showNum:"7",subLength:length1,sectionFontSize:"14"},	
									success:function(response){
										if(response==""){
										         $("#Lddbtx").hide();
										}else{
					                		lddbtxcontent.innerHTML=replaceMore(response);	
					                		$("#Lddbtx").show();			                		 
										}
									},
									error:function(data){
						...
						<div style="display: none"  id="ldybsycontent"> 
	                        <script type="text/javascript">
								var ldybsycontent = document.getElementById("ldybsycontent");
								function ref_ldybsy(){
									$.ajax({ type:"post",
											url:'<%=path%>/senddocLeader/sendDocLeader!showLdTableDesktopDoneWork.action?'+new Date(),
											data:{ processStatus:"0",showCreator:"1",showDate:"1",showNum:"7",subLength:"15",sectionFontSize:"14",filterSign:"0",excludeWorkflowTypeName:"收文办件登记"},
											success:function(response){
												if(response==""){
												      $("#Ldybsy").hide();
												}else{
													var ldybsycontentHtml = response.replace("<div class=\"select\">","<div class=\"select\" style=\"display:none\">")
							                		ldybsycontent.innerHTML = replaceMore(response);	
							                		$("#Ldybsy").show();
												}
											},
											error:function(data){
	          ...
	          <div  id="cnrgbl02" class="cnrgbl01">
	            <dl class="cnrgblist">
	              <dt class="tit">
          				<span class="dtmore"><a id="doMore2"  href="javascript:refreshWorkByTitle('<%=path%>/work/work!listprocessed.action?state=0','办公门户')" >更多&gt;&gt;</a></span>
          				<b class="cnr01bing"><a href="javascript:refreshWorkByTitle('<%=path%>/work/work!listprocessed.action?state=0','办公门户')">已办事宜</a></b>
          				<b><a href="javascript:refreshWorkByTitle('<%=path%>/traceDoc/traceDoc.action','办公门户')" >文件跟踪</a></b>
          				<b><a href="javascript:refreshWorkByTitle('<%=path%>/work/work!listprocessed.action?state=1&workflowType=2','办公门户')" >发文办结</a></b>
          				
          				<security:authorize ifAllGranted="001-000700030001">
          				<b style="width: 130px;"><a href="javascript:refreshWorkByTitle('<%=path%>/receives/recvDoc!todo.action','待处理文件')" >待收公文<span><strong id="dclwjSize">0</strong></span></a></b>
          				</security:authorize>
          				<security:authorize ifAllGranted="001-000700030001">
          				<b style="width: 130px;"><a href="javascript:refreshWorkByTitle('<%=path%>/sends/docSend!doclist.action','待发公文')" >待发公文<span><strong id="dfgwSize">0</strong></span></a></b>
          				</security:authorize>
          				<security:authorize ifAllGranted="001-000100050006">
          				<b><a href="javascript:refreshWorkByTitle('<%=path%>/senddoc/sendDoc!showListYQ.action','每日要情')" >每日要情</a></b>
          				</security:authorize>
          				
          				<security:authorize ifAllGranted="001-000100050007">
          				<b><a href="javascript:refreshWorkByTitle('<%=path%>/senddoc/sendDoc!showListYQ.action?flag1=success','每日要情')" >每日要情</a></b>
          				</security:authorize>
           		  </dt>
		             <dd style="height:192px; overflow:hidden;">
		                <div class="content" id="zbwjcontent"> 
	                        <script type="text/javascript">
								var zbwjcontent = document.getElementById("zbwjcontent");
								function ref_zbwj(){
									$.ajax({ type:"post",
											url:'<%=path%>/senddoc/sendDoc!showTableDesktopDoneWork.action?'+new Date(),
											data:{ processStatus:"0",showCreator:"1",showDate:"1",showNum:"7",subLength:"15",sectionFontSize:"14",filterSign:"0"},
											success:function(response){
												if(response==""){
												}else{
							                ...
							                var ybsySize =  $("#zbwjcontent").children(".listsize").val();
										   $("#ybsySize").html("(共"+ybsySize+"条)");//待处理
												}
											},
											error:function(data){
												//alert("对不起，操作异常"+data);
		              	...
		              	<div id="zywjcontent" style="display: none">
		              	   <script type="text/javascript">
										...
										function ref_zywj(){
											$.ajax({ type:"post",
													...
													data:{showCreator:"1",showDate:"1",showNum:"7",subLength:length2,sectionFontSize:"14"},
													success:function(response){
														if(response==""){
														}else{
						 ...
						 <div  id="fwbjcontent" style="display: none"> 
	                        <script type="text/javascript">
								var fwbjcontent = document.getElementById("fwbjcontent");
								function ref_fwbj(){
									$.ajax({ type:"post",
											url:'<%=path%>/senddoc/sendDoc!showDesktopSendDocDoneWork.action?'+new Date(),
											data:{showCreator:"1",showDate:"1",showNum:"7",subLength:"15",sectionFontSize:"14"},
											success:function(response){
												if(response==""){
												}else{
													var fwbjcontentHtml = response.replace("<div class=\"select\">","<div class=\"select\" style=\"display:none\">")
							                		fwbjcontent.innerHTML = replaceMore(response);	
							                var ybsySize =  $("#fwbjcontent").children(".listsize").val();
										   $("#ybsySize").html("(共"+ybsySize+"条)");//待处理
												}
											},
											error:function(data){
						...
						<security:authorize ifAllGranted="001-000700030001">
						<div id="mydbsycontent" style="display: none"> 
		                	<script type="text/javascript">
										var mydbsycontent = document.getElementById("mydbsycontent");
										function myref_dbsy(){
											$.ajax({ type:"post",
												...
												success:function(response){
													if(response==""){
								                		...
								                		mydbsycontent.innerHTML=replaceMore(response);
								                		var t = $("#mydbsycontent table").length;
								                		var t = response.substring(response.lastIndexOf("\"")+2,response.length-7);
														$("#dclwjSize").html("(共"+t+"条)");//待处理
													}
												},
												error:function(data){
													//alert("对不起，操作异常"+data);
						...
						<security:authorize ifAllGranted="001-000700030001">
						<div id="mydfsycontent" style="display: none"> 
		                	<script type="text/javascript">
										var mydfsycontent = document.getElementById("mydfsycontent");
										function myref_dfsy(){
											$.ajax({ type:"post",
												url:'<%=path%>/sends/docSend!showDesktop.action',
												data:{ showDate:"1",showCreator:"1",showNum:"7",subLength:22,sectionFontSize:"14"},
												success:function(response){
													if(response==""){
													}else{
								                		mydfsycontent.innerHTML=replaceMore(response);
								                		var t = response.substring(response.lastIndexOf("\"")+2,response.length-7);
														$("#dfgwSize").html("(共"+t+"条)");//待处理
													}
												},
												error:function(data){
													//alert("对不起，操作异常"+data);
						...
						<security:authorize ifAllGranted="001-000100050006">
						 <div style="display: none"  id="mryq"> 
	                        <script type="text/javascript">
								function ref_mryq(){
									$.ajax({
										url: "<%=path%>/senddoc/sendDoc!showTableYQ.action",
						...
						<security:authorize ifAllGranted="001-000100050007">
						 <div style="display: none"  id="mryq1"> 
	                        <script type="text/javascript">
								function ref_mryq1(){
									$.ajax({
										url: "<%=path%>/senddoc/sendDoc!showTableYQ.action?flag1=success",
										success: function(data){
											$("#mryq1").html(data);
										},
										cache: false,
	        ...
	        <div class="cnrgbrig fri">
	          <div class="cnrgbr02 cnrgbr04">
                <div class="crgbrsyj"><div></div></div>	
                  <dl id="wy">
                    <dt class="tit"><span class="tit1">我的文件</span></dt>
                    <dd class="cnrgbr2dd clearfix qh">	
                      <p class="searp"><input type="text" id= "processNameTEXT" class="inptext" /><input type="button" class="inpbutt" onclick="search();"/></p>
                    </dd>
                  </dl>
                  <security:authorize ifAnyGranted="001-00060025,001-00060024">
                  <dl id="wjqh" style="margin-top:10px;">
                  <dt class="tit"> <span class="tit1">发文文件</span> <security:authorize ifAllGranted="001-00060024"><span>厅内文件</span></security:authorize></dt>
                    <dd class="cnrgbr2dd clearfix">	
                    <p class="searp"><input type="text" id= "senddocFileName" class="inptext" /><input type="button" class="inpbutt" onclick="senddocFilesearch();"/></p>
                  </dd>
                  <security:authorize ifAllGranted="001-00060024">
                   <dd class="cnrgbr2dd clearfix" style="display:none;">	
                    <p class="searp"><input type="text" id= "hallFileName" class="inptext" /><input type="button" class="inpbutt" onclick="hallFilesearch();"/></p>
                  </dd>
                  </security:authorize>
                  </dl>
                  </security:authorize>
              ...
              <div class="cnrgbr02 cnrgbr03">
	            <div class="crgbrsyj"><div></div></div>	
	            <dl id="wy">
	              <dt class="tit"><span class="tit1">快捷菜单</span><span class="dcdsz"><a href="#" onclick="openLink('<%=root%>/shortcutmenu/fastMenu.action?param=person','办公门户')">菜单设置</a></span></dt>
				  <!--   <dd class="cnrgbr3dd clearfix" style=" min-height:270px; height:auto!important; height:270px;overflow:hidden;">	-->
				   <dd class="cnrgbr3dd clearfix" style="height:150px; overflow:hidden;">
	                <ul>
	                	<s:iterator id="ufl" value="userFMList" status="ufl">
	                	
	                	<s:if test="#ufl.privilName=='个人日程'">                	
						...
						<s:elseif test="#ufl.privilName=='个人信息'">
						...
						<s:elseif test="#ufl.privilName=='发送记录'">
						...
						<s:elseif test="#ufl.privilName=='已办事宜'">
						...
						<s:elseif test="#ufl.privilName=='我的请求'">
						...
						<s:elseif test="#ufl.privilName=='公文回收站'">
						...
						<s:else>
							<li><a href="#" onclick="navigate('<%=root%><s:property value="privilAttribute" />','办公门户')" title="<s:property value="privilName" />"><s:if test="privilName.length()>4"><s:property value="privilName.substring(0,4)" />...</s:if><s:else><s:property value="privilName" /></s:else></a></li>
						</s:else>
						</s:iterator>
					<!-- 
	          ...
	          <div class="cnrgbr02 cnrgbr05">
	            ...
	            <dl>
	              <dt class="tit">办件统计</dt>
		              <dd class="cnrgbr2dd" id="bjtj" style="height:18px; overflow:hidden;">	
		                <ul>
		                <li><a href="#" onclick="refreshWorkByTitle('<%=path%>/work/work!listtodo.action','办公门户')">
		                  		待办<span><strong id="dbsySize">0</strong></span>件
		                  		<a href="#" onclick="refreshWorkByTitle('<%=path%>/work/work!listprocessed.action?state=0','办公门户')">
		                  		已办<span><strong id="zbwjSize">0</strong></span>件
		                  		</a>
		                  		<a href="#" onclick="refreshWorkByTitle('<%=path%>/work/work!listprocessed.action?state=1','办公门户')">
		                  		办结<span><strong id="bjwjSize">0</strong></span>件
		                  		</a></li>
		                ...
		                <!--  
		                  ...
		                  <li><a href="#" onclick="refreshWorkByTitle('<%=path%>/senddoc/sendDoc!processedWorkflow.action?state=0&filterSign=1&excludeWorkflowType=370020,413460','办公门户')">
		                  		在办<span> <strong id="zbwjSize">0</strong></span> 件
		                  		</a></li>
	         ...
	         <!--  
	          <div style="margin-top:12px;">
		          <a href="#" onclick="refreshWorkByTitle('<%=path%>/workflowDesign/action/processMonitor!mainFrameOrg.action?model=person','办公门户')">
		          	<img src="<%=root %>/frame/theme-web/webFtabforRB/images/lcjcx.jpg" />
		          </a>
	          </div>
	               -->
	        </div>
	...
	<script>
		function reLoadDate() {
			...
			ref_zbwj();
			//刷新待处理文件
			if($("#mydbsycontent").length>0){
			myref_dbsy();
			}
			//刷新待发公文
			if($("#mydfsycontent").length>0){
			myref_dfsy();
			}
			//刷新通知公告
			ref_dbnf();
