	...
	<div id="kpc">
	  <div id="cont" class="clearfix">
	    <div class="conlef fle">
	      <div class="conld02">
	        <dl class="conldlist">
	          <dt class="tit">通知公告<a style="font-size:12px; font-weight:normal; padding-left:100px;" href="#" onclick="refreshWorkByTitle('<%=path%>/notify/notify.action','通知公告')">更多>></a></dt>
	      ...
	      <div class="conld03" id="drag_402882a031c2e8ce0131c2ee583f0023">
	        <dl class="conldlist">
	          <dt class="tit">个人日程<a style="font-size:12px; font-weight:normal; padding-left:100px;" href="#"  onclick="refreshWorkByTitle('<%=path%>/calendar/calendar.action','个人日程')">更多>></a></dt>
	                    ...
	                    <script type="text/javascript">
									...
									function ref_dbpc(){
										$.ajax({ type:"post",
											...
											data:{showNum:"7",subLength:"11",sectionFontSize:"14"},
											success:function(response){
												if(response==""){
												}else{
	    ...
	    <div class="conrig fri">
	      <div class="conrigdb clearfix">
	        <div class="cnrgblef fle">
              <div class="conrigdt">
		         <div style="position:absolute; z-index:9999; top:6px; left:100px; right:12px;">
					<span style="float:right;"><a style="font-size:12px; color:#FFF; font-weight:normal;" href="#" onclick="refreshWorkByTitle('<%=path%>/senddoc/sendDoc!todoWorkflow.action?workflowType=-116153&type=sign','待办提醒')">更多>></a></span><span style="font-size:14px; color:#FFF; "><a style="color:#FFF; font-weight:normal;" herf="#" onclick="refreshWorkByTitle('<%=path%>/senddoc/sendDoc!todoWorkflow.action?workflowType=-116153&type=sign','待办提醒')"><strong id="dbcount">0</strong></a> 条</span></div>
	        ...
	        <dl>
	          ...
	          <div class="cnrgbl01">
	            <dl class="cnrgblist">
	              <dt class="tit"><span class="dtmore"><a href="#" onclick="refreshWorkByTitle('<%=path%>/traceDoc/traceDoc.action','重要文件跟踪')">更多&gt;&gt;</a></span>重要文件跟踪</dt>
	          ...
	          <div class="cnrgbl02">
	            <dl class="cnrgblist">
	              <dt class="tit"><span class="dtmore"><a href="#" onclick="refreshWorkByTitle('<%=path%>/senddoc/sendDoc!processedWorkflow.action?state=0&filterSign=1&excludeWorkflowType=370020,413460','已办事宜')">更多&gt;&gt;</a></span>已办事宜</dt>
	        ...
	        <div class="cnrgbrig fri">
              ...
              <div class="cnrgbr02 cnrgbr03">
	            <div class="crgbrsyj"><div></div></div>	
	            <dl>
	              <dt class="tit"><span class="dcdsz"><a href="#" onclick="openLink('<%=root%>/shortcutmenu/fastMenu.action?param=person','快捷菜单设置')">菜单设置</a></span>快捷菜单</dt>
				   <dd class="cnrgbr3dd clearfix" style=" min-height:243px; height:auto!important; height:243px;overflow:hidden;">	
	                <ul>
	                	<s:iterator id="ufl" value="userFMList" status="ufl">
	                	
	                	<s:if test="#ufl.privilName=='新建流程'">                	
							 <li class="cnrddli01"><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:if> 
						<s:elseif test="#ufl.privilName=='手机短信'">
							<li class="cnrddli02"><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:elseif>
						
						<s:elseif test="#ufl.privilName=='流程查询'">
							<li class="cnrddli03"><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:elseif>
						
						<s:elseif test="#ufl.privilName=='办结事宜'">
							<li class="cnrddli04"><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:elseif>
						
						<s:elseif test="#ufl.privilName=='系统通讯录'">
							<li class="cnrddli05"><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:elseif>
						
						<s:elseif test="#ufl.privilName=='意见管理'">
							<li class="cnrddli06"><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:elseif>
						<s:elseif test="#ufl.privilName=='个人日志'">
							<li class="cnrddli07"><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:elseif>
						
						<s:elseif test="#ufl.privilName=='常用下载'">
							<li class="cnrddli08"><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:elseif>
						
						<s:else>
							<li><a href="#" onclick="top.bottomFrame.navigate('<%=root%><s:property value="privilAttribute" />','<s:property value="privilName" />')"><s:property value="privilName" /></a></li>
						</s:else>
						</s:iterator>
					<!-- 
          ...
          <div class="cnrgbr01">
            <h3>消息提醒</h3>
            <p>您有 <span><strong id="unread"><%=unreadCount%></strong></span> 条未阅读通知，请及时<a href="#" onclick="refreshWorkByTitle('<%=path%>/fileNameRedirectAction.action?toPage=message/message.jsp','我的消息')">查看</a></p>
          </div>
	          <div class="cnrgbr02">
	            <div class="crgbrsyj"><div></div></div>	
	            <dl>
	              <dt class="tit">办件统计</dt>
		              <dd class="cnrgbr2dd" id="bjtj" style="height:120px; overflow:hidden;">	
		                <ul>
		                  <li><a href="#" onclick="refreshWorkByTitle('<%=path%>/senddoc/sendDoc!todoWorkflow.action?workflowType=-116153&type=sign','待办文件')">
		                  		我的待办文件<span> <strong id="dbsySize">0</strong></span> 件
		                  		</a></li>
		                  <li><a href="#" onclick="refreshWorkByTitle('<%=path%>/senddoc/sendDoc!processedWorkflow.action?state=0&filterSign=1&excludeWorkflowType=370020,413460','在办文件')">
		                  		我的在办文件<span> <strong id="zbwjSize">0</strong></span> 件
		                  		</a></li>
		                  <li><a href="#" onclick="refreshWorkByTitle('<%=path%>/senddoc/sendDoc!processedWorkflow.action?state=1&filterSign=1','办结文件')">
	          ...
	          <div style="margin-top:12px;">
		          <a href="#" onclick="refreshWorkByTitle('<%=path%>/workflowDesign/action/processMonitor!mainFrameOrg.action?model=person','流程查询与监控')">
		          	<img src="<%=root %>/frame/theme-web/webTab/images/lcjcx.jpg" />
		          </a>
	          </div>
	        </div>
	      </div>
