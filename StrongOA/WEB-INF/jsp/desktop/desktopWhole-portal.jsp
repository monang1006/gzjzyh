...
<%
	Integer rt=(Integer)session.getAttribute("refreshTime");
	String refreshTime=rt.toString();
...
<html>
	<head>
		...
		function addDragDiv(blockid,col,pos,isnotinit,blocktpl,imgurl,blocktitle,blocktype,blockurl){
			...
			mvAry[mvAry.length]  ='		<table width="100%"  border="0" cellspacing="0" border="0">';
...
//			objDragTitle.style.backgroundImage = "url('<%=path%>/oa/image/desktop/index_i/tpl/"+tpl+"/title_bg.png')";
		...
		function modifyBlock(id){
			...
			//编辑时，只展现编辑内容，不展现数据内容
  		    document.getElementById('drag_content_'+id).style.display="none";
			if(objDivID.style.display == ""){
			  //展现数据内容
			  document.getElementById('drag_content_'+id).style.display="";
		...
		function switchOptionImg(blockid,n){
			var isClose = 0
			try{
				isClose =  document.getElementById('drag_isclose_'+blockid).value;
			}catch(e){
			}
			if(n==1){
	...
	//日程模块-万年历调用的方法
		...
		function refresh(){
			...
			window.parent.refreshCurrentWork(window);
		...
		if(parseInt(refreshTime)!= 0 ){
			//alert(refreshTime);
			setInterval("refresh()",parseInt(refreshTime)*1000);
		}
	...
	<div id="detail" class="UserCard" style="display:none" >
		...
		<div id="contentborder" style="margin-left: 0px;">
			<div style="width:100%;">
				...
				<!-- 
				<div style="float:right;width:200px;color:#999;height:20px;">	
					<font size='3'>【字体：</font>
					<a href="javascript:doZoom(22)">特大</a>			
					<a href="javascript:doZoom(16)">标准</a>
					<font size='3'>】</font>
				</div>
				 -->
			</div>
			...
			<div id=test>
				<div id="col_1" class="col_div" style="width:${layout1}%">
					<s:iterator value="col1">
					 <input type='hidden' id='num_<s:property value="sectionId"/>'  value='<s:property value="sectionRow"/>' />
								...
								<table width="100%" cellpadding="0" cellspacing="0" border="0">
									<tr>
										<td style="width:142px;font-weight:bold;padding:2px">
											...
											<input type="hidden" id="drag_isclose_<s:property value="sectionId"/>" value="<s:property value="isClose"/>" />
										</td>
										<td>
				...
				<s:if test="col3!=null&&col3.size>0">
					<div id="col_3" class="col_div" style="width:${layout3}%">
						<s:iterator value="col3">
						 <input type='hidden' id='num_<s:property value="sectionId"/>'  value='<s:property value="sectionRow"/>' />
							<div id="drag_<s:property value="sectionId"/>" class="drag_div" style="background:#FFF;" url="<%=path%><s:property value="sectionurl"/>">
								<div style="width:100%;background:url(<%=path%>/oa/image/desktop/index_i/tpl/<s:property value="sectionColor"/>/title_bg.png);height:25px;" id="drag_title_<s:property value="sectionId"/>">
									<input type="hidden" name="blocktypevalue_<s:property value="sectionId"/>" id="blocktypevalue_<s:property value="sectionId"/>" value="<s:property value="sectionType"/>">
									<table width="100%" cellpadding="0" cellspacing="0" border="0">
										<tr>
											<td style="width:142px;font-weight:bold;padding:2px">
			...
			<div id="popupConMenuID" style="display:none;position:absolute;z-index:10;width:200px;top:20px;left:15px;border: 1px solid #A2C7D9;background-color: #F1F7F9;padding:5px">
				...
				<div id="panelmodifycol">
					...
					<div id="layoutdisplay1" style="width:100%;padding:5px;color:#333333;display:">
						...
						<a class="button" style="cursor: pointer;" onClick="changeColWidth();">确定</a>&nbsp;
					</div>
