...
<TABLE cellSpacing=0 cellPadding=3 width="100%" border=0>
  ...
  <BR>
<FORM name=form1 action="<%=root %>/survey/survey!edit.action" method=post>
<script type="text/javascript" src="<%=path%>/common/js/newdate/WdatePicker.js"></script>
<INPUT type=hidden value=0 name=model.state>
<INPUT type=hidden value="${model.surveyId}" name=model.surveyId>
<input type=hidden   name=model.surveyQusSort value="${model.surveyQusSort}" />
<input type=hidden   name=model.surveyCount value="${model.surveyCount}" />
<input type=hidden   name=model.tableTitle value="${model.tableTitle}" />
...
border=0>
  ...
  <TR>
    ...
    <INPUT class=text  name=model.surveyName value="${model.surveyName}">&nbsp;&nbsp;</TD></TR>
  <TR>
    <TD class=tablecol2 noWrap align=right>显示样式：</TD>
    <TD class=tablecol1 noWrap> 
      <s:select list="styleList" style="width:460"  listKey="styleId" listValue="styleName" id="model.surveyStyleId" headerValue="请选择样式"  name="model.surveyStyleId"  />
    </TD></TR>
  <TR>
...
<INPUT class=button onclick="clearConten('SELECT_USER','SELECT_USER_NAME')" type=button value=清空 name=""> 
  ...
  <TR>
    ...
    <TD class=tablecol1 noWrap><TEXTAREA class=textarea name=model.explain rows=5 wrap=yes cols=58 >${model.explain}</TEXTAREA> 
    </TD></TR>
    
  <TR>
    <TD class=tablecol2 noWrap align=right>开始时间(<font color="red">*</font>)：</TD>
    <TD class=Tablecol1>
    <strong:newdate name="model.surveyStartTime" id="search1" width="30%" skin="whyGreen" isicon="true"  dateform="yyyy-MM-dd" nowvalue="${startTime}" ></strong:newdate>
    </TD></TR>
  <TR>
    <TD class=tablecol2 noWrap align=right>结束时间(<font color="red">*</font>)：</TD>
    <TD class=tablecol1 noWrap>
    <strong:newdate name="model.surveyEndTime" id="search2" width="30%" skin="whyGreen" isicon="true" dateform="yyyy-MM-dd"  nowvalue="${endTime}" ></strong:newdate>  
    </TD></TR>
