...
function setOrgId(){
		//orgId = id;
		//refreshWorkByTitle('<%=root%>/workinspect/worktodo/worktodo!list.action,'个人办理');
		//refreshWorkByTitle('<%=root%>/workinspect/worktodo/worktodo!returnList.action,'部门办理');
}
</SCRIPT>

<script for="window" event="onload">
	state = true;//初始化完毕
	navigate('<%=path%>/workinspect/worktodo/workTodo!list.action','个人办理');
	//navigate('<%=path%>/workinspect/worktodo/workTodo!returnList.action','部门办理');
	setSelectedIndex(0);
	try{
		msg2.StartListen();
	}catch(e){
