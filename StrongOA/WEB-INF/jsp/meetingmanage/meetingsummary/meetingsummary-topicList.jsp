  ...
  //alert(noticeIs);
	...
	<BODY class=contentbodymargin oncontextmenu="return false;"
      ...
      src="<%=path%>/common/js/newdate/WdatePicker.js"></script>
		...
		<s:form theme="simple" id="myTableForm" action="/meetingmanage/meetingsummary/meetingsummary!topicList.action">
...
<c:forEach items="${page.result}" var="obj" varStatus="status">
         	 	var bid = '<c:out value="${obj.topicId}"/>';
         	 	if(bid == id){
         	 		ss = '<c:out value="${obj.topicStatus}"/>';//议题ID
         	 	}
  </c:forEach>
   // alert(ss);
   if(ss=='0'){
   		alert('该会议还在准备当中不能填写纪要！');
   		return ;
   }
   else{
//  var result=window.showModalDialog("<%=path %>/meetingmanage/meetingsummary/meetingsummary!input.action?meetingId="+id,window,'help:no;status:no;scroll:no;dialogWidth:700px; dialogHeight:550px');
 // window.close();
 --%>
  document.forms(0).action="<%=path %>/meetingmanage/meetingsummary/meetingsummary!input.action?meetingId="+id;
