					...
					<td height="10px">
						<div class="dtree">
						  ...
						  function setNodeItem(checkFlag, id, name) {
						      if (checkFlag) {
						        parent.addOption("corg", name, id);						        
						      } else {
						        parent.removeOption("corg", name, id);
						      }
						  }
...
//					        }
                          ...
                          var d = new dTree('d');           
						  d.add('c${topTreeNodeId}','-1','组织机构','','','','<%=root%>/workflow/images/tree/folder_closed.gif','2');
						  <c:set var="topTreeId" value="${topTreeNodeId}"/>
						  <c:forEach items="${orgList}" var="tree" varStatus="status">
						  <c:choose>
						    <c:when test="${tree[1] == topTreeId}">
						      d.add('c<c:out value="${tree[0]}"/>','c${topTreeNodeId}','<c:out value="${tree[2]}"/>','','','','<%=root%>/workflow/images/tree/folder_closed.gif','-2');
						    </c:when>
						    <c:otherwise>
						      d.add('c<c:out value="${tree[0]}"/>','c<c:out value="${tree[1]}"/>','<c:out value="${tree[2]}"/>','','','','<%=root%>/workflow/images/tree/folder_closed.gif','-2');
						    </c:otherwise>
						  </c:choose>						  
						  </c:forEach>
						  document.write(d);
						   ...
						   var aHasSelected = "";
							...
							if(initDatas.length > 0){
								...
								for(var j=0; j<initDatas.length; j++){
									if(initDatas[j].substring(0, 1) == 'c'){
