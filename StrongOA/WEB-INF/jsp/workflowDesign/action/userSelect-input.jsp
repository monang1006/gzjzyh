...
<html>
	...
	<body class="contentbodymargin">
					...
					<tr>
						<td width="100%">
							<table border="0" width="100%" cellpadding="0" cellspacing="0">
								...
								<tr>
									<td valign='middle'>
										<img src='<%=frameroot%>/images/tb-change.gif'
		...
		<form action="" id="hiddenForm" name="hiddenForm" method="post" style="width:0px; height:0px; display:none;">
			<input id="initData" name="initData" type="hidden" />
		</form>
<script>
	...
	$(window).load(function(){
		function changeTree(alias, url) {
		    ...
		    if($("#frame_" + alias).attr("visited") == "0"){
		    	...
		    	**/
		    	...
		    	$("#initData").val(selected);
		    	$("#hiddenForm").attr("action", url).attr("target", "frame_" + alias).submit();
		...
		if(actorSettings.length > 0){
			...
			for(var i=0; i<actorSettings.length; i++){
				...
				var selected = actorSettings[i].name;
				selected = selected.substring(0,selected.length-2)+"已选";
				...
				$("#frameDiv").append("<iframe visited='0' id='frame_" + alias + "' name='frame_" + alias + "' frameborder='0' scrolling='no' style='width:100%; height:100%;' style='display:none;' />");
				
				$("#selectDiv").append("<div align='left'>"+ selected +"</div><div align='left'><select id='select_" + alias + "' size='100' multiple='multiple' style='width:200px; height:" + selectHeight + "px;'></select><span style='width:50px;'><input type='button' value='置顶' onclick='moveUp(1,\"select_" + alias + "\")'><input type='button' value='上移' onclick='moveUp(2,\"select_" + alias + "\")'><input type='button' value='下移' onclick='moveUp(3,\"select_" + alias + "\")'><input type='button' value='置底' onclick='moveUp(4,\"select_" + alias + "\")'></span></div>");
