					...
					<td height="10px">
						<div class="dtree">
						  ...
						  function setNodeItem(checkFlag, id, name) {
						      if (checkFlag) {
						        parent.addOption("person", name, id);						        
							      } else {
						  ...
						  <c:forEach items="${orgList}" var="tree" varStatus="status">
						      <c:forEach items="${tree[3]}" var="user" varStatus="status">
						          d.add('u<c:out value="${user[0]}"/>$<c:out value="${tree[0]}"/>','g<c:out value="${tree[0]}"/>','<c:out value="${user[1]}"/>','','','','<%=root%>/workflow/images/tree/renyuan.gif','-2');
						      </c:forEach>
						  </c:forEach>
