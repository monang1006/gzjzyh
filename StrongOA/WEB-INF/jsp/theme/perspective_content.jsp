...
<%
	String side2 =  "200"; 		// 左侧导航菜单宽度 12号字体时为170px；16号字体时200px
	if(themePath.indexOf("12")>0){
		side2 = "170";
	}
...
<html>
	<head>
		...
		<style type="text/css">
             ...
             .side2 { position: absolute; height:100%; background:#fff; overflow:hidden;  float:left; width:<%=side2%>px; z-index:999; display:none; left:32px;}
             .side3 { position: relative; left:0px;}
             .main { position:relative; overflow:hidden; height:100%; margin-left:36px; _margin-left:0; z-index:2; border-left:1px solid #999;}
		...
		function displaySubPrivil(index, text, id, privilCode){
			...
			if(subDiv.attr("flag") != "true"){
				...
				$.ajax({
	              ...
	              //  data: {privilCode: privilCode, sysSysCode: "<%=sysCode%>"},
	                ...
	                data: {priviparent: privilCode, sysSysCode: "<%=sysCode%>",url:"<%=themePath%>"},
	                async: false,
	                dataType: "json",
			...
			$("#side2").show();
			//$("#menu_lock").click();
			//alert($("#menu_lock").hasClass("tree_menu_lock"));
		...
		//修改点击菜单后打开页面的方法 modifyby luosy 2013-01-24
			document.mainIframe.refreshWorkByTitle(url, name,id);
			return;
		/*
		...
		function suoding(){
			if(!$("#side2").hasClass("side3")){
			//$("#side2").removeClass("side2");
			   $("#side2").addClass("side3");
			}
			if(!$("#main").hasClass("main2")){
			//$("#main").removeClass("main");
			   $("#main").addClass("main2");
			}
			
			if(!$("#menu_lock").hasClass("tree_menu_lock")){
			    $("#menu_lock").removeClass("tree_menu_unlock");
			   $("#menu_lock").addClass("tree_menu_lock");
			}
	...
	<body>
		...
		<div id="side2" class="side2" style="height:100%">
			<div class="tree_menu" id="tree_menu">
			  <div class="tremenk">
			  <div class="tremenkn">
				<div class="tree_menu_unlock" id="menu_lock" title="收起"></div>
		...
		<div id="main" class="main">
			<iframe id="mainIframe" name="mainIframe" width="100%"
				src="<%=path%>/fileNameRedirectAction.action?toPage=/theme/nav_right.jsp"
