...
<script for="window" event="onload">
	state = true;//初始化完毕
	if("${param.taskId}"!="" || "${param.instanceId}"!=""){
		navigate("<%=root%>/senddoc/sendDoc!annallist.action?taskId=${param.taskId}&instanceId=${param.instanceId}","办理记录");
		if("${param.type}"=="processurgency"){
			navigate("<%=root%>/senddoc/sendDocProcessUrgency.action?processurgency.processInstanceId=${param.instanceId}","催办记录");
		}
	}
	$.ajax({
			type:"post",
		...
		<TABLE onselectstart="return false" 
			...
			<TR>
				<TD style="padding-left: 0">
					<DIV id="tabContainer"
						style="position: absolute; top: 2; width: 90%; height: 22; font-size: 14px; overflow-y: no; overflow-x: hidden;">
