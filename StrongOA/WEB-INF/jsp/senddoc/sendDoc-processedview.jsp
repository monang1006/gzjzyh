   ...
   function transmitting(){
	...
	<body class="contentbodymargin" onload="window.focus();gotoDone();">
		...
		<form id="form" action="<%=root%>/senddoc/sendDoc!list.action"
			...
			<!--判断是否是转发的邮件 -->
			<input type="hidden" id="sendMailfalg" name="sendMailfalg"
				value="${sendMailfalg}" />
		</form>
						...
						<table width="100%" border="0" align="right" cellpadding="0"
							...
							<tr>
								...
								<td>
									<table align="right">
										<tr>
											<!-- 增加意见征询反馈功能 businessType=0
											<s:if test="state!=1 && processSuspend==0 && flag=='true'">
												<td
													style='businessType =="0"?"display: block ":" display : none "'>
												...
												<td width="5"
													style='businessType =="0"?"display: block ":" display : none "'></td>
												<td
													style='businessType =="0"?"display: block ":" display : none "'>
													<a class="Operation" href="#"
											...
											<s:if test="processSuspend==0">
												<td style="display: none;" id="td_todone">
													<a class="button" href="#"
														onclick="javascript:doGottoDone();">代办意见</a>&nbsp;
												</td>
												<td width="5"></td>
											</s:if>
											...
											<s:if test="processSuspend==0">
												<td style="display: none;" id="td_toredone">
													<a class="button" href="#"
														onclick="javascript:doGotoReDone();">重新办理</a>&nbsp;
												</td>
												<td width="5"></td>
											</s:if>
											<s:if
												test="state != 1 && processSuspend==0 && #request.suporgcode==001&&sendMail!='true'">
												<td>
													<a class="button" href="#"
														onclick="javascript:sendReminder();">催办&nbsp;</a>&nbsp;
												</td>
												<td width="5"></td>
											</s:if>
											<s:if test="sendMail!='true'">
												<td>
													<a class="button" href="#"
											...
											<%
												String isFirstYjzx = (String) (request.getParameter("isFirstYjzx") == null ? ""
														: request.getParameter("isFirstYjzx"));
												if (!"true".equals(isFirstYjzx)) {
											%>
											<s:if
												test="state != 1 && processSuspend==0 && #request.suporgcode==001&&sendMail!='true'">
												<td>
													<a class="button" href="#"
														onclick="JavaScript:fetchTask();">取回流程</a>&nbsp;
												</td>
												<td width="5"></td>
											</s:if>
											<%
											...
											%>
											<s:if
												test="state != 1 && processSuspend==0&&sendMail!='true' ">
												<td id="mustFetchTask" style="display: none;">
													<a class="button" href="#"
														onclick="JavaScript:mustFetchTask();">强制取回</a>&nbsp;
												</td>

												<td id="mustFetchTasksplit" style="display: none;" width="5">
											</s:if>
											<%--<td>
											...
											<s:if
												test="(workflowName == '征求意见流程' || workflowName == '公文转办流程') && instanceId != null">
												<td>
													<a class="button" href="javascript:transmitting();">转办记录</a>&nbsp;
												</td>
												<td width="5"></td>
											</s:if>
											<s:if
												test="(state!=1) && (processSuspend==0) && (flag=='true') && (#request.showrepeal == 1)&&sendMail!='true'">
												<td>
