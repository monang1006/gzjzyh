...
String orgId=request.getParameter("orgId") ;
	...
	ToaGlobalConfig toaGlobalConfig = globalConfigService.getToaGlobalConfig("plugins_overrule");
	String overRuleView = "0";
	if(toaGlobalConfig != null){//0:列表，1：流程图，2:二者都有
		if(toaGlobalConfig.getGlobalValue() != null){
			overRuleView = toaGlobalConfig.getGlobalValue();
		}
	}
%>
<HTML>
...
<script for="window" event="onload">
	state = true;//初始化完毕
	if("${param.taskId}"!=""){
		var overRuleView = "<%=overRuleView%>";
		if(overRuleView == "0"){//列表
			navigate("<%=root%>/senddoc/sendDocWorkflow!preSuperNodeList.action?taskId=${param.taskId}","办理环节");
		} else if(overRuleView == "1"){//流程图
			navigate("<%=root%>/fileNameRedirectAction.action?toPage=workflowDesign/action/processType-workflowPic.jsp?taskId=${param.taskId}&type=bohui","流程图");
		} else if(overRuleView == "2"){
			navigate("<%=root%>/senddoc/sendDocWorkflow!preSuperNodeList.action?taskId=${param.taskId}","办理环节");
			navigate("<%=root%>/fileNameRedirectAction.action?toPage=workflowDesign/action/processType-workflowPic.jsp?taskId=${param.taskId}&type=bohui","流程图");
		}
		setSelectedIndex(0);
