...
<title>提交下一步流程</title> <meta content="m.taobao.com" name="author" /> 
			...
			<div id="container">
				<div class="nav" style="width: 540px;">
					...
					<div class="ntopbot clearfix">
						...
						<a href="<%=root%>/wap/work.action?listMode=10&businessTitle=${businessTitle}&userName=${userName}&currentPage=${currentPage}&worktype=${worktype}"><img src="<%=root%>/oa/image/login/sytctop2.png" /></a><a href="<%=root%>/wap/login!getMainInfo.action"><img src="<%=root%>/oa/image/login/sytctop1.png" /></a>
						</s:if>
						<s:else>
						<a href="<%=root%>/wap/work!wapViewForm.action?taskId=${taskId}&businessTitle=${businessTitle}&userName=${userName}&currentPage=${currentPage}&worktype=${worktype}"><img src="<%=root%>/oa/image/login/sytctop2.png" /></a><a href="<%=root%>/wap/login!getMainInfo.action"><img src="<%=root%>/oa/image/login/sytctop1.png" /></a>
						</s:else>
   	 					</div>
					</div>
									...
									<table width="100%">
										<tr>
											<td>
												...
												<s:else>
													<form id="form3"
														action="<%=root%>/wap/work.action?worktype=${worktype}"
														method="post">
								...
								<td class="xqytdta">
									<table width="100%">
										...
										<tr>
											<td>
												<form id="form"
													action="<%=root%>/wap/work!getWapNextTransitionsChosePerson.action"
													method="post">
													<s:hidden id="instanceId" name="instanceId"></s:hidden>
													<s:hidden id="formId" name="formId"></s:hidden>
										...
										<tr>
											<td align="left">
												...
												<%--<a
													href="<%=root%>/wap/work!wapViewForm.action?taskId=${taskId}&businessTitle=${businessTitle}&userName=${userName}&currentPage=${currentPage}&worktype=${worktype}">返回</a>--%>
												</form>
											</td>
										</tr>
									</table>
