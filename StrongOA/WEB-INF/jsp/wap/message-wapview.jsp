...
<%
	...
	Date  dt =msg.getMsgDate();
	SimpleDateFormat sf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	String newtime=sf.format(dt);
%>
...
.ntopbot{ padding:4px 4px 0; color:#aeb9c9; }
	...
	<body>
		<div id="container">
			...
			<div class="ntopcenter">查看邮件</div>
					<div class="ntopbot clearfix">
						<div >
						<a href="<%=root%>/wap/message!waprecvlist.action?currentPage=${currentPage}"><img src="<%=root%>/oa/image/login/sytctop2.png" /></a>
					...
					<table width="100%">
						<tr>
							...
							<table width="100%">
								<tr align="right">
									<td>
										<form action="<%=root%>/wap/message!wapnew.action?forward=reply&msgId=${model.msgId}" method="post">
											<input type="submit" value="回复" class="button">
										</form>
									</td>
									<td>
										<form action="<%=root%>/wap/message!wapnew.action?forward=forward&msgId=${model.msgId}" method="post">
											<input type="submit" value="转发" class="button">
										</form>
									</td>
									<td>
										<form action="<%=root%>/wap/message!wapdelete.action?forward=notreal&msgId=${model.msgId}" method="post">
											<input type="submit" value="删除" class="button">
										</form>
									</td>
									
								<%--<a
									href="<%=root%>/wap/message!waprecvlist.action?currentPage=${currentPage}"
									class="button">返回</a>--%>
								</tr>
								<tr>
									<td >
								...
								<tr>
									<td colspan="3" id="ArticleContent">
										内&nbsp;&nbsp;容：</br>${model.msgContent}
									</td>
