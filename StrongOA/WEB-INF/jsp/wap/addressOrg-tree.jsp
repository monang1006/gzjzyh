...
<title>通讯录</title> <meta content="m.taobao.com" name="author" /> 
...
<% List<TempPo> lst=(List<TempPo>)request.getAttribute("orgList");
   String parentName="";
   String parentId="";
   String parentCodeId="";
   List<TempPo> newlst=new ArrayList<TempPo>();
   for(Iterator<TempPo> it=lst.iterator();it.hasNext();){
	   TempPo po=(TempPo)it.next();
	   if("0".equals(po.getParentId())||po.getParentId()==null||"null".equals(po.getParentId())){
		  parentId="";
		  parentName=po.getName();
		  parentCodeId=po.getCodeId();
		  it.remove();
	  }else{
	  }
   }
...
<div id="container">
  <div class="nav" style="width:540px;">
    <div class="ntopcenter">通讯录</div>
    <div class="ntopbot clearfix">
    	<div>
       <%if("".equals(parentId)){ %>
      		<a href="<%=root%>/wap/login!getMainInfo.action"><img src="<%=root%>/oa/image/login/sytctop2.png" /></a>
       <%}else{%>
       		<a href="<%=root%>/wap/addressOrg!getOrguserlist.action?orgId=<%=parentId%>"><img src="<%=root%>/oa/image/login/sytctop2.png" /></a>
       <% }%>
  ...
  <div class="con">
    <div class="tyym">
      <h3 class="h3zbt"><a href="<%=root%>/wap/addressOrg!getOrguserlist.action?orgId=<%=parentCodeId%>"><%=parentName %></a></h3>
      <table width="100%">
      	<tr>
          <th>下属机构</th>
        </tr>
        <%if(!"".equals(showFlag)){ %>
	        <c:if test="${orgList!=null}">
		        <c:forEach items="${orgList}" var="orgDataRow" varStatus="orgStatus" begin="0">
					<tr><td>
						<a href="<%=root%>/wap/addressOrg!getOrguserlist.action?orgId=<c:out value="${orgDataRow.codeId}"/>"><c:out value="${orgDataRow.name}" /></a>
					</td></tr>
				</c:forEach>
			</c:if>
		<% }else{%>
        ...
        <c:forEach items="${page.result}" var="dataRow" varStatus="status">
			...
			<div class=sec>
				...
				<security:authorize ifAnyGranted="001-0001000400020005">
					<a href="<%=root%>/wap/addressOrg!userDetail.action?userId=<c:out value="${dataRow.userId}"/>&orgId=${orgId}&userName=${userName}&currentPage=${currentPage}"
						title="查看">
				</security:authorize>
				...
				<security:authorize ifAnyGranted="001-0001000400020003">
					<a href="<%=root%>/wap/message!view.action?forward=write&moduleCode=<%=GlobalBaseData.SMSCODE_ADDRESS%>&msgReceiverIds=<c:out value="${dataRow.userId}"/>&orgId=${orgId}&userName=${userName}&currentPage=${currentPage}">发消息</a>
				</security:authorize>
			</div>
			</td></tr>
		</c:forEach>
