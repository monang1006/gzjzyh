...
<html>
	...
	<script type="text/javascript">
	    ...
	    if(document.getElementById("iscode").value != ""||document.getElementById("iscode").value!=null){
	       if(!numtest.test(document.getElementById("groupSyscode").value)){
		    alert('用户组编号必须为整数');
		    document.getElementById("groupSyscode").focus();
		        return false;
	       }
	       var flag=document.getElementById("iscode").value;
	        if(flag==document.getElementById("groupSyscode").value){
	           alert("合法编码。");
	           document.getElementById("groupSyscode").focus();
	       }
	       else{
	...
	var groupIsactive = inputDocument.getElementById("groupIsactive").value;
	if(oldIsactive != null && oldIsactive != "" && oldIsactive != groupIsactive){
		if(groupIsactive == "0"){
			if(!confirm("设定该用户组为未启用将导致其下级用户组都为未启用状态，是否继续操作?")){
				return false;
			}
		}else if(groupIsactive == "1"){
			if(confirm("是否同时将该用户组的下级用户组都设置为启用状态?")){
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" style="vertical-align: top;">
				<tr>
					<td height="100%">
						...
						<s:form action="/usergroup/baseGroup!save.action" method="post" id="22" theme="simple">
							<input type="hidden" id="childTogether" name="childTogether" value="0" />
							<input type="hidden" id="oldIsactive" name="oldIsactive" value="${model.groupIsactive }" />
							<input type="hidden" id="groupId" name="groupId" value="${model.groupId}">
							<input type="hidden" id="iscode" name="iscode" value="${model.groupSyscode}">
							<table border="0" cellpadding="0" cellspacing="0" class="table1" width="100%" >
								<tr>
									...
									<td class="td1" colspan="3" align="left">
										<input id="groupSyscode" name="model.groupSyscode"
											url="<%=path%>/usergroup/baseGroup!compareCode.action" type="text" maxLength="21" size="22" value="${model.groupSyscode}">
								...
								<tr>
									...
									<td class="td1" colspan="3" align="left">
										<input id="groupName" name="model.groupName" type="text"
											maxLength="50" size="22" value="${model.groupName}">
									</td>
								</tr>
								...
								<tr>
									...
									<td class="td1" colspan="3" align="left">
										<input id="groupSequence" name="groupSequence" type="text"
											size="22" maxlength="10" style="width:11em" value="${model.groupSequence}">
									</td>
								</tr>
								<tr>
									...
									<td class="td1" colspan="3" align="left">
										<textarea rows="6" cols="30" id="groupDescription"
											name="model.groupDescription">${model.groupDescription}</textarea>
									</td>
								</tr>
								<tr>
