...
<html>
	<head>
		...
		<script type="text/javascript" src="<%=root%>/uums/js/md5.js">
</script>
	...
	<body>
		...
		<div class="information_out" id="information_out">
			...
			<s:form id="formid" name="formid"
				...
				<div id="tabBody">
				<input type="hidden" id="conferenceId" name="model.conferenceId"
							value="${model.conferenceId}" />
					<!--<div id="_tab1">
						<input type="hidden" id="conferenceId" name="model.conferenceId"
							value="${model.conferenceId}" />
						<input type="hidden" id="depId" name="depId" value="">
						<input type="hidden" id="conferenceFromId" name="conferenceFromId" value="${model.conferenceFromId}" ">
						
						<input type="hidden" id="conEntryTime" name="conEntryTime" value="${model.conEntryTime}"/>
							...
							<tr>
								...
								<td class="dxktslx" width="450px">
									<input class="dxkinptext" id="conferenceStime"
										...
										value="<s:date name="model.conferenceStime" format="yyyy-MM-dd HH:mm" />"								 
										required="true"
							...
							<tr>
								...
								<td class="dxktslx">
									<input class="dxkinptext" id="conferenceRegendtime"
										...
										name="model.conferenceRegendtime" type="text"
							...
							<tr>
								...
								<td class="dxktslx">
									<input class="dxkinptext" id="conferenceleader"
										...
										style="width: 89%" required="true" value="${model.conferenceAttenleaders}"
							...
							<tr>
								...
								<td colspan="3">
									<textarea class="dxkttar" id="conferenceUndertaker"
										name="model.conferenceUndertaker" rows="2" style="width: 96%">${model.conferenceUndertaker}</textarea>
								</td>
							...
							<tr>
								...
								<td colspan="3">
									<textarea class="dxkttar" rows="5" style="width: 96%" readonly="true"
										...
										onClick="addDept();">${deptNames}</textarea>
									<input type="hidden" name="depIds" id="depIds" value="${depIds}" />
									<!-- 初始化已选人员使用 -->
							...
							<tr>
								...
								<td colspan="3" style="font-size: 12px; font-weight: bolder;">
									<div style="float: left; overflow: auto;">
										...
										<span id="<s:property value='attachId'/>">
										<input type="button" value="删除" onclick="del('<s:property value="attachId"/>')"/><img src="<%=path%>/oa/image/mymail/yes.gif">
		...
		.click(
				...
				var ids = $("#depIds").val();
					...
					if (confirm("确定发送吗?")) {
						$
								.post(
										"<%=root%>/provinconference/bigconfer/apply/bigConnotice!save.action",
										{
											"orgIds" : ids
										}, function(data) {
											   window.close();  
							                   window.dialogArguments.callback(1);  
...
function del(id){
	var time = new Date();
	$.get("<%=root%>/noticeconference/clientConference!delAttr.action?id="+id+"&time="+time.getTime(),function(ret){
		$("#"+id).css("display","none");
	});	
}
