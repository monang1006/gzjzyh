				...
				<tr>
					<td height="100%">
						...
						<s:form id="myTableForm" method="get"
							action="/noticeconference/noticeConference!handleConference.action">
...
function initMenuT(){
	...
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/ico/chakan.gif","查看报名情况","viewHandleReport",1,"ChangeWidthTable","checkOneDis");
	sMenu.addItem(item);
		...
		if(conferenceId.indexOf(",")!=-1){
			alert("一次只能查看一条会议通知！");
			 ...
			 var width=screen.availWidth-10;
		  	 ...
		  	 OpenWindow('<%=path%>/noticeconference/noticeConference!viewHandleReport.action?conNoticeId='+conferenceId+'&state=${state}', width, height, window);
		 }
	}
}
	...
	if(conferenceId==null||conferenceId==""){
		...
		if(conferenceId.indexOf(",")!=-1){
		    ...
		    //
			 ...
			 var url="<%=path%>/noticeconference/noticeConference!assignDeptListTree.action?conNoticeId="+conferenceId+"&state=${state}";	  
