...
<script for="window" event="onload">
	...
	navigate('<%=root%>/noticeconference/clientConference.action?state=0&conferId=<%=conferId %>','待签收会议通知');
	navigate('<%=root%>/noticeconference/clientConference.action?state=1&conferId=<%=conferId %>','已签收会议通知');
	setSelectedIndex(0);
	try{
		msg2.StartListen();
	}catch(e){
