...
.noprint{display : none }
  ...
  <tr>
    <th style="background-color:#e4e9ee; text-align: right; width: 250px ; height: 51px; line-height:1;" ><img src="<%=themePath%>/images/title1.jpg" /></th>
     <th width="50">1月</th>
    <th  width="50">2月</th>
    <th  width="50">3月</th>
    <th  width="50">4月</th>
  ...
  %>
	...
	$("#img_sousuo").click(function(){
		...
		location = "${root}/xxbs/action/statistics!findUse.action?orgType="+flag+"&year="+year+"&flag=3";
	});
	$("#findOrg").click(function(){
			location = "${root}/xxbs/action/statistics!findOrg.action";
		});
	
	$("#findPoint").click(function(){
	...
	function excel(){
		...
		location = "${root}/xxbs/action/statistics!excelSubmit.action?orgType="+orgType+"&flag=Use";
	}
	/**
	 * 根据上报信息统计
	 */
	...
	$(function(){
		...
		$("#year ").val(year);
		if($("#isOrg").val()==0){
		$("#top").html(year+"年市政府部门及相关单位信息上报情况");
		}
		if($("#isOrg").val()==1){
			$("#top").html(year+"年全市各设区市信息上报情况");
			}
		if($("#isOrg").val()==2){
			$("#top").html(year+"年1-12月全市县（市、区）信息上报情况");
			}
		if($("#isOrg").val()==3){
			$("#top").html(year+"年市各驻外办信息上报情况");
			}
		if($("#isOrg").val()==4){
			$("#top").html(year+"年市所有部门上报情况");
			}
