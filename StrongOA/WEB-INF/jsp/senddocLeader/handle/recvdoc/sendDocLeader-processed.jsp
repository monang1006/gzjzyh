	...
	<body class="contentbodymargin" onload="initMenuT();">
		...
		<div id="contentborder" align="center">
							...
							<webflex:flexTable name="myTable" width="100%" height="200px" wholeCss="table1"
								...
								<s:if test="state!=1">
									...
									<%
										if (column3 != null) {
									%>
									<webflex:flexTextCol caption="<%=column3[1]%>" property="<%=column3[3]%>"
								...
								<s:else>
									...
									<%
										if (column11 != null) {
									%>
									<webflex:flexTextCol caption="<%=column11[1]%>" property="<%=column11[3]%>"
			...
			<div style="display: none;">
				<s:select id="nodeName" name="nodeName" list="#{'':'全部','0':'办文','1':'送领导','2':'会文','3':'意见征询','4':'办结'}" listKey="key" listValue="value" style="height:10px;width:150px;" />
			</div>
		</div>
		<script language="javascript">
			...
			*/
			...
			//alert(state);
	      	var width=screen.availWidth-10;
	   		var height=screen.availHeight-30;
	      	var ret=WindowOpen("<%=root%>/senddocLeader/sendDocLeader!viewProcessed.action?instanceId="+instanceId+"&processSuspend=0&state="+state+"&searchType=1&taskId="+taskId+"&businessType="+businessType,'processed',width, height, "");
	     		if(ret){
	     			if(ret == "OK"){
	     				reloadPage();
	     			}
