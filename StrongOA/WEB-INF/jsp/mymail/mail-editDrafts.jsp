	...
	function saveDraft(){
		...
		if($("#ischarge").val()=="false"){
			if(confirm("请问您是否要将其保存至草稿箱呢")==true){
	            ...
	            ischanged=false;
				document.mailForm.submit();
	        	event.returnValue="您已经提交保存草稿操作！是否要离开本页面？";
			}else{
...
<body class="contentbodymargin" onbeforeunload="saveDraft()">
	...
	<input type="hidden" id="msgId" name="msgId" value="${myMail.mailId }"> 
		...
		<table  width="100%" border="0" cellpadding="0" cellspacing="1"  class="table1">
			...
			<tr>
				...
				<td colspan="3" class="td1" width="75%">
					<input type="text" name="mailaddress" id="mailaddress" title="多个收件人之间请用英文逗号分隔" style="width:100% " value="${myMail.mailReceiver }" />
				</td>
			</tr>
			...
			<tr>
				<td class="biao_bg1" width="15%"><span class="wz">附件操作：</span></td>
				<td id="attfile" colspan="3" class="td1" width="75%">
					...
					<s:if test="#request.attList!=null">
						<s:iterator value="#request.attList" status="statu" id="att">
							<div id=div<s:property value="#att.attachId"/> >[<a href="#" onclick="javascript:deldbobj('<s:property value="#att.attachId"/>')">删除</a>]<s:property value="#att.attachName" /></div><input type="hidden" name="dbobj" id="<s:property value="#att.attachId"/>" value=<s:property value="#att.attachId"/>  />
						</s:iterator>
					</s:if>
				</td>
			</tr>
			...
			<tr id=fsxxtr>
				<td class="biao_bg1" width="15%"></td>
				<td colspan="3" class="td1" width="75%">
				<s:if test="myMail.mailIsReply=='1'">
					<input type="checkbox" name="ydhz" id="ydhz" checked="checked" value="1" onchange="ischanged = true;" >已读回执&nbsp;
				</s:if>
				<s:else>
					<input type="checkbox" name="ydhz" id="ydhz" value="1" onchange="ischanged = true;" >已读回执&nbsp;
				</s:else>
				</td>
			</tr>
			<tr>
