        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="00">
          <tr>
            ...
            <table border="0" align="right" cellpadding="00" cellspacing="0">
                <tr>
                  <td width="*">&nbsp;</td>
                  <td ><a class="Operation" href="#" onclick="editdraft()"><img src="<%=root%>/images/ico/bianji.gif" width="15" height="15" class="img_s">编辑</a></td>
                  <td width="5"></td>
                  ...
                  <td width="5"></td>
                  <td ><span class="Operation" id="test" style="cursor:hand"><img src="<%=root%>/images/ico/zujuan.gif" width="15" height="15" class="img_s">转移</span></td>
	     ...
	     <webflex:flexTable name="myTable" width="100%" height="100%" wholeCss="table1" property="0" isCanDrag="true" isCanFixUpCol="true" clickColor="#A9B2CA" footShow="showCheck" getValueType="getValueByArray" onclick="quicklyVeiwMail(this.value)" collection="${page.result}" page="${page}">
			...
			<webflex:flexTextCol caption="主题" valuepos="0" valueshowpos="1" width="30%" isCanDrag="true" isCanSort="true" onclick="editdraft(this.value)"></webflex:flexTextCol>
...
function initMenuT(){
	...
	var item = null;
	item = new MenuItem("<%=root%>/images/ico/bianji.gif","编辑","editdraft",1,"ChangeWidthTable","checkOneDis");
	sMenu.addItem(item);
...
function editdraft(id){
	...
	if(redId==null||redId==""){
		...
		if(redId.indexOf(",")!=-1){
			...
			msg=OpenWindow('<%=root%>/mymail/mail!editDrafts.action?boxId=${boxId}&sendid='+redId,'700',430,window);
			if(msg=="true"){
			  	$.ajax({
			  		type:"post",
			  		dataType:"text",
			  		url:"<%=root%>/mymail/mail!delete.action",
			  		data:"sendid="+redId+"&type=real",
			  		success:function(msg){
			  			if(msg=="true"){
			  				window.location.reload();
						}else{
