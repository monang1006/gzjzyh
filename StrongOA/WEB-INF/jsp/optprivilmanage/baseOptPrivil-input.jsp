	...
	function testcode(){
	   ...
	   function comparecode(obj,flag){	
		...
		if(oldIsactive != null && oldIsactive != "" && oldIsactive != privilIsactive){
			if(privilIsactive == "0"){
				if(!confirm("设定该权限为未启用将导致其下级权限都变为未启用状态，是否继续操作?")){
					return false;
				}
			}else if(privilIsactive == "1"){
				if(confirm("是否同时将该权限的下级权限都设置为启用状态?")){
				...
				<tr>
					<td height="100%">
						...
						<s:form action="/optprivilmanage/baseOptPrivil!save.action"
							method="post" id="22" theme="simple">
							<input type="hidden" id="childTogether" name="childTogether" value="0" />
							<input type="hidden" id="oldIsactive" name="oldIsactive" value="${model.privilIsactive }" />
							<input type="hidden" id="privilId" name="model.privilId"
								value="${model.privilId}">
								 <input type="hidden" id="iscode" name="iscode"  value="${model.privilSyscode}">
							<table width="100%" height="10%" border="0" cellpadding="0"
								...
								<tr>
									...
									<td class="td1" colspan="3" align="left">
										<input id="privilSyscode" name="model.privilSyscode" maxLength="21"
											type="text" size="22" url="<%=path%>/optprivilmanage/baseOptPrivil!compareCode.action" style="width:11em"
											value="${model.privilSyscode}"> 
											<input type="button" class="input_bg" value="检测合法性" onclick="testMessage();">
										&nbsp;
