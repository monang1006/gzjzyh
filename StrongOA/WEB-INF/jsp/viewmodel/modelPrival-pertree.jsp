					...
					<td height="10px">
						<div class="dtree">
						  ...
						  function setNodeItem(checkFlag, id, name) {
						      if (checkFlag) {
						    	parent.addOption("person", name, id,geshu);						        
							      } else {
						        parent.removeOption("person", name, id);
						      }
						  }
...
//							      }
						...
						function gStart(){
							var g=$("input[name='rightCheckBox']:checked");
							for(var i=0;i<=g.length-1;i++){
								if(g[i].value.substring(0, 1) == 'u'){
									geshu=geshu+1;
								}
							}
						 ...
						 var geshu=0;
						  ...
						  var d = new dTree('d');
						  d.add('g${topTreeNodeId}','-1','机构人员','','','','<%=root%>/workflow/images/tree/folder_closed.gif','2');
						  ...
						  <c:forEach items="${orgList}" var="tree" varStatus="status">
						      <c:forEach items="${tree[3]}" var="user" varStatus="status">
						       ...
						       //	d.add('<c:out value="${user[0]}"/>','<c:out value="${tree[0]}"/>','<c:out value="${user[1]}"/>','','','','<%=root%>/workflow/images/tree/renyuan.gif','-2');
						      </c:forEach>
						  </c:forEach>
						  document.write(d);
						  ...
						  parent.frameInitChecked('modelPrival-pertree');
						</script>
						</div>
					</td>
				</tr>
