...
<script language="JavaScript">
	...
	mTreeData.addTreeNode('<c:out value="${orgList.orgSyscode}" />',code,
	                    new Array("<c:out value='${orgList.orgName}' />"),
	                     'select()', true,
	                     "", "folder_closed.gif", lStyleArr, "<c:out value='${orgList.orgid}' />,<c:out value='${orgList.orgName}' />",'0');       
	...
	<script>
		...
		function select(){
			...
			window.dialogArguments.document.getElementById("orgName").value=id[1];
			window.dialogArguments.document.getElementById("baseOrg").value=id[0];
			window.close();
		}
