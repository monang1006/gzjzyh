...
$(document).ready(function(){
	simpleTreeCollection = $('.simpleTree').simpleTree({
		...
		afterClick:function(node){
			...
			parent.project_work_content.document.location="<%=root%>/personnel/trainingmanage/training!column.action?clumnId="+folderId;
		},
			...
			'delete': function(t) {
				...
				var id = node.get(0).id;
			 	if(confirm("确定要删除栏目“"+folderName+"”吗?")){
			 		$.post(
			 		"<%=path%>/personnel/trainingmanage/training!deleteCloumn.action",
			 		{clumnId:id},
			 		function(data){
			 			if(data == "delfalse"){	 								 			
			 				alert("该栏目下存在培训项目或子栏目，请先删除培训项目和子栏目！");
			 			}else{
...
<div class="contextMenu" oncontextmenu="return false;" id="myMenu1" style="width:100%">
	<ul>
	    <li id="newcolumn"><img src="<%=path %>/oa/image/mymail/folder_add.png" /> 添加顶级栏目</li>
		<li id="add"><img src="<%=path %>/oa/image/mymail/folder_add.png" /> 添加子栏目</li>
		<li id="edit"><img src="<%=path %>/oa/image/mymail/folder_edit.png" /> 修改栏目</li>
