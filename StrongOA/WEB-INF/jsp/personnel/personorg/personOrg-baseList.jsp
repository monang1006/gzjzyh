...
<HTML><HEAD><TITLE>统一用户组织机构树</TITLE>
      ...
      <tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="00"  style="vertical-align: top;">
          <tr>
            ...
            <td width="35%">统一用户组织机构树</td>
            <td width="5%">&nbsp;</td>
            <td width="60%">
           <s:form id="getpostform" action="/personnel/personorg/personOrg!saveUumsOrgs.action">
</s:form>
...
<c:forEach items="${uumsList}" var="uumsList">

<script language="JavaScript">
myObject = new Object();
	var code = findFatherCode('<c:out value="${uumsList.orgSyscode}" />','<c:out value="${codeType}" />');
	mTreeData.addTreeNode('<c:out value="${uumsList.orgSyscode}" />',code,
	                    new Array("<c:out value='${uumsList.orgName}' />","<c:out value='${uumsList.orgSyscode}' />"),
	                     '',
	                      true,
	                     "", "folder_closed.gif", lStyleArr, '<c:out value="${uumsList.orgId}" />','2');                              	                          
	</script>
	...
	<script>
		...
		if(confirm("再次导入时将替换原有的统一用户机构与人员,确定导入吗?")) 
		{ 
		     document.getElementById("getpostform").submit();
		// window.dialogArguments.parent.organiseTree.location="<%=path%>/personnel/personorg/personOrg!tree.action";
		// window.close();
