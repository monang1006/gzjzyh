...
<script>
	...
	function initMenuT(){
		...
		sMenu.addItem(item);
		item = new MenuItem("<%=root%>/images/ico/bianji.gif","还原","restoreOrg",1,"JTreeNode","checkSelected");
		sMenu.addItem(item);
		item = new MenuItem("<%=root%>/images/ico/goujian.gif","移动","tomove",1,"JTreeNode","checkSelected");
		sMenu.addItem(item);		
		sMenu.addLine();
		item = new MenuItem("<%=root%>/images/ico/goujian.gif","合并","merge",1,"JTreeNode","checkSelected");
		sMenu.addItem(item);
	//	item = new MenuItem("<%=root%>/images/ico/yidong.gif","导入","moveDown",1,"JTreeNode","checkSelected");
	//	sMenu.addItem(item);
	///	item = new MenuItem("<%=root%>/images/ico/yidong.gif","导出","",1,"JTreeNode","checkSelected");
	//	sMenu.addItem(item);
	    item = new MenuItem("<%=root%>/images/ico/yidong.gif","编制情况","showstructure",1,"JTreeNode","checkSelected");
		sMenu.addItem(item);
		item = new MenuItem("<%=root%>/images/ico/yidong.gif","人员信息","showperson",1,"JTreeNode","checkSelected");
		sMenu.addItem(item);
...
<script language="JavaScript">
	...
	mTreeData.addTreeNode('<c:out value="${orgList.orgSyscode}" />',code,
	                    new Array("<c:out value='${orgList.orgName}' />"),
	                     'select()', true,
	                     "", "folder_closed.gif", lStyleArr, "<c:out value='${orgList.orgid}' />,<c:out value='${orgList.orgName}' />,<c:out value='${orgList.orgIsdel}' />,<c:out value='${orgList.rest}' />",'2');       
	...
	<script>
		...
		function select(){
			...
			parent.organiseInfo.location="<%=root%>/personnel/personorg/personOrg!input.action?orgId="+id[0]+"&audittype=get";
		}
		...
		function restoreOrg(){
			...
			{ 
				location ="<%=root%>/personnel/personorg/personOrg!restore.action?orgId="+id[0]+"&audittype=1";
			} 
		}
		          ...
		          function(data){
		           ...
		           for(var i=0;i<ids.length;i++){
		              for(var j=0;j<ids1.length;j++){
		                 if(ids1[j]==ids[i]){
		                 alert("请不要选择"+id[1]+"的父机构！");
		                   return;
		                 }
		              }
		           }
		         if(confirm("确定要合并吗？")){
		          window.showModalDialog("<%=root%>/personnel/personorg/personOrg!input.action?moveOrgId="+audit+"&orgId="+id[0]+"&audittype=merge"
	...
	function showstructure(){
		...
		var sNode = mTree.GetTreeNode(mTree.mSelected );
			var id = sNode.others.split(',');
		 top.perspective_content.actions_container.personal_properties_toolbar.navigate("<%=root%>/personnel/baseperson/person.action?orgId="+id[0],"人员信息");
		}
	</script>
