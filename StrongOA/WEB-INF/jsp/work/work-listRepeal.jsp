...
<html>
  <head>
    ...
    $(document).ready(function(){
     ...
     //重定向到此页面
      function reloadPage() {
      	window.location = "<%=root%>/work/work!listRepeal.action?state="+$("#state").val()+"&workflowName="+encodeURI(encodeURI($("#workflowName").val()))+"&formId="+$("#formId").val()+"&workflowType=${workflowType}&excludeWorkflowType=${excludeWorkflowType}&handleKind=${handleKind}";
      }	      
        //返回到此页面的父页面
        function goBack(){
...
<script type="text/javascript" src="<%=path%>/common/script/My97DatePicker/WdatePicker.js"></script>
    <div id="contentborder" align="center">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="vertical-align: top;">
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
                <td class="table_headtd_img" >
						...
						<td align="left">
						<strong>
						    <s:if test='%{state=="0"}'>我的在办文件</s:if>
						    <s:elseif test='%{state=="1"}'>我的已办文件</s:elseif>
						    <s:elseif test='%{state=="6"}'>我的取消文件</s:elseif>
						    <s:else>公文回收站</s:else>
						    </strong>
						</td>
						<td align="right">
		                  ...
		                  <tr>
			                ...
			                <td width="4"><img src="<%=frameroot%>/images/bt_l.jpg"/></td>
		                 	...
		                 	<td width="4"><img src="<%=frameroot%>/images/bt_r.jpg"/></td>
		                 	<td width="5"></td>
		                 	<s:if test="(state!=1 && state!=0 ) || state==''">
		                 	<td width="4"><img src="<%=frameroot%>/images/bt_l.jpg"/></td>
        ...
        <tr>
          <td height="100%">
          <s:form name="myTableForm" action="/work/work!listRepeal.action?noTotal=1">
...
function tjst(){
	window.location.href="<%=path%>/senddoc/sendDoc!repealWorkflow.action?state=${state}&workflowType=${workflowType}&excludeWorkflowType=${excludeWorkflowType}&handleKind=${handleKind}";
}
     //还原公文,重新启动流程实例
      function returnProcess(){
        ...
        var taskId = getValue();
         ...
         //通过任务实例id 获取流程实例id 再还原
					 $.ajax( {
							type : "post",
							dataType : "text",
							url : "<%=root%>/senddoc/sendDoc!getInstanceIdBytask.action",
							data : "taskId=" + taskId,
							success : function(msg) {
								if (msg!=null && msg!="") {//返回的流程实例id
								 	if(confirm("确定要还原吗？")){
