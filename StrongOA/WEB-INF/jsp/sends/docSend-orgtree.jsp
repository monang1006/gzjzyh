...
<HTML>
	<HEAD>
		...
		$(document).ready(function(){
			...
			$("#btnOK").click(function(){
				...
				/*$.post(scriptroot + "/sends/docSend!viewSendOrg.action",
    			  {"docId":docId,"orgIds":orgIds,"defPrint":defPrint},
    			  function(ret){
    				if(ret == "0"){
    					alert("公文分发成功！");
    					returnValue = "reload";
				...
				var url = "<%=path%>/sends/docSend!viewSendOrg.action?docId=${docId}&orgIds=" + orgIds + "&defPrint=" + defPrint;
				
				//var a=window.showModalDialog(url,'window','dialogWidth:550 pt;dialogHeight:400 pt;status:no;help:no;scroll:auto;');				
				var a = OpenWindow(url,'550', '400', window);
				if(a=="reload"){
					returnValue = "reload";
					window.close();
					//window.location="<%=root%>/sends/docSend!orgTree.action?docId=${docId}";
				}
			});
	...
	<body class=contentbodymargin oncontextmenu="return false;"  topmargin="0" leftmargin="0">
			...
			<table width="100%" height="391" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
				...
				<tr>
					<td align="center">
						<b>默认打印份数</b>：<input type="text" id="defPrint" name="defPrint" value="5" size="3">
					</td>
				</tr>
				<tr>
					<td align="center"><br>
