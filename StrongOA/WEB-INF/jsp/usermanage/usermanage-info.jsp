...
//menu.addItem("close","x","<font class=w2kfont>关 闭</font>","ROOT","closeWorkByIdx(false,SEL_IDX);");
...
function addTab(sTitle,sIcon,sUrl){
	...
	oCel.innerHTML ="<table width='100%' height='22' border='0' cellpadding='00' cellspacing='0'><tr><td class='bt1_bg1'>&nbsp;</td><td align='center' class='bt1_bg2' noWrap valign='center'><IMG src='"+sIcon+"' id='img_switch'/><LABEL>"+titleSegment+"</LABEL>&nbsp;</td><td width='27' class='bt1_bg3'>&nbsp;</td></tr></table>";
...
function setUserId(id){
		userId = id;
		refreshWorkByTitle('<%=root%>/usermanage/usermanage!roleInfo.action?userId='+userId,'角色信息');
		refreshWorkByTitle('<%=root%>/usermanage/usermanage!postInfo.action?userId='+userId,'岗位信息');
		refreshWorkByTitle('<%=root%>/usermanage/usermanage!groupInfo.action?userId='+userId,'用户组信息');
		//location.reload();
}
</SCRIPT>
<script for="window" event="onload">
	state = true;//初始化完毕
	navigate('<%=path%>/fileNameRedirectAction.action?toPage=/usermanage/usermanage-noinfo.jsp','角色信息');
	navigate('<%=path%>/fileNameRedirectAction.action?toPage=/usermanage/usermanage-noinfo.jsp','岗位信息');
	navigate('<%=path%>/fileNameRedirectAction.action?toPage=/usermanage/usermanage-noinfo.jsp','用户组信息');
	setSelectedIndex(0);
	...
	<body class=toolbarbodymargin>
			...
			<TR>
				<TD style="padding-left: 0">
					<DIV id="tabContainer"
						...
						<TABLE style="height: 22;" border=0 cellSpacing=2 cellPadding=0>
							<TR id=tabList>
							</TR>
						</TABLE>
					</DIV>
		...
		<TABLE style="padding-left: 0; padding-right: 0; padding-top: 0; padding-bottom: 0; " border=0
			width="100%" height="80%" cellpadding="0" cellspacing="0">
			<TBODY id="workPanel">
			</TBODY>
		</TABLE>
