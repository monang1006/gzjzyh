...
<jsp:directive.page import="com.strongit.oa.bo.ToaAgencyPrsnfldrFolder"/>
...
$(document).ready(function(){
	simpleTreeCollection = $('.simpleTree').simpleTree({
		...
		afterClick:function(node){
			...
			parent.project_work_content.document.location="<%=root%>/prsnfldr/privateprsnfldr/prsnfldrFile!agencyFileList.action?folderId="+folderId;
		},
		...
		$($('span:first',node)).contextMenu('myMenu1', {
		  ...
		  bindings: {
			...
			'add': function(t) {
				var folderId = node.get(0).id;
			    var ret=OpenWindow("<%=root%>/prsnfldr/agencyprsnfldr/agencyPrsnfldrFolder!init.action?folderId="+folderId,"400","100",window);
			...
			'edit': function(t) {
			   ...
			   var folderId = node.get(0).id;
			    var ret=OpenWindow("<%=root%>/prsnfldr/agencyprsnfldr/agencyPrsnfldrFolder!initEdit.action?folderId="+folderId,"400","105",window);
				if(ret!=undefined && ret!="None"){
	...
	//添加文件夹
	function addFolder(){
		var ret=OpenWindow("<%=root%>/prsnfldr/agencyprsnfldr/agencyPrsnfldrFolder!init.action","400","100",window);
		if(ret!=undefined && ret!="None"){
...
<div class="contextMenu" id="myMenu1" style="width:100%"  >
		<ul>
			<security:authorize ifAnyGranted="001-0001000700050003">
					<li id="add">
						<img src="<%=path%>/oa/image/mymail/folder_add.png" />
						&nbsp;<SPAN class="wz">新建</SPAN> 
					</li>
			</security:authorize>
			<security:authorize ifAnyGranted="001-0001000700050002">
					<li id="delete">
						<img src="<%=path%>/oa/image/mymail/folder_delete.png" />
						<SPAN class="wz">删除</SPAN>
					</li>
			</security:authorize>
			<security:authorize ifAnyGranted="001-0001000700050001">
					<li id="edit">
						<img src="<%=path%>/oa/image/mymail/folder_edit.png" />
						<SPAN class="wz">重命名</SPAN>
					</li>
			</security:authorize>
		</ul>
		</div>
...
<ul class="simpleTree">
	<li class="root" id='1'><span style="cursor: hand;" id="rootSpan">机构文件柜</span>
		<ul>
			<%
				List<ToaAgencyPrsnfldrFolder> list = (List<ToaAgencyPrsnfldrFolder>) request
						.getAttribute("folderList");
				String[] child = (String[]) request.getAttribute("hasChild");
				for (int i = 0; i < list.size(); i++) {
					ToaAgencyPrsnfldrFolder folder = list.get(i);
					...
					if ("1".equals(charge)) {//存在子节点
						...
						out
								.println("<li id="
										...
										+ ">{url:"
										+ root
										+ "/prsnfldr/agencyprsnfldr/agencyPrsnfldrFolder!syntree.action?folderId="
										+ folder.getFolderId() + "}</li>");
						out.println("</ul>");
						out.println("</li>");
					} else {
						out.println("<li id=" + folder.getFolderId() + ">");
