...
<jsp:directive.page import="com.strongit.oa.bo.ToaDepartmentPrsnfldrFolder"/>
...
.simpleTree .line-over
{
	...
	height:3px;
	font-size:3px;
	background: url(<%=path %>/oa/image/mymail/line_bg_over.gif) 0 0 no-repeat 3eeeff3;
}
...
$(document).ready(function(){
	simpleTreeCollection = $('.simpleTree').simpleTree({
		...
		afterClick:function(node){
			...
			parent.project_work_content.document.location="<%=root%>/prsnfldr/privateprsnfldr/prsnfldrFile!departmentFileList.action?folderId="+folderId;
		},
		...
		$($('span:first',node)).contextMenu('myMenu1', {
		  ...
		  bindings: {
			...
			'add': function(t) {
				var folderId = node.get(0).id;
			    var ret=OpenWindow("<%=root%>/prsnfldr/departmentprsnfldr/departmentPrsnfldrFolder!init.action?folderId="+folderId,"400","150",window);
			...
			'edit': function(t) {
			   ...
			   var folderId = node.get(0).id;
			    var ret=OpenWindow("<%=root%>/prsnfldr/departmentprsnfldr/departmentPrsnfldrFolder!initEdit.action?folderId="+folderId,"400","150",window);
				if(ret!=undefined && ret!="None"){
...
<ul class="simpleTree">
	<li class="root" id='1'><span style="cursor: hand;" id="rootSpan">部门文件柜</span>
		<ul>
			<%
				List<ToaDepartmentPrsnfldrFolder> list=(List<ToaDepartmentPrsnfldrFolder>)request.getAttribute("folderList");
				String[] child=(String[])request.getAttribute("hasChild");
				for(int i=0;i<list.size();i++){
					ToaDepartmentPrsnfldrFolder folder=list.get(i);
					...
					if("1".equals(charge)){//存在子节点
						...
						out.println("<li id="+id +">{url:"+root+"/prsnfldr/departmentprsnfldr/departmentPrsnfldrFolder!syntree.action?folderId="+folder.getFolderId() +"}</li>");	
						out.println("</ul>");	
						out.println("</li>");	
					}else{
						out.println("<li id="+folder.getFolderId()+">");		
