...
<html>
	...
	<script type="text/javascript"
		...
		<input type="hidden" id="carId" name="model.carId"
								value="${model.carId}">
		<input type="hidden" id="editDelImg" name="editDelImg"
								value="${editDelImg}">						
			<table width="100%"
			...
			<table align="center" width="90%" border="0" cellpadding="0"
				...
				<tr>
					...
					<td rowspan="1" colspan="3" align="left" class="td1">
					   <div id="fileinput" style="margin-left:5px;margin-top:5px">
						<INPUT id="uploads" class="upFileBtn"  onkeydown="return false;" name=uploads TYPE="file" onchange=setTimeout('showimg()',500) style="display: block;">
						</div>	
						<div id="delpic" style="display:none">
						  ...
						  <div id="showimage" style="margin-left:1px;margin-right:5px;margin-top:5px;margin-bottom:5px">
						<IMG id="imageSrc" name=imageSrc SRC="<%=path %>/car/car!viewImg.action?carId=${model.carId}" width="444" height="179"  BORDER="0"> 	
							</div>	
							<script>
...
function showimg(){
                   $("#editDelImg").val("0");
				var img=document.carform.uploads.value;
				if(img!=null&img!=""){
				   $("#delpic").css("display","");
					if(validimg(img)){
...
function save(){
    ...
    inputDocument.getElementById("carno").value = carNo;
     if(inputDocument.getElementById("carno").value.length==null || inputDocument.getElementById("carno").value.length==""){
    	alert("车牌号不能为空！");
