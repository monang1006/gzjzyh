...
<div class="contextMenu" oncontextmenu="return false;" id="myMenu2" style="width:100%">
	<ul>
		...
		<li id="semdmsg"><img src="<%=path %>/oa/image/address/sms.gif" width="15" height="15" /> 发消息</li>
		<li id="sendphone"><img src="<%=path %>/oa/image/address/mobile_sms.gif" width="15" height="15" /> 发短信</li>
		<li id="sendIM"><img src="<%=path %>/oa/image/address/rtx.bmp" width="15" height="15"/> 发即时消息</li>
	</ul>
</div>
	<ul class="simpleTree">
		<li class="root" id='1'><label style="cursor: hand;" id="rootSpan">系统通讯录</label>
			<ul id="groupTree">
				<%
				List<Organization> list=(List<Organization>)request.getAttribute("orgList");
				...
				for(int i=0;i<list.size();i++){
					...
					if("1".equals(charge)){//存在子节点
						out.println("<li id="+org.getOrgId() +">");	
						out.println("<span>"+org.getOrgName()+"</span>");	
						out.println("<label style='display:none;'>org</label>");	
						out.println("<ul class=ajax>");
						out.println("<li id="+org.getOrgId()+1 +">{url:"+root+"/address/addressOrg!synajaxtree.action?orgId="+org.getOrgId() +"}</li>");	
						out.println("</ul>");	
						out.println("</li>");	
					}else{
						out.println("<li id="+org.getOrgId()+" leafChange='folder-org-leaf'>");	//  picChange='folder-org'	
						out.println("<span>"+org.getOrgName()+"</span>");
						out.println("<label style='display:none;'>org</label>");	
						out.println("</li>");
					}
