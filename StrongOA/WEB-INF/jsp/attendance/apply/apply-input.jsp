...
<html>
	...
	function goBack(){
		alert("发送成功！");
   		window.location="<%=root%>/attendance/apply/apply.action";
	}

	//送审
	function submitNext() {
		var eform = document.getElementById("FormInputOCX");
		eform.SetFieldValue('T_OA_ATTEN_APPLY','APPLY_STATE',"1");
		doNext();
    }
    //保存表单成功以后的回调函数
	...
	//保存表单数据,返回生成的表单ID,@param:isReturn:保存后是否需要返回到草稿页。true：返回；false：停留在当前页
    function saveFormData(isReturn){
		...
		if(ret == 1){
			$.post(contextPath + "!eFormDateSave.action",{formData:formData},
				   function(data){
						if(data == "-1"){
   ...
   function getTypeName(id){
	...
	<!-- 业务数据名称 -->
		...
		<DIV id=contentborder align=center>
			...
			<form id="recvdocForm" action="<%=root%>/attendance/apply/apply!eFormDateSave.action"
				method="post" enctype="multipart/form-data">
				<input type="hidden" name="recvid" id="recvid">
				<input type="hidden" name="isbutian" id="isbutian" value="${isbutian}">
									...
									<td width="70%">
										<table border="0" align="right" cellpadding="00"
											...
											<tr>
												...
												<td width="5"></td>
												<td >
													<a class="Operation" href="javascript:saveFormData(true);">
