...
<html>
    ...
    <script type="text/javascript">
		...
		function initialDefaultInfo(workflowName){
			...
			//默认的提醒内容
	        $("#handlerMes").val("申请单转交提醒：\n"+$("#businessName").val()+"("+strNow+")");
		}
    </script>
	</head>
	<base target="_self">
	<body class="contentbodymargin" scroll="auto">
	<s:form id="form" name="form" action="/attendance/apply/applyaudit!handleWorkflow.action" method="post">
	<!-- 电子表单模板id -->
    ...
    <s:hidden name="dictId"></s:hidden>
		<div id="contentborder">
					...
					<div style="padding: 15px 15px 15px 15px; text-align: left;">
						...
						<div>
							<s:iterator value="workflows" status="status">
								<input type="radio" id="<s:property value="workflows[#status.index][1]" />" name="workflowName" value="<s:property value="workflows[#status.index][0]"/>">
		 ...
		 <!-- 提醒方式标签 -->
		 <strong:remind  isDisplayInfo="false" code="<%=GlobalBaseData.SMSCODE_WORKFLOW %>"/>
