				...
				<tr>
					<td height="10px">
						<div class="dtree">
						  ...
						  d.add('p0','g0','曾经处理人','','','','<%=root%>/workflow/images/tree/renyuan.gif','-2');
						  //<c:forEach items="${orgList}" var="tree" varStatus="status">
						  //<c:choose>
						  //  <c:when test="${tree[1] == '0'}">
						  //    d.add('g<c:out value="${tree[0]}"/>','-1','<c:out value="${tree[2]}"/>','','','','<%=root%>/workflow/images/tree/folder_closed.gif','-2');
						  //    d.add('p0','g<c:out value="${tree[0]}"/>','曾经处理人','','','','<%=root%>/workflow/images/tree/renyuan.gif','-2');
						  //  </c:when>
						  //  <c:otherwise>
						  //  </c:otherwise>
						  //</c:choose>						  
