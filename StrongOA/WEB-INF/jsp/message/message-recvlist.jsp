															...
															<table border="0" align="right" cellpadding="00"
																...
																<td width="4"><img src="<%=frameroot%>/images/bt_l.jpg"/></td>
											                 	...
											                 	<td width="4"><img src="<%=frameroot%>/images/bt_r.jpg"/></td>
										                  		<td width="5"></td>
										                  		<td width="4"><img src="<%=frameroot%>/images/bt_l.jpg"/></td>
											                 	<td class="Operation_list" onclick="retell();"><img src="<%=root%>/images/operationbtn/Reply.png"/>&nbsp;回&nbsp;复&nbsp;</td>
											                 	<td width="4"><img src="<%=frameroot%>/images/bt_r.jpg"/></td>
										                  		<td width="5"></td>
										                  		<td width="4"><img src="<%=frameroot%>/images/bt_l.jpg"/></td>
...
function initMenuT() {
	...
	var item = null;
	item = new MenuItem("<%=root%>/images/operationbtn/add.png", "新建", "gotoWrite", 1,
	...
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/Reply.png", "回复",
			"retell", 1, "ChangeWidthTable", "checkOneDis");
	sMenu.addItem(item);
	item = new MenuItem(
			"<%=root%>/images/operationbtn/del.png", "删除",
			"del", 1, "ChangeWidthTable", "checkOneDis");
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/completely_del.png", "彻底删除",
			"realDel", 1, "ChangeWidthTable", "checkOneDis");
	sMenu.addItem(item);
...
function retell(msgId) {
	...
	if (relId == null || relId == "") {
		...
		if (relId.indexOf(",") != -1) {
			...
			var checkCol = $("td[innerValue ='" + relId + "']").parents();
			if (checkCol.children("td[value='系统消息<id:system>']").length == 0) {
				OpenWindow(
						'<%=root%>/message/message!reply.action?msgId=' + relId,
						'700', '446', window);
			} else {
				alert("不能回复系统消息。");
			}
...
function quicklyVeiwMail(object) {
	...
	//	$
	//			.post("<%=root%>/message/message!getModelById.action",
					//	{
					//		msgId : msgId
					//	},
					//	function(data) {
					...
					//	if (data != "") {
					//		var ret = data.split(",");
					//		var isReturnBack = ret[1];
					//		var isSendBack = ret[0];
					//		if ((isSendBack != null && isSendBack == "1")
					//				&& (isReturnBack == null
					//						|| isReturnBack == "" || isReturnBack == "0")) {
					//			var rtn = OpenWindow(
					//					'fileNameRedirectAction.action?toPage=message/message-receipt.jsp?msgId=' + msgId,
					//					'250', '150', window);
					//		}
					//	}
				//	});
