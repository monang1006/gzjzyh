...
.myhidden {
		...
		<script type="text/javascript">
			...
			function OpenMessage(Url, Width, Height, WindowObj) {
				...
				var ParamValue = window.open(Url,"_blank","width=" + Width + ",height=" + Height +
		                                ",top=100,left=300, status=no,help=no,scrollbars=no,resizable=yes");
	...
	<BODY class=contentbodymargin oncontextmenu="return false;"
															...
															<table border="0" align="right" cellpadding="00"
																...
																<tr>
												                 	...
												                 	<td width="4"><img src="<%=frameroot%>/images/bt_r.jpg"/></td>
											                  		<td width="5"></td>
											                  		<td width="4"><img src="<%=frameroot%>/images/bt_l.jpg"/></td>
												                 	<td class="Operation_list" onclick="replymsg();"><img src="<%=root%>/images/operationbtn/tb-list16.png"/>&nbsp;短&nbsp;信&nbsp;回&nbsp;复&nbsp;统&nbsp;计&nbsp;</td>
												                 	<td width="4"><img src="<%=frameroot%>/images/bt_r.jpg"/></td>
											                  		<td width="5"></td>
											                  		<td width="2%"></td>
																</tr>
						...
						<s:form id="myTableForm" action="/message/message.action"
							...
							<webflex:flexTable name="myTable" width="100%" height="100%"
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table1_search">
								 <tr>
								 	<td>
										...
										&nbsp;&nbsp;阅读状态：&nbsp; <s:select name="readstate" list="#{'0':'未读','1':'已读'}" listKey="key" listValue="value" headerKey="" headerValue="全部" onchange='$("#img_sousuo").click();' />
								...
								<webflex:flexCheckBoxCol caption="选择" valuepos="0"
									...
									onclick="toViewMail(this.value)" showsize="34"></webflex:flexTextCol>
		...
		<div id="d1x" class="Operation" style="width: 60px;">
			<div
				style="border-top: 1px solid #fff; border-left: 1px solid #fff; border-right: 1px solid #B4C5E0; border-bottom: 1px solid #B4C5E0; padding: 5px;">
...
function initMenuT(){
	...
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/ico/tb-list16.gif","短信回复统计","replymsg",1,"ChangeWidthTable","checkOneDis");
	sMenu.addItem(item);
...
function toViewMail(msgId){
	...
	if(msg=="reply"){
		...
		OpenWindow('<%=root%>/message/message!input.action?moduleCode=<%=GlobalBaseData.SMSCODE_MESSAGE%>&msgId='+msgId, '700', '500', window);
	}
}
...
function sendagain(msgId){
	...
	var boo=OpenWindow('<%=root%>/message/message!input.action?moduleCode=<%=GlobalBaseData.SMSCODE_MESSAGE%>&msgId='+sendId, '700', '500', window);
	if(boo=="true"){
		window.location.reload();
	}else if(boo=="false"){
	}
...
function replymsg(){
	...
	if(msgId==null||msgId==""){
		...
		if(msgId.indexOf(",")!=-1){
			...
			//OpenWindow('<%=root%>/reply_message/messageReply!view.action?messageId='+msgId+'&moduleId=<%=GlobalBaseData.message_model%>', '700', '500', window);			
			getSysConsole().navigate('<%=root%>/reply_message/messageReply!view.action?messageId='+msgId+'&moduleId=<%=GlobalBaseData.message_model%>',"短信回复统计");
		}
	}
}
