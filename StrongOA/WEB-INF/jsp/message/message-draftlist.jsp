...
<div style="exportDiv">
		...
		<DIV id=contentborder onmousewheel="$('#d1x').css('display','none');"
						...
						<s:form id="myTableForm" action="/message/message.action"
							...
							<webflex:flexTable name="myTable" width="100%" height="100%"
								...
								<webflex:flexCheckBoxCol caption="选择" valuepos="0"
									valueshowpos="1" width="40px" isCheckAll="true" isCanDrag="false"
									isCanSort="false"></webflex:flexCheckBoxCol>
										<webflex:flexTextCol caption="标题" valuepos="0" valueshowpos="1"
									width="53%" isCanDrag="true" isCanSort="true"
									onclick="editMsg(this.value)" showsize="35"></webflex:flexTextCol>
			...
			--%>
								<webflex:flexTextCol caption="收件人" valuepos="8" valueshowpos="8"
...
function initMenuT() {
	...
	var item = null;
	item = new MenuItem("<%=root%>/images/operationbtn/add.png","新建","gotoWrite",
			1,"ChangeWidthTable","checkOneDis");
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/edit.png", "编辑", "gotoEdit",
			1, "ChangeWidthTable", "checkOneDis");
	sMenu.addItem(item);
	...
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/del.png", "删除",
...
function editMsg(msgId){
	if(msgId==null||msgId==""){
		...
		if(msgId.indexOf(",")!=-1){
			...
			var boo=OpenWindow('<%=root%>/message/message!input.action?moduleCode=<%=GlobalBaseData.SMSCODE_MESSAGE%>&folderId=${folderId }&msgId='+msgId, '700', '446', window);	
			if(boo=="true"){
				window.location.reload();
			}else if(boo=="false"){
				window.location.reload();
