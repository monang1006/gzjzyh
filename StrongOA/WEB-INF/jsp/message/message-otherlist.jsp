			...
			$(document).ready(function(){
			  ...
			  $("#test").click(function(){
				...
				$("#img_sousuo").click(function(){
					$("#searchTitle").val(encodeURI($("#textfield").val()));
...
<BODY class=contentbodymargin oncontextmenu="return false;" onload="initMenuT();onunLoadClick();" >
...
<DIV id=contentborder onmousewheel="$('#d1x').css('display','none');" onscroll="$('#d1x').css('display','none');" align=center>
  <table width="100%" border="0" cellspacing="0" cellpadding="00">
  <tr>
    <td colspan="3" class="table_headtd">
    <table width="100%" border="0" cellspacing="0" cellpadding="00">
      <tr>
        <td height="55" style="FILTER:progid:DXImageTransform.Microsoft.Gradient(gradientType=0,startColorStr=#ededed,endColorStr=#ffffff);">
        <table width="100%" border="0" cellspacing="0" cellpadding="00">
          <tr>
				...
				<td>
					<table border="0" align="right" cellpadding="00"
						...
						<td width="4"><img src="<%=frameroot%>/images/bt_l.jpg"/></td>
	                 	<td class="Operation_list" onclick="gotoWrite();"><img src="<%=root%>/images/operationbtn/add.png"/>&nbsp;新&nbsp;建&nbsp;</td>
  ...
  <tr>
    <td height="100%">
    <s:form id="myTableForm" action="/message/message.action" method="get">
    	 <input type="hidden" name="folderId" id="folderId" value="${folderId }">
    	 <input type="hidden" name="type" id="type" value="${returnType }">
    	 <input type="hidden" name="searchTitle" id="searchTitle" value="${searchTitle}">
    	 <input type="hidden" name="beginDate" id="beginDate" value="${firstDate }">
    	 <input type="hidden" name="endDate" id="endDate" value="${otherDate }">
	     <webflex:flexTable name="myTable" width="100%" height="100%" wholeCss="table1" property="0" isCanDrag="true" isCanFixUpCol="true" clickColor="#A9B2CA" footShow="showCheck" getValueType="getValueByArray" onclick="quicklyVeiwMail(this)" collection="${page.result}" page="${page}">
			...
			<webflex:flexTextCol caption="标题" valuepos="0" valueshowpos="1" width="53%" isCanDrag="true" isCanSort="true" onclick="toViewMail(this.value)" showsize="33"></webflex:flexTextCol>
			<webflex:flexDateCol caption="日期" valuepos="3" valueshowpos="3" width="22%" isCanDrag="true" isCanSort="true" dateFormat="yyyy-MM-dd HH:mm" showsize="16"></webflex:flexDateCol>
			<webflex:flexTextCol caption="状态" valuepos="2" valueshowpos="javascript:viewState(2)" width="6%" isCanDrag="true" isCanSort="true"></webflex:flexTextCol>
			<webflex:flexTextCol caption="附件" valuepos="3" valueshowpos="javascript:isAtt(7)" width="6%" isCanDrag="true" isCanSort="true"></webflex:flexTextCol>
			...
			--%>
			<webflex:flexTextCol caption="发件人"  valuepos="5" valueshowpos="5" width="13%" isCanDrag="true" isCanSort="true"></webflex:flexTextCol>
...
<div style="border-top: 1px solid #fff; border-left: 1px solid #fff; border-right: 1px solid #B4C5E0; border-bottom: 1px solid #B4C5E0; padding: 5px;">
    <table width="100%" border="0" cellspacing="0" class="tbaCss" cellpadding="0">
      <s:iterator value="#request.folderList" status="statu" id="msgFolder">
      	<tr>
      		<td onclick="changeFolder('<s:property value="#msgFolder.msgFolderId"/>')" onmouseout="this.className='myshow'" onmouseover="this.className='myhidden'"><s:property value="#msgFolder.msgFolderName"/></td>
      	</tr>
      </s:iterator>
    </table>
</div>
...
function initMenuT(){
	...
	var item = null;
	item = new MenuItem("<%=root%>/images/operationbtn/add.png","新建","gotoWrite",1,"ChangeWidthTable","checkOneDis");
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/reading.png","阅读","readmail",1,"ChangeWidthTable","checkMoreDis");
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/Forward.png","转发","resend",1,"ChangeWidthTable","checkOneDis");
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/del.png","删除","del",1,"ChangeWidthTable","checkOneDis");
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/completely_del.png","彻底删除","realDel",1,"ChangeWidthTable","checkOneDis");
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/Export_to_eml.png","导出为eml","exportMail(1)",1,"ChangeWidthTable","checkMoreDis");
	sMenu.addItem(item);
	item = new MenuItem("<%=root%>/images/operationbtn/Afolderto_export.png","导出为文件夹","exportMail(2)",1,"ChangeWidthTable","checkMoreDis");
	sMenu.addItem(item);
...
function resend(msgId){
	...
	if(reId==null||reId==""){
		...
		if(reId.indexOf(",")!=-1){
			...
			boo=OpenWindow('<%=root%>/message/message!tran.action?msgId='+reId, '700', '446', window);
			if(boo=="true"){
				window.location.reload();
			}
		}
...
function toViewMail(msgId){
	msg=OpenMessage('<%=root%>/message/message!view.action?forward=view&msgId='+msgId, '700', '400', window);
	parent.parent.msg_manager_tree.freashUnread("${folderId }");
	if(msg=="reply"){
		OpenWindow('<%=root%>/message/message!reply.action?msgId='+msgId, '700', '500', window);
	}else if(msg=="del"){
		infoFlag=true;
		window.location.reload();
	}else if(msg=="resend"){
		OpenWindow('<%=root%>/message/message!tran.action?msgId='+msgId, '700', '500', window);
	}
}
...
function quicklyVeiwMail(object){
	...
	if(msgId!=null&&msgId!=""&&msgId!="null"&&!infoFlag){
		...
		$('img:eq(0)',object).attr("src","<%=path%>/oa/image/mymail/read.gif");
		parent.parent.msg_manager_tree.freashUnread("${folderId }");
	}else{
		window.parent.msg_main_content.personal_status_content.location="<%=root %>/fileNameRedirectAction.action?toPage=message/info.jsp";
		infoFlag=false;
...
<%--	var boo=OpenWindow('<%=root%>/fileNameRedirectAction.action?toPage=message/message-write.jsp', '700', '500', window);				--%>
	var boo=OpenWindow('<%=root%>/message/message!view.action?forward=write&moduleCode=<%=GlobalBaseData.SMSCODE_MESSAGE %>', '700', '446', window);	
	if(boo=="true"){
		window.location.reload();
	}else if(boo=="false"){
	}
