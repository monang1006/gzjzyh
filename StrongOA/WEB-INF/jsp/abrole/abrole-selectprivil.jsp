...
var imageRootPath='<%=path%>/common/frame';
	...
	var hasSelectCode = "";
	if("${abRolePrivil}" != "" && "${abRolePrivil}" != null){
		hasSelectCode = "," + "${abRolePrivil}" + ",";
	}
</script>
		<DIV id=treecontentborder>
			<table border="0" cellpadding="0" cellspacing="0" width="100%"
				height="100%" style="vertical-align: top;">
				<form action="<%=path%>/rolemanage/baseRole!savePrivil.action"
					method="post">
...
<c:forEach items="${systemLst}" var="systemLst">
	<script language="JavaScript">
	mTreeData.addTreeNode('<c:out value="${systemLst.sysId}" />','0',
	                    new Array("<c:out value='${systemLst.sysName}' />",'系统'),
	                     '', true,
...
<%--<c:forEach items="${privilList}" var="privilList">
	<script language="JavaScript">
		myObject = new Object();
		var code = findFatherCode('<c:out value="${privilList.privilSyscode}" />','<c:out value="${codeType}" />','<c:out value="${privilList.basePrivilType.baseSystem.sysSyscode}" />');
...
/**			if(id == ""||id==null){
		...
		function lazyLoadSubTree(aNode, aExpandOrClose, isFilter){
			...
			var initFlag = true;
			var url = scriptroot + "/abrole/abrole!lazyLoadPrivil.action";
			var privilCode = "";
			var systemId = "";
			var srcUserid = "${srcUserid}";
			var abroleId = "${abroleId}";
			if(aNode.others == "system"){
				systemId = aNode.id;
			}else{
				privilCode = aNode.id;
			}
			$.getJSON(url,{privilCode: privilCode,systemId: systemId,srcUserid: srcUserid,abroleId: abroleId}, function(jsons){
				$.each(jsons,function(i,json){
						...
						aNode.subtree.addTreeNode(json.code,aNode.id,
		                    new Array(json.name,json.code),
		                     "", true,
		                     "", "renyuan.gif", lStyleArr, json.id, '0');
				});
