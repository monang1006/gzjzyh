<html>
	...
	%>
		...
		//是否可以选择用户
		function isShow_change(issel){
			//var isShow_sel=document.getElementById("divshowuser");
			var ifShowUser=document.getElementById("ifShowUser");
			
			if(issel=="yes"){
				//isShow_sel.style.display="block";
				ifShowUser.value=1;
				$("#divshowuser").show();
					
			}else{
				//isShow_sel.style.display="none";
				ifShowUser.value=0;
				$("#divshowuser").hide();
		...
		function save(){
			var strTmp="";	
			var userlist = document.getElementById("ShowUserList").value;
			$("#isShowsel input:checked ").each(function(i){
		   		strTmp=strTmp+this.value+",";
		 	});
			...
			if(strTmp!=""){
				...
				if(calid!=""){
					//var ifShowUser=document.getElementById("ifShowUser");
					//ifShowUser.value=0;
					//alert(ifShowUser.value);
				}else{
					alert("请选择用户。")
					return;
				}	
			}
			$("#ShowUserList").val(strTmp);
			
			//标题验证
			var list=$("#assignNameList");
...
<%--			if(document.getElementById("isShow_yes").checked){--%>
<%--			}else{--%>
<%--				alert("请选择用户。");--%>
<%--				return;--%>
<%--			}--%>
...
<%--            }--%>
	...
	$(document).ready(function() {
		...
		$("#isShowsel input:checkbox").each(function(i){
			if($("#checkname").val()==this.value){
				$("#isShowsel input").eq(i).attr("checked",true);
		   	}
		 });
		...
		//获取说明内容
         ...
         function setRemind(){
			...
			var a = window.showModalDialog(url,window,'dialogWidth:550px;dialogHeight:260px;help:no;status:no;scroll:no');
...
<%--								+"），提醒共享人："+remShare+" <input type='button' value='删除' class='input_bg' onclick='delDBremind(\""+arr[0]+"\");'><hr></div>";--%>
	...
	<s:form action="/calendar/calendar!saveother.action" name="form" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" id="ifShowUser" name="ifShowUser" value="${ifShowUser}">
	<input type="hidden" id="ShowUserList" name="ShowUserList" value="${ShowUserList}">
		...
		<table  border="0" cellpadding="0" cellspacing="0" class="table1" width="100%" >
				...
				<tr style="display:none">
						...
						<td class="td1" style="padding-left:5px;">
							...
							<span>
							<input type="button" class="button" value="添加" onclick="newActivity.style.display='';" title="单击添加新的活动分类">
							</span>
							...
							<input type="button" class="button"  value="确认" onclick="addNewActivity();" title="确定添加">
							<input type="button" class="button" value="取消" onclick="newActivity.style.display='none';" title="取消">
							</span>
							</div>
						</td>
						</tr>
				<tr id="onceTaskTime" style="display:" >
						...
						<td class="td1" title="请输入活动的开始时间和结束时间" style="padding-left:5px;" onmouseover="viewShow();" onmouseout="viewShow();">
				...
				<tr id="selremind" style="display:none"  title="在这里输入每次循环的开始时间和结束时间"> 
							...
							<td class="td1" style="padding-left:5px;">&nbsp;
								<select id="TYPE"  name="TYPE" class="BigSelect" onchange="sel_change()"
									onmouseover="viewShow();" onmouseout="viewShow();">
									<option value="2">
										按日重复
									</option>
									<option value="3">
				...
				<tr id="setRemind" style="display:none" >
							...
							<td class="td1" id="remindContent" style="padding-left:5px;">
								&nbsp;&nbsp;&nbsp;<a id="remBTN" onclick="setRemind();" title="点击添加提醒"  href="#" class="button">添加提醒</a>
							</td>
				</tr>
				<!--   共享的功能屏蔽掉
				<tr>
				...
				<tr id="isShare_sel" style="display:none" >
							<td  class="biao_bg1" valign="top" align="right" valign="bottom" >
							...
							<td  class="td1"  style="padding-left:5px;" > 
								...
								<a id="addPerson" url="<%=root%>/address/addressOrg!tree.action" href="#" class="button">添加</a>
								</div>
								<div>
				...
				-->
				<tr id="onlyread" style="display:none" >
					<td class="biao_bg1" align="right">
						<span class="wz">日程用户：</span>
					</td>
					<td class="td1" style="padding-left:5px;">
						&nbsp; <font color="blue">${model.calUserName}</font>
						&nbsp;
					</td>
				</tr>
				<tr id="selusers">
					...
					<td class="td1" style="padding-left:5px; padding-top: 4px;">
<%--							<input type="radio" name="isShow" id="isShow_yes" value = "yes" checked onclick="isShow_change(this.value)">是&nbsp;&nbsp;--%>
						<table id="isShowsel">
						  <s:if test="assignNameList!=null&&assignNameList.size()>0">	 
							<s:iterator value="assignNameList" id="list" status="status">
								<tr id="<s:property value='#status.index' />">
									<td width="3%" id="chkButtonTd" class="td1" style="text-indent: 0px;" align="center">
										&nbsp;&nbsp;<input id="chkSelUser_<s:property value="#list[1]"/>" type="checkbox" name="chkButton" value="<s:property value="#list[1]"/>" >
									</td>
									<td class="td1"    value="<s:property value="#list[1]"/>"><s:property value="#list[0]"/>
									</td>
								</tr>
							</s:iterator>
						 </s:if>
						 <s:else>
						 	<tr><td><font color="blue">用户列表为空</font></td></tr>
						 </s:else>
						</table>
					</td>
				</tr>
