...
<html>
	...
	<LINK href="<%=root %>/frame/theme-web/css/properties_windows.css" type=text/css rel=stylesheet>
	
	<link rel="stylesheet" href="<%=frameroot %>/webFtabforRB/calendar/calendar.css" type="text/css">
	<link rel="stylesheet" href="<%=root %>/frame/theme-web/css/calendar/window.css" type="text/css">
...
<body id ="calbody" Style="background-color: white;">
...
function closeAddwindow(){ 
   ...
   function selectInSmall(date){
   	 ...
   	 window.parent.parent.refreshWorkByTitle(scriptroot+'/calendar/calendar.action?setDate='+setDate, '个人日程');
   ...
   function setSchedules(startD,endD){
   	...
   	function nextMonth(startDate,endDate,type){
   		//alert("月初 = "+startDate+"\n 月末 = "+endDate+"\n type = "+type);
   		location = "<%=path%>/calendar/calendar!desktop.action?inputType=red&termStart="+startDate+"&termEnd="+endDate;
   	}
	...
	//	parent.mouseoverToShow(date,element,event);
   	...
   	parent.document.getElementById("cont").document.onmousewheel = displaynone;
