...
String orgSysCode=request.getParameter("orgSysCode");
%>
<HTML>
	<HEAD>
		<title>无标题文档</title>
...
function setOrgId(id){
		...
		refreshWorkByTitle('<%=root%>/organisemanage/orgmanage!input.action?orgId='+orgId,'机构基本信息');
		refreshWorkByTitle('<%=root%>/usermanage/usermanage.action?extOrgId='+orgId+'&module=orgType','用户信息');
		//location.reload();
}
</SCRIPT>
...
<script for="window" event="onload">
	...
	var orgId="<%=orgId%>";
	var operate="<%=operate%>";
	if(orgId!=null&&orgId!=""&&orgId!="null"&&operate!="add"){
		navigate('<%=path%>/organisemanage/orgmanage!input.action?orgId='+orgId,'机构基本信息');
	}else{
		var orgSysCode="<%=orgSysCode%>";
		if(orgSysCode=="null"){
			orgSysCode="";
		}
		navigate('<%=path%>/organisemanage/orgmanage!addchild.action?orgId='+orgId+'&audittype=<%=audittype%>&orgSysCode='+orgSysCode,'机构基本信息');
	}
	navigate('<%=path%>/fileNameRedirectAction.action?toPage=/organisemanage/orgmanage-noinfo.jsp','用户信息');
	setSelectedIndex(0);
	try{
		msg2.StartListen();
	}catch(e){
	...
	<body class=toolbarbodymargin>
		...
		<TABLE style="padding-left: 0; padding-right: 0; padding-top: 0; padding-bottom: 0; " border=0
			width="100%" height="90%" cellpadding="0" cellspacing="0">
			<TBODY id="workPanel">
			</TBODY>
		</TABLE>
