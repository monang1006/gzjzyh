...
<HTML>
	<HEAD>
		...
		<TITLE>系统工作区</TITLE>
		<link rel="stylesheet"
			href="<%=path%>/frame/theme-web/css/tab/properties_toolbar.css" type="text/css">
		<link rel="stylesheet" href="<%=path%>/frame/theme-web/css/tab/style.css" type="text/css">
...
.tab_sel {
	...
	background: url("<%=path%>/frame/theme-web/images/tab/images/perspective_leftside/qh1_2.jpg");
	BORDER-TOP: gray 1px solid;
	...
	display: none
		...
		$(document).ready(function() {
			...
			if(width > 1024){
				...
				$("#tabconw").css("width","1252px");
				$("#zph").attr("id","kph"); 
			}else{
			    $("#toolbar").css("width","984px");
			    $("#tabconw").css("width","984px");
				$("#kph").attr("id","zph"); 
			}
...
/**
  *luosy
 * 在TAB_LIST中根据索引获取对应tab页对象
 * @param:flag 要获取的值类型，TITLE：获取标题，URL：获取内容链接，ID：获取Tab页编号
 * @param:idx tab页在TAB_LIST中的索引位置
 */
function getTabObj(tabID,idx){
	var iframeId = "frame_"+tabID;
	var frame = document.frames[iframeId].privil_content;
	if(frame=="undefined"||frame==undefined){
		return getTabParam("URL",idx);
	}else{
		var contentUrl = document.frames[iframeId].privil_content.location;
		return contentUrl;
	}
}
...
function flagDir(){
	if(SELECTED_INDEX == -1){//没有当前选中tab页，即没有打开的tab页时，设置向左向右滚动按钮为不可点击的图标状态
		toRight.src = "<%=path%>/frame/theme-web/images/tab/arrow2_2.gif";//向右滚动按钮
		toLeft.src = "<%=path%>/frame/theme-web/images/tab/arrow1_2.gif";//向左滚动按钮
		return;
	}
	
	var oCel = tabList.cells(SELECTED_INDEX);//获取当前选中tab页标题对应的单元格

	toLeft.src = "<%=path%>/frame/theme-web/images/tab/arrow1.gif";//设置向左滚动按钮为可点击的图标状态
	toRight.src ="<%=path%>/frame/theme-web/images/tab/arrow2.gif";//设置向右滚动按钮为可点击的图标状态
	//
	...
	if((oCel1.offsetLeft +oCel1.offsetWidth) <= (tabContainer.scrollLeft+tabContainer.offsetWidth)){
		toRight.src = "<%=path%>/frame/theme-web/images/tab/arrow2_2.gif";
	}
	//如果第一个tab页标题对应的单元格的左位移大于等于tab容器的左位移，即没有向左溢出的情况，则向左滚动按钮为不可点击的图标状态
	if(oCel2.offsetLeft>= tabContainer.scrollLeft){
		toLeft.src = "<%=path%>/frame/theme-web/images/tab/arrow1_2.gif";
	}
}
/**
 * 滚动的方向标鼠标事件处理
...
function scroll_doMouseOver(obj){
	...
	//将图标设置为选中状态
	obj.src = "<%=path%>/frame/theme-web/images/tab/arrow"+flag+"_1.gif";
}
/**
 * 滚动的方向标鼠标事件处理
 * @param obj 对象
...
function scroll_doMouseOut(obj){
	...
	//将图标设置为非选中可点击状态
	obj.src = "<%=path%>/frame/theme-web/images/tab/arrow"+flag+".gif";
}
/**
 * 指定的索引Tab滚动到当前位置
 *
...
function addTab(sTitle,sIcon,sUrl){
	...
	//设置单元格内容
	<%-- 
	oCel.innerHTML ="<table width='100%' height='22' border='0' cellpadding='00' cellspacing='0'>"+
						"<tr><td class='bt1_bg1'>&nbsp;</td>"+
						"<td align='center' class='bt1_bg2' noWrap valign='center'>"+
						"<IMG src='"+sIcon+"' id='img_switch'/>"+
						"<LABEL onmouseover=\"this.style.color='red'\" onmouseout=\"this.style.color='black'\">"+titleSegment+"</LABEL>"+
						"</td>"+
						"<td width='14' class='bt1_bg2'>"+
						"<IMG src='<%=path%>/frame/theme-web/images/tab/close_03.jpg' id='img_close' align='bottom' onclick='closeWork()'/>"+
						"</td>"+
						"<td width='27' class='bt1_bg3'>&nbsp;</td>"+
						"</tr></table>";
	--%>
	
	//start 默认第一个tab“办公门户”不能删除modified by luosy  
	if(oCel.cellIndex==0){
		oCel.innerHTML ="<table width='100%' height='22' border='0' cellpadding='00' cellspacing='0'>"+
							"<tr><td class='bt1_bg1'>&nbsp;</td>"+
							"<td align='center' class='bt1_bg2' noWrap valign='center'>"+
							"<IMG src='"+sIcon+"' id='img_switch'/>"+
							"<LABEL onmouseover=\"this.style.color='#8d0000'\" onmouseout=\"this.style.color='#000000'\">"+titleSegment+"</LABEL>"+
							"</td>"+
							"<td width='14' class='bt1_bg2'>"+
							//"<IMG src='<%=path%>/frame/theme-web/images/tab/close_03.jpg' id='img_close' align='bottom' onclick='closeWork()'/>"+
							"</td>"+
	...
	//end 默认第一个tab“办公门户”不能删除modified by luosy  
	...
	addPanels(sTitle,sIcon,sUrl,aParams["ID"]);//添加Tab页内容
 ...
 * @param sUrl 链接路径
 */
function addPanels(sTitle,sIcon,sUrl,tabId){
	...
	oCel.innerHTML = "<IFRAME id=\"frame_"+tabId+"\" class=\"difhw\" frameBorder=\"0\" scrolling=\"no\" onreadystatechange=\"stateListener(this)\" width=\"100%\" height=\"100%\" src=\""+sUrl+"\" >对不起，您的浏览器不支持IFRAME!</IFRAME>";
	//oRow1.valign = "bottom";	
...
function setSelectedIndex(idx){
	...
	toLeft.src = "<%=path%>/frame/theme-web/images/tab/arrow1.gif";//向左滚动图片
	toRight.src = "<%=path%>/frame/theme-web/images/tab/arrow2.gif";//向右滚动图片
	if(idx == -1){
		return ;
	}
	//自动滚动Tab页
	...
	if(oCel.cellIndex==0){
		...
		oCel.childNodes[0].rows[0].cells[2].innerHTML = "<IMG src='<%=path%>/frame/theme-web/images/tab/close_03.jpg' id='img_close' align='bottom' onclick='closeWork()'/>";
	}
	//end 默认第一个tab“办公门户”不能删除modified by luosy  
...
{
	var src = "<%=path%>/frame/theme-web/images/tab/info.gif";
	switch(flag)
	{
		case 0:
			src = "<%=path%>/frame/theme-web/images/tab/info.gif";
			break;
		case 1:
			src = "<%=path%>/frame/theme-web/images/tab/warning.gif";
			break;
		case 2:
			src = "<%=path%>/frame/theme-web/images/tab/error.gif";
			break;
		case 3:
			src = "<%=path%>/frame/theme-web/images/tab/error.gif";
			break;
...
function navigate(sUrl,sTitle,bl){
	if(bl == null || !bl){
		...
		if(getTabCount() < MAX_TAB_COUNT){//如果已打开的TAB页总数小于设置的最大TAB页总数，则可添加新的TAB页
			...
			addTab(sTitle,"<%=path%>/frame/theme-web/images/tab/state1.gif",sUrl);//添加TAB页
...
function closeWork(bl,oWin){
	...
	//start 默认第一个tab“办公门户”不能删除modified by luosy  添加判断
	if(callerIdx!=0){
...
function notifyChange(bl,oWin){
	var sImg = bl? "<%=path%>/frame/theme-web/images/tab/state2.gif" : "<%=path%>/frame/theme-web/images/tab/state1.gif";
	//只能更新当前页面的
	//alert("change");
...
function refreshCurrentWork(oWin){
	...
	if(document.frames.length > 0&&callerIdx>=0&&callerIdx<document.frames.length){
		...
		if(window.frames(callerIdx).privil_content){//判断是否是门户模式，门户模式则刷新内容区 modify by luosy
			window.frames(callerIdx).privil_content.location.reload();
		}else{
...
function setActive(bl,str)
{
	if(bl == "true")
	{
		MESSENGER.src = "<%=path%>/frame/theme-web/images/tab/ring-2.gif";
	...
	{
		MESSENGER.src = "<%=path%>/frame/theme-web/images/tab/ring-1.gif";
...
function resizeWork(obj)
{
	if(top.FRM_WHOLE.rows == sizeWholeOrg){
		...
		obj.src = "<%=path%>/frame/theme-web/images/tab/show-all.gif";
		//obj.innerHTML = "2";
		obj.title = "还原";
	}else{
		top.FRM_WHOLE.rows = sizeWholeOrg;
		top.FRM_MIDDLE.cols = sizeMiddleOld;//
		obj.src = "<%=path%>/frame/theme-web/images/tab/show-all-1.gif";
		obj.title = "工作区全屏";
	}
	setWorkFocus();
}
...
function resizeImage(obj,flag){
	if(top.FRM_WHOLE.rows == sizeWholeOrg){
		...
		if(flag == 0){
			obj.src = "<%=path%>/frame/theme-web/images/tab/show-up-1.gif";
		}else{
			obj.src = "<%=path%>/frame/theme-web/images/tab/show-up.gif";
		}
	}
}
/** 左边权限显示/隐藏的开关控制
...
<script for="window" event="onload">
	//initDoc();
	state = true;//初始化完毕
	navigate('<%=path%>/desktop/desktopWhole!gotoWebDesktop.action?webType=webFtabforRB','办公门户');
</script>


	</head>

	<body style="BACKGROUND:#8d0000">
		<table border=0 cellSpacing=0 cellPadding=0 width=100% height="100%"
					...
					<DIV id=toolbarborder>
						<DIV id=toolbar>
							<TABLE onselectstart="return false" border=0
								style="position: absolute; top: 0; width: 100%; height: 26; z-index: 0"
								cellSpacing=0 cellPadding=1>
								<TR>
									<TD style="padding-left: 0">
										<DIV id="tabContainer"
											style="position: absolute; top: 2px; width: 100%; height: 23; font-size: 15; overflow-y: no; overflow-x: hidden;">
											<TABLE style=" height: 23;" border=0 cellSpacing=0
												cellPadding=0>
									...
									<TD style="width: 105; cursor: hand;" nowrap valign="bottom">
										<img src="<%=path%>/frame/theme-web/images/tab/arrow1.gif" id="toLeft"
											onMouseOver="scroll_doMouseOver(this)"
											onMouseOut="scroll_doMouseOut(this)" title="向左滚动"
											onClick="scrollTab('PageLeft')" />
										<img src="<%=path%>/frame/theme-web/images/tab/arrow2.gif" id="toRight"
											onMouseOver="scroll_doMouseOver(this)"
											onMouseOut="scroll_doMouseOut(this)" title="向右滚动"
											onClick="scrollTab('PageRight')" />
										<img src="<%=path%>/frame/theme-web/images/tab/Back.jpg" title="当前工作区后退"
											onClick="history.back();"
											onMouseOver="this.src='<%=path%>/frame/theme-web/images/tab/Back_o.jpg'"
											onMouseOut="this.src='<%=path%>/frame/theme-web/images/tab/Back.jpg'" />
										<img src="<%=path%>/frame/theme-web/images/tab/refresh_o_03.gif"
											title="刷新当前工作区" onClick="refreshCurrentWork()"
											onMouseOver="this.src='<%=path%>/frame/theme-web/images/tab/refresh_z_03.gif'"
											onMouseOut="this.src='<%=path%>/frame/theme-web/images/tab/refresh_o_03.gif'" />
										<img src="<%=path%>/frame/theme-web/images/tab/close.gif" title="关闭当前工作区"
											onClick="closeWork()"
											onMouseOver="this.src='<%=path%>/frame/theme-web/images/tab/close1.gif'"
											onMouseOut="this.src='<%=path%>/frame/theme-web/images/tab/close.gif'" />
									</TD>
								</TR>
							</TABLE>
						</DIV>
			...
			<!-- 横向TAB页 截止 -->
			<tr>
				<td height="100%" align="center" style="background-color:#f4f4f4;">
					<TABLE
						style="padding-left: 0; padding-right: 0; padding-top: 0; padding-bottom: 0;"
						border=0 height="100%" cellpadding="0"
						cellspacing="0" id="tabconw">
	...
	function showSysAddress(){
		...
		<DIV id=LOADING onClick="showLoading(false)"
			...
			<br>
			<IMG src="<%=path%>/frame/theme-web/images/tab/loading.gif">
		</DIV>
