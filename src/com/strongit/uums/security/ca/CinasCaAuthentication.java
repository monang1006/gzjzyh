 ...
 * @date 2011-2-15 下午01:31:26
 * @version 1.0
 * @classpath com.strongit.uums.security.ca.CinasCaAuthentication
 */
package com.strongit.uums.security.ca;
...
public class CinasCaAuthentication extends AbstractCaAuthentication {
	Logger log = Logger.getLogger(CinasCaAuthentication.class);

	private Properties props = null;
	private Map attrMap = null;
	
	@Override
	public CaAuthenticationInfo authentication(CaAuthenticationInfo info) throws Exception {
		/***************************************************************************
		...
		// 初始化配置文件属性
		InputStream in = PropertiesUtil.class.getClassLoader().getResourceAsStream("caConfig.properties");
		...
		String original_data = null, signed_data = null, original_jsp = null;
		/**************************
		 * 获取认证数据信息 *
		 **************************/
		if (isSuccess) {
			log.info("应用标识及网关的认证地址读取成功！\n应用标识：" + appId + "\n认证地址："
					+ authURL + "\n");
			//if (isNotNull((String) this.getProperties(
			//		KEY_ORIGINAL_DATA))
			//		&& isNotNull(this.getProperties(KEY_SIGNED_DATA))
			//		&& isNotNull(this.getProperties(KEY_ORIGINAL_JSP))) {
			if (isNotNull(info.getCaKey())
							&& isNotNull(info.getValidInfo())){
					//		&& isNotNull(this.getProperties(KEY_ORIGINAL_JSP))) {			
				// 获取session中的认证原文
				//original_data = this.getProperties(
				//		KEY_ORIGINAL_DATA);
				// 获取request中的认证原文
				original_jsp = info.getValidInfo();

				/**************************
				 * 第五步：服务端验证认证原文 (此处暂时省略)*
				 **************************/
			...
			if (isSuccess) {
				...
				if (statusCode == HttpStatus.SC_OK
					...
					try {
						...
						isSuccess = false;
						errCode = String.valueOf(statusCode);
						errDesc = "读取认证响应报文出现异常\n" + e.getMessage();
						log.error("读取认证响应报文出现异常");
					}
				}
			}
			...
			if (isSuccess) {
				...
				if (attrsElement != null) {
					...
					for (int i = 0; i < attributeNodeList.size(); i++) {
						...
						/**
						 * 由于在吉大正元中返回的数据中包含身份证信息，故在此将身份证信息单独取出保存
						 * 吉大正元返回数据格式为：<attr name="X509Certificate.SubjectDN" namespace="http://www.jit.com.cn/cinas/ias/ns/saml/saml11/X.509">CN=XX, EMAILADDRESS=k, T=360102197703236311, OU=04, L=00, L=00, ST=36, O=01, O=MOF, C=CN</attr>
						 */
						if("X509Certificate.SubjectDN".equals(name)){
							String[] values = value.split(",");
							attrMap.put("personID", values[1].trim().substring(3));
						}
					}
				}
			}
		...
		if (!isSuccess) {
			...
			if (isNotNull(errDesc)) {
				info.setErrorMsg(errDesc);
			}
			log.error("处理数据结束，业务处理失败，失败原因：" + errDesc + "\n");
		} else {
			info.setSuccess(true);
			info.setAttrMap(attrMap);
			log.info("处理数据结束，一切正常！\n");
		}
		return info;
	}
