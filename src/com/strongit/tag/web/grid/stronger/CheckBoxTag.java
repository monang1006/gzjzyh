...
public class CheckBoxTag extends TableColTag {
	...
	public int doStartTag() throws JspException {
		 ...
		 * 处理图片列问题
		 * */
		List list = flextag.getCollection();
		int imgWidth = 25;
		int num = 0;
		int collectionSize = (list == null?0:list.size());
		if(collectionSize > 0){
			if(list.get(0) instanceof Map){
				Map map = (Map)list.get(0);
				if(map.containsKey("png")){
					String png = (String)map.get("png");
					if(png != null){
						num = StringUtils.splitByWholeSeparator(png,"<img").length-1;
					}
				}
				...
				for(Object obj:objs){
					if(obj != null && obj instanceof String){
						png = (String) obj;
						if(png.indexOf("<img") != -1){
							if(png.startsWith("<img")){
								num = StringUtils.splitByWholeSeparator(png,"<img").length;
							}else{
								num = StringUtils.splitByWholeSeparator(png,"<img").length - 1;
							}
							break;
		...
		out.append("<th width=\"").append(40 + imgWidth * num).append("px\" height=\""+height+"\" align=\"left\"  class=\"").append(headCss).append("\"");
	...
	public String innerHTML(Object valueid,Object showvalue,Object onclick,Object ondbclick,Object map){
		...
		String clickcolor=flextag.getClickColor();
		boolean isShowChecked = "showCheck".equals(flextag.getFootShow())?true:false;
		String tdcss="chk_"+flextag.getDetailCss();
