...
public class JdbcIssueService
{
	...
	{ 
		String sql = "select u.iss_id,t.jour_name,u.iss_time ,u.iss_number " +
		...
		String order = " order by  u.iss_time desc limit ?";
	...
	public List<IssueDto> lastIssuesBysb(String issId) throws ServiceException,
	SystemException, DAOException
	{ 
		String sql = "select u.iss_id,t.jour_name,u.iss_time ,u.iss_number " +
				"from t_info_base_journal t ,t_info_base_issue u " ;
		...
		String order = " order by  u.iss_time desc ";
		String sqlnum = sql+where+order;
		@SuppressWarnings("unchecked")
		List<IssueDto> rets = template.query(sqlnum, new PreparedStatementSetter(){
	...
	public Page<IssueDto> getIssues(final Page<IssueDto> pagePl) throws ServiceException, SystemException,
			DAOException
	{
		String selectCount = "select count(*) ";
		String select = "select u.iss_id,t.jour_name,u.iss_number,u.iss_time ";
		String from = "from t_info_base_journal t ,t_info_base_issue u ";
		String where ="where t.jour_id=u.jour_id ";
		String order = "order by u.iss_date desc limit ? offset ? ";
		...
		@SuppressWarnings("unchecked")
		List<IssueDto> rets = template.query(sql, new PreparedStatementSetter(){
	...
	public Page<IssueDto> getIssuesBysubmitter(final Page<IssueDto> pagePl , final String orgId) throws ServiceException, SystemException,
	DAOException
	{
		String selectCount = "select SUM(*) ";
		String select = "select t2.iss_id ,t2.iss_number,t3.jour_name ";
		String from = "from t_info_base_journal t3 ,t_info_base_issue t2 ,t_info_base_publish t1 ";
		String where ="where t3.jour_id=t2.jour_id and t1.iss_id=t2.iss_id and t1.org_id=? group by t2.iss_id ,t2.iss_number,t3.jour_name,t2.iss_date ";
		String order = "order by t2.iss_date desc limit ? offset ? ";
		
		String sqlcount = "SELECT COUNT(*) FROM (SELECT COUNT(*) FROM T_INFO_BASE_JOURNAL T3 ,T_INFO_BASE_ISSUE T2 ,T_INFO_BASE_PUBLISH T1 "+
							" WHERE T3.JOUR_ID=T2.JOUR_ID AND T1.ISS_ID=T2.ISS_ID AND T1.ORG_ID=? "+
							" GROUP BY T2.ISS_ID ,T2.ISS_NUMBER,T3.JOUR_NAME,T2.ISS_DATE )TAB1";
		...
		List<IssueDto> rets = template.query(sql, new PreparedStatementSetter(){
			...
			{
				ps.setString(1, orgId);
				ps.setLong(2, pagePl.getPageSize());
				ps.setLong(3, (pagePl.getPageNo()-1) * pagePl.getPageSize());
			}
		}, new BeanPropertyRowMapper(IssueDto.class));
