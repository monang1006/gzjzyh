 ...
 * @since
 */
public class LeaderSignAction implements ActionHandler {
    ...
    public void execute(ExecutionContext arg0) throws Exception {
        ...
        for(Object[] task1:task){
            if(task1[1].toString().indexOf("审签")>-1){
               task2.add(task1);
               }
        }
...
//          List<Object[]> parentPList = (List<Object[]>) workflowService.getMonitorParentInstanceIds(new Long(instanceId));
        ...
        String gwLeaderStartTime="0天";
        if(startTime!=null&&endTime!=null){
      //计算处室用时（工作日）
         leaderworkDays  = feedBackmanager.getWorkDaysBetweenStartDateAndEndDate(startTime, endTime);
         gwLeaderStartTime=fmt.format(startTime)+"-"+fmt.format(endTime)+","+leaderworkDays+"天";
        }
        //manager.insertLeadSign(bussiness1);
         manager.insertLeaderSign(gwLeaderStartTime,bussiness1);
      }
}