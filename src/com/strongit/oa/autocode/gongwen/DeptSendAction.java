 ...
 * @since
 */
public class DeptSendAction implements ActionHandler {
...
//          List<Object[]> parentPList = (List<Object[]>) workflowService.getMonitorParentInstanceIds(new Long(instanceId));
        ...
        Date startTime=null;
        startTime = processInstance.getStart();
        Date now = new Date();
       ...
       //计算处室用时（工作日）
        int deptSendworkDays  = feedBackmanager.getWorkDaysBetweenStartDateAndEndDate(startTime, now);
        String fwDeptStartTime=fmt.format(startTime)+"-"+fmt.format(now)+","+deptSendworkDays+"天";
        manager.insertDeptSend(department,startTime,fwDeptStartTime,bussiness1);
     }
}