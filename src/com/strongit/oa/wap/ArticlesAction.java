...
public class ArticlesAction extends AbstractBaseWorkflowAction<ToaInfopublishArticle> {
	...
	@Autowired ArticlesManager manager ;
	...
	private Page<ToaInfopublishColumnArticl> articlpage=new Page<ToaInfopublishColumnArticl>(7,true);
	...
	private int currentPage;//WapOA当前的数据页
	private String articleHtml;		//wap开发添加
	private String sysType;			//wap版OA系统
	...
	public String androidview() throws Exception {
		...
		try{
			...
			String content="";
			content=model.getArticlesArticlecontent();
			Form form=new Form();
			List<Item> lst=new ArrayList<Item>();
			Item item=new Item();
			item.setType("articlecontent");
	 ...
	 * @return
	 */
	public String androidGrouplist() throws Exception {
		HttpServletRequest request = getRequest();
		Data data = new Data();
		try{
			
			
			List<ToaInfopublishColumn> columnLists = columnManager.getMyColumn();
			
			Date date=new Date();
			Form form=new Form();
			List<Item> lst=new ArrayList<Item>();
			for(ToaInfopublishColumn column:columnLists){
				Set set=column.getToaInfopublishColumnArticls();
				Iterator it=set.iterator();
				int count=0;
				while(it.hasNext()){
					ToaInfopublishColumnArticl article=(ToaInfopublishColumnArticl)it.next();
					if("9".equals(article.getColumnArticleState())){
					}else{
						continue;
					}
					if(article.getToaInfopublishArticle()!=null&&article.getToaInfopublishArticle().getArticlesAutopublishtime()!=null&&!article.getToaInfopublishArticle().getArticlesAutopublishtime().before(date)){
						continue;
					}
					if(article.getToaInfopublishArticle()!=null&&article.getToaInfopublishArticle().getArticlesAutocancletime()!=null&&!article.getToaInfopublishArticle().getArticlesAutocancletime().after(date)){
						continue;
					}
					count+=1;
				}
				//column.setArticleNum(count);
				Item item=new Item();
				item.setType(column.getClumnName());
				item.setValue(column.getClumnId()+","+count);
				lst.add(item);
			}
			form.setItems(lst);
			data.setForm(form);
	...
	protected void prepareModel() throws Exception {
		...
		if (articlesId != null && !"".equals(articlesId)
			...
			if (YES.equals(model.getArticlesGuidetype())) {// 导读
				daoduBoolean = true;
			}
			if (YES.equals(model.getArticlesIshot().trim())) {// 热点
				redianBoolean = true;
			}
			...
			model.setArticlesLatestuser(userService.getCurrentUser()
					.getUserName());
	...
	public String save() throws Exception {
		...
		if (daoduBoolean) {// ?导读
			model.setArticlesGuidetype(YES);
		} else {
			model.setArticlesGuidetype(NO);
		}
		if (redianBoolean) {// 热点
			model.setArticlesIshot(YES);
		} else {
			model.setArticlesIshot(NO);
		}
		if (gudingBoolean) {// 固顶
		...
		articlesManager.saveArticle(model, columnId, columnArticleId,new OALogInfo(this.getRequest().getRemoteAddr(),"稿件上栏"));
		getRequest().setAttribute("backlocation", "javascript:cancel()");
		...
		if (columnId != null && !"".equals(columnId)&&(isPromulgate==null||"".equals(isPromulgate))) {//判断是否在栏目下添加文章
			return "cainit";
		}
	...
	public String tree() throws Exception {
		...
		if (NO.equals(treeType)) {//显示信息发布结构树
			...
			return "showtree";
		}else if("wap".equals(treeType)){			//wap开发添加
			Date date=new Date();
...
/***********************************以下为Wap手机系统开发代码********************************/
	...
	public String getColumnArticleList() throws Exception {
		articlesManager.notStandtop();
		articlesManager.isStendTop();
		int length=8;
		StringBuffer innerHtml = new StringBuffer();
		currentPage=currentPage==0?1:currentPage;
		articlpage.setPageNo(currentPage);
		articlpage = articlesManager.getColumnArticleList(articlpage,columnId);
		List<ToaInfopublishColumnArticl> articleList=articlpage.getResult();
		//List<ToaInfopublishColumnArticl> articleList=articlesManager.getColumnArticleList(columnId);
		if (articleList==null||articleList.size()== 0){// 判断是否有值
				return "list";
		}
		for(ToaInfopublishColumnArticl columnBo:articleList){
			...
			innerHtml
				...
				.append("/wap/articles!showColumnArticl.action?sysType=wap&columnId=").append(columnId)
				.append("&columnArticleId=").append(columnBo.getColumnArticleId())
			...
			if (!"0".equals(colour) && !"".equals(colour)) {// 设置了颜色
				if ("1".equals(font)) {			// 粗体
					...
					innerHtml.append("<B><I><font color=\"")
					.append(colour)
					.append("\">")
					.append(title)
					.append("</font></I></B></a></span>");
				} else {
					innerHtml.append("<font color=\"")
					.append(colour)
					.append("\">")
					.append(title)
					.append("</font></a></span>");
				...
				if ("1".equals(font)) {			// 粗体
					innerHtml.append("<B>").append(title).append("</B></a></span>");
				} else if ("2".equals(font)) {	// 斜体
					innerHtml.append("<I>").append(title).append("</I></a></span>");
				} else if ("3".equals(font)) {	// 粗斜体
					innerHtml.append("<B><I>").append(title).append("</I></B></a></span>");
				} else {
					innerHtml.append(title).append("</a></span>");
				}
	
			}
			innerHtml.append("</td><td nowrap=\"nowrap\">");
			/*innerHtml.append("</td><td>");
			if (YES.equals(columnBo.getToaInfopublishArticle().getArticlesIshot())) {// 是否为热点
				innerHtml.append("&nbsp;&nbsp;<IMG SRC=\"")
					.append(getRequest().getContextPath())
					.append("/oa/image/infopub/hot.gif\" WIDTH=\"23\" HEIGHT=\"7\" BORDER=\"0\" ALT=\"\">");
			}
			if ("".equals(columnBo.getToaInfopublishArticle()
					.getArticlesAuthor())
					|| columnBo.getToaInfopublishArticle()
					.getArticlesAuthor() == null) {
				innerHtml.append("&nbsp;&nbsp;<span style =\"color:gray\"></span>");
			} else {
				innerHtml.append("&nbsp;&nbsp;<span style =\"color:gray\">")
					.append(columnBo.getToaInfopublishArticle().getArticlesAuthor()).append("</span>");
			}
			innerHtml.append("</td><td>");*/
			innerHtml
			.append("<span style =\"color:gray\">")
			.append(st.format(columnBo.getColumnArticleLatestchangtime())).append("</span></td>")
			.append("</tr>");
			/*innerHtml.append("<div class=\"sec\" title=\"\">")
				.append("<IMG SRC=\"")
				.append(getRequest().getContextPath())
				.append("/oa/image/infopub/news_bullet.gif\" WIDTH=\"15\" HEIGHT=\"10\" BORDER=\"0\" ALT=\"\">");
			if (YES.equals(columnBo.getColumnArticleIsstandtop())) {	// 是否为固顶
				innerHtml.append("<font color=\"#0000FF\">[顶]</font>");
			}
			innerHtml.append("<span title=\"").append(
			...
			if ("".equals(columnBo.getToaInfopublishArticle()
					...
					|| columnBo.getToaInfopublishArticle()
					.getArticlesAuthor() == null) {
				innerHtml.append("&nbsp;&nbsp;<span style =\"color:gray\">无</span>");
			} else {
				innerHtml.append("&nbsp;&nbsp;<span style =\"color:gray\">")
					.append(columnBo.getToaInfopublishArticle().getArticlesAuthor()).append("</span>");
			}
			innerHtml
			.append("&nbsp;&nbsp;<span style =\"color:gray\">")
			.append(st.format(columnBo.getColumnArticleLatestchangtime())).append("</span>")
			.append("</div>");*/
		}
		articleHtml=innerHtml.toString();
		return "list";
	}
	 ...
	 * @throws Exception
	 */
	public String androidGetColumnArticleList() throws Exception {
		Data data=new Data();
		HttpServletRequest request = getRequest();
		try{
			articlesManager.notStandtop();
			...
			currentPage=currentPage==0?1:currentPage;
			articlpage.setPageNo(currentPage);
			articlpage = articlesManager.getColumnArticleList(articlpage,groupId);
			List<ToaInfopublishColumnArticl> articleList=articlpage.getResult();
			...
			itempage.setTotalPages(articlpage.getTotalPages());
			itempage.setTotalCount(articlpage.getTotalCount());
			data.setStatus(status);
	...
	public String syntree() {
		...
		for (int i = 0; i < folds.size(); i++) {
			...
			List<ToaInfopublishColumn> subColumnList = columnManager
	...
	public String setArticlesClumn() throws Exception {// 稿件上栏
		...
		for (String aid : list) {
			articlesManager.setArticlesClumn(columnId, aid,new OALogInfo("稿件上栏"));
		}
	...
	public String colAtrdelete() throws Exception {// 栏目中文章删除
		...
		for (int i = 0; i < ids.length; i++) {
			articlesManager.colAtrdelete(ids[i],new OALogInfo(this.getRequest().getRemoteAddr(),"逻辑删除栏目里稿件"));
		}
		return renderHtml("<script>  window.location='"
				+ getRequest().getContextPath()
	...
	public String truedelete() throws Exception {
		articlesManager.truedelete(articlesId,new OALogInfo(this.getRequest().getRemoteAddr(),"彻底删除稿件信息"));
	...
	public String clear() throws Exception {
		try {
			articlesManager.clear(new OALogInfo(this.getRequest().getRemoteAddr(),"清空回收站"));
	...
	public String showColumnArticl() throws Exception {
		...
		if(ca.getColumnArticleLatestchangtime()!=null){//判断最后修改时间是否为空
			...
			"yyyy-MM-dd HH:mm:ss");
			latestchangtime=format.format(ca.getColumnArticleLatestchangtime());
		}
		commentsList.clear();
		...
		if (ca.getToaInfopublishArticle().getArticlesPaginationnum() > 0) {
        ...
        /*if("1".equals(ca.getToaInfopublishArticle().getArticlesIsredirect())){
        	getRequest().setAttribute("outlink", ca.getToaInfopublishArticle().getArticlesRedirecturl());
	...
	public String showDesk() throws Exception {
		...
		String showType = map.get("showType");// 是否显示日期
		String showco = "";
		StringBuffer innerHtml = new StringBuffer();
		articlpage.setPageSize(Integer.parseInt(showNum));
		articlpage = articlesManager
				.getColumnArticleList(articlpage,columnId);
...
if (showco != "") {
	innerHtml
			.append(
					"    <div align=\"right\" style=\"padding:2px；font-size:12px;\"><a href=\"javascript:window.top.parent.navigate('")
			.append(getRequest().getContextPath())
			.append(
					"/fileNameRedirectAction.action?toPage=infopub/articles/articles-showcontent.jsp?columnId=")
	...
	public String deleteComment() throws Exception {
		...
		isactive = userService.getCurrentUser().getUserIsactive();// 判断是否是管理员
		// 获取一条评论
		commentsList = articlesManager.getComments(articlesId,new OALogInfo("删除评论"));
	...
	public String comments() throws Exception {
		...
		commentName = "";
		isactive = userService.getCurrentUser().getUserIsactive();// 判断是否是管理员
		articlesManager.comments(articlesId, commentText, commentName,new OALogInfo(this.getRequest().getRemoteAddr(),"添加评论"));// 添加一条评论

		// 获取添加的评论ID
		commentsList = articlesManager.getComments(articlesId,new OALogInfo("添加评论"));
	...
	public String pubArticl() throws Exception {
		...
		for (int i = 0; i < id.length; i++) {
			...
			articlesManager.updataColumnArtile(ca,new OALogInfo(this.getRequest().getRemoteAddr(),"稿件发布"));
			articlesManager.saveArticle(model,new OALogInfo(this.getRequest().getRemoteAddr(),"修改稿件状态"));
	...
	public String getProcessName() throws Exception {
		...
		if ((processName != null && !"".equals(processName)
				&& !"9".equals(articlesState))||"0".equals(articlesState)) { // 文章栏目已挂流程并且文章状态未发布
			StringBuffer str = new StringBuffer(100);
			str.append(columnId).append(",").append(columnArticleId)
					.append(",").append(articlesState);
	...
	public String handleNextStep(){
		try{
			...
			if ("".equals(strTaskActors)) {
				getManager().handleWorkflowNextStep(taskId,transitionName,"",isNewForm,formId,bussinessId,suggestion,curUserId,null);
			} else {
				getManager().handleWorkflowNextStep(taskId,transitionName,"",isNewForm,formId,bussinessId,suggestion,curUserId,strTaskActors.split(","));
			}	
//			在这里保存审批意见
	...
	public String back() throws Exception {
		...
		try{
			if(StringUtils.hasLength(taskId)){
				...
				String curUserId=userService.getCurrentUser().getUserId();
				getManager().backSpace(taskId, returnNodeId, formId, suggestion,curUserId, formData,new OALogInfo("任务退回上一办理人，taskId="+taskId));
			}else{
				ret = "-1";
			}
