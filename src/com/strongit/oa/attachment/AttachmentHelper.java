...
public class AttachmentHelper {
	 ...
	 * 			保存的文件名
	 * @author 刘皙
	 * @createTime 2012年3月13日18:48:20
	 * @return
	 */
	public static synchronized String transformFile(String cxtPath, InputStream is,
			String fileName) {
		deleteInvalid();
		String path = PathUtil.getRootPath() + cxtPath + File.separatorChar;// 得到相对工程目录的路径
		File file = new File(path);
		if (!file.exists()) {
			if(!file.mkdir()) {
				throw new SystemException("创建目录 " + path + " 目录不存在。");
		...
		mapFormAttachment.put(path, System.currentTimeMillis());
		return path;
	}
