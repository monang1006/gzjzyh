...
public class CAAuth {
	...
	public Boolean auth(HttpSession session,HttpServletRequest request) throws Exception {
		/***************************************************************************
		...
		// 可以根据需求使用不同的获取方法
		Properties prop = PropertiesUtil.getPropertiesWithFileName("caConfig.properties");
		String appId = prop.getProperty(KEY_APP_ID);
		String authURL = prop.getProperty(KEY_AUTHURL);
		...
		if (isSuccess) {
			System.out.println("应用标识及网关的认证地址读取成功！\n应用标识：" + appId + "\n认证地址："
					+ authURL + "\n");
			if (isNotNull((String) session.getAttribute(
					KEY_ORIGINAL_DATA))
		...
		//认证成功后需要清楚SESSION中保留的密钥信息
		request.getSession().removeAttribute(KEY_ORIGINAL_DATA);
		return isSuccess;
	}

	public String getSignInfo(String data) throws Exception {
		if(data == null) {
			return "";
		}
		String[] datas = data.split(",");
		String original_jsp = datas[0];
		String signed_data = datas[1];
