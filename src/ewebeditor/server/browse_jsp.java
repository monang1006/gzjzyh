...
//		out = m_response.getWriter();
	...
	private void InitUpload() throws ServletException,IOException{
		...
		if (!bValidStyle) {
			out = out + getOutScript("alert('Invalid Style!')");
//			out.close();
			return;
		}
		...
		if (nAllowBrowse!=1){
			out = out + getOutScript("alert('Do not allow browse!')");
//			out.close();
			return;
		}
		...
		sPathShareImage = GetSlashPath(myUtil.getRealPathFromRelative(m_request.getServletPath(), "sharefile/image/"));
		sPathShareFlash = GetSlashPath(myUtil.getRealPathFromRelative(m_request.getServletPath(), "sharefile/flash/"));
		sPathShareMedia = GetSlashPath(myUtil.getRealPathFromRelative(m_request.getServletPath(), "sharefile/media/"));
		sPathShareOther = GetSlashPath(myUtil.getRealPathFromRelative(m_request.getServletPath(), "sharefile/other/"));
