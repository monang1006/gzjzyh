...
public class upload_jsp
{
	...
	private int nAllowSize, nUploadObject;
...
//		out = m_response.getWriter();
	...
	private void InitUpload() throws ServletException,IOException{
		...
		if (!sBaseUrl.equals("3")){
			sRealUploadPath = myUtil.getRealPathFromRelative(m_request.getServletPath(), sUploadDir);
		}else{
		...
		if (!sSYPicPath.equals("")){
			sSYPicPath = myUtil.getRealPathFromRelative(m_request.getServletPath(), sSYPicPath);
		}
	...
	private void DoRemote(){
			...
			out = out + "<html><head><title>eWebEditor</title><meta http-equiv='Content-Type' content='text/html; charset=GBK'></head><body><input type=hidden id=UploadText value=\"" + inHTML(sRemoteContent) + "\"></body></html>";
			OutScript("parent.setHTML(document.getElementById('UploadText').value);try{parent.addUploadFile('" + sOriginalFileName + "', '" + sPathFileName + "');} catch(e){} parent.remoteUploadOK();");
	...
	private void DoSave() throws ServletException{
		out = out + "<html><head><title>eWebEditor</title><meta http-equiv='Content-Type' content='text/html; charset=GBK'></head><body>";
	...
	private void DoLocal() throws ServletException{
		...
		sPathFileName = sContentPath + sSaveFileName;
		out = out + sPathFileName;
	}



	private void DoMFU() throws ServletException{
		...
		if (sParamBlockFlag.equals("end")){
			...
			sPathFileName = sContentPath + sSaveFileName;
			out = out + sPathFileName + "::" + s_SmallImagePathFile;
		}else{
			out = out + sSaveFileName.substring(0, sSaveFileName.lastIndexOf("."));
		}
	}
	...
	private String MFU_GetSavePath(String s_Path){
	...
	private void OutScript(String str){
		out = out + "<script language=javascript>" + str + "</script>";
	}
	...
	private boolean CheckSpaceSize(){
