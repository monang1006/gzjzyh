...
public class style_jsp
{
	...
	private ewebeditor.admin.util myUtil;
		...
		String sWordImportInitMode="", sQuickFormatInitFontName="", sQuickFormatInitFontSize="";
	...
	public final void Load(PageContext pagecontext) throws ServletException,IOException{
		...
		if (myUtil.sAction.equals("STYLEPREVIEW") || myUtil.sAction.equals("COPY") || myUtil.sAction.equals("STYLESET") || myUtil.sAction.equals("STYLEDEL") || myUtil.sAction.equals("CODE") || myUtil.sAction.equals("TOOLBAR") || myUtil.sAction.equals("TOOLBARADD") || myUtil.sAction.equals("TOOLBARMODI") || myUtil.sAction.equals("TOOLBARDEL") || myUtil.sAction.equals("BUTTONSET") || myUtil.sAction.equals("BUTTONSAVE")){
			...
			if (!b){
				out.append(myUtil.getError("无效的样式ID号，请通过页面上的链接进行操作！"));
				return;
			}
		}
		...
		out.append(myUtil.Header());

//		out.append("<table border=0 cellspacing=1 align=center class=navi>"
...
//			+"</table><br>");
		...
		if (myUtil.sAction.equals("BUTTONSET") || myUtil.sAction.equals("BUTTONSAVE")){
			...
			if (!b){
				out.append(myUtil.getError("无效的工具栏ID号，请通过页面上的链接进行操作！"));
				return;
			}
		...
		if(myUtil.sAction.equals("STYLEADD") || myUtil.sAction.equals("STYLESET")){
			...
			String s_FormAutoDirOrderFlag, s_FormWordImportInitMode;
			...
			s_FormAdvApiFlag = myUtil.InitSelect("d_advapiflag", myUtil.split("禁用|启用一般接口(cusdir)|启用高级Session安全接口", "|"), myUtil.split("0|1|2", "|"), sAdvApiFlag, "", "");
			...
			out.append("<table border=0 cellpadding=0 cellspacing=1 align=center class=form>" +
					...
					"<tr><td>一键排版初始值：</td><td>字体：<input type=text class=input name=d_quickformatinitfontname size=10 title='字体名称' value=\"" + sQuickFormatInitFontName + "\"> 字号：<input type=text class=input name=d_quickformatinitfontsize size=10 title='字体大小' value=\"" + sQuickFormatInitFontSize + "\"></td><td></td><td></td></tr>" +
			...
			out.append(myUtil.GetMessage(s_Msg));
			...
			if(myUtil.sAction.equals("STYLEADDSAVE")){
				...
				if (StyleName2ID(sStyleName, myUtil.aStyle) != -1) {
					out.append(myUtil.getError("此样式名已经存在，请用另一个样式名！"));
					return;
				}
				...
				out.append(myUtil.GetMessage("<b><span class=red>样式增加成功！</span></b><li><a href='?action=toolbar&id=" + String.valueOf(myUtil.aStyle.size()-1) + "'>设置此样式下的工具栏</a>"));
				...
				out.append(myUtil.GetMessage("<b><span class=red>样式修改成功！</span></b><li><a href='?action=stylepreview&id=" + sStyleID + "' target='_blank'>预览此样式</a><li><a href='?action=toolbar&id=" + sStyleID + "'>设置此样式下的工具栏</a><li><a href='?action=styleset&id=" + sStyleID + "'>重新设置此样式</a>"));

			}

		}else if(myUtil.sAction.equals("CODE")){

			out.append("<table border=0 cellspacing=1 align=center class=list>" +
			...
			for(int nLoop=0; nLoop<1; nLoop++){
				...
				out.append(myUtil.GetMessage("<b class=blue>当前样式：<span class=red>" + myUtil.htmlEncode(sStyleName) + "</span>&nbsp;&nbsp;当前工具栏：<span class=red>" + myUtil.htmlEncode(sToolBarName) + "</span></b>"));

				out.append("<script language='javascript' src='../js/buttons.js'></script>");
				out.append("<script language='javascript' src='../js/zh-cn.js'></script>");

				out.append("<table border=0 cellpadding=5 cellspacing=0 align=center>"+
				...
				out.append(myUtil.GetMessage("<b class=blue>提示：</b>你可以通过按“Ctrl”“Shift”来快速多选定，可以在指定项上“双击”快速增加或删除项。可以选定多个按钮同时上移或下移操作。"));
			...
			out.append(myUtil.GetMessage("<b><span class=red>工具栏按钮设置保存成功！</span></b><li><a href='?action=stylepreview&id=" + sStyleID + "' target='_blank'>预览此样式</a><li><a href='?action=toolbar&id=" + sStyleID + "'>返回工具栏管理</a><li><a href='?action=buttonset&id=" + sStyleID + "&toolbarid=" + sToolBarID + "'>重新设置此工具栏下的按钮</a>"));
			...
			if(myUtil.sAction.equals("TOOLBARADD")){
				...
				if (sToolbarAdd_Name.equals("")) {
					out.append(myUtil.getError("工具栏名不能为空！"));
					return;
				}
				if (!myUtil.isNumber(sToolbarAdd_Order)) {
					out.append(myUtil.getError("无效的工具栏排序号，排序号必须为数字！"));
					return;
				}
				...
				out.append("<script language=javascript>alert(\"工具栏（" + myUtil.htmlEncode(sToolbarAdd_Name) + "）增加操作成功！\");</script>");
				out.append(myUtil.GetGoUrl("?action=toolbar&id=" + sStyleID));
				return;
				...
				out.append("<script language=javascript>alert('工具栏修改操作成功！');</script>");
				out.append(myUtil.GetGoUrl("?action=toolbar&id=" + sStyleID));
				return;
				...
				if (myUtil.isNumber(s_DelID)) {
					...
					myUtil.WriteConfig(myUtil.eWebEditorPath, myUtil.sLicense, myUtil.sUsername, myUtil.sPassword, myUtil.aStyle, myUtil.aToolbar);
					out.append("<script language=javascript>alert('工具栏（ID：" + s_DelID + "）删除操作成功！');</script>");
					out.append(myUtil.GetGoUrl("?action=toolbar&id=" + sStyleID));
					return;
				}
			...
			out.append(myUtil.GetMessage("<b class=blue>样式（" + myUtil.htmlEncode(sStyleName) + "）下的工具栏管理：</b>"));
			...
			out.append("<hr width='80%' align=center size=1><table border=0 cellpadding=4 cellspacing=0 align=center>" +
			...
			out.append("<form action='?id=" + sStyleID + "&action=toolbarmodi' name=modiform method=post>" +
			...
			out.append("<tr><td colspan=4 align=center><input type=submit name=b1 value='  修改  '></td></tr>");
			...
			// Show Style List
			out.append(myUtil.GetMessage("<b class=blue>以下为当前所有样式列表：</b>"));

			out.append("<table border=0 cellpadding=0 cellspacing=1 class=list align=center>" + 
			...
			out.append(myUtil.GetMessage("<b class=blue>提示：</b>你可以通过“拷贝”一样式以达到快速新建样式的目的。"));

		}


		out.append(myUtil.Footer());
