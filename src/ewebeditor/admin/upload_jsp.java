	...
	public final void Load(PageContext pagecontext) throws ServletException,IOException{
		...
		if (!sCurrDir.equals("")){
			...
			out.print("<table border=0 cellspacing=1 class=list align=center>" +
				"<form action='?id=" + sStyleID + "&d_viewmode=" + s_ViewMode + "&dir=" + utf8enc(sDir) + "&action=del' method=post name=myform>" +
				"<tr align=center>" +
					"<th width='10%'>类型</th>" +
					"<th width='50%'>文件地址</th>" +
					"<th width='10%'>大小</th>" +
			...
			if (!sDir.equals("")) {
				...
				if (sDir.indexOf("/")>0){
					out.print(utf8enc(sDir.substring(0, sDir.lastIndexOf("/"))));
				}
			...
			for (int i=0; i<aSubFolders.size(); i++){
				...
				if (!sDir.equals("")) {
					out.print(utf8enc(sDir + "/" + aSubFolders.get(i).toString()));
				}else{
					out.print(utf8enc(aSubFolders.get(i).toString()));
				}
				out.print("\">" + aSubFolders.get(i).toString() + "</a></td>" +
					"<td><a href='?id=" + sStyleID + "&d_viewmode=" + s_ViewMode + "&dir=" + utf8enc(sDir) + "&action=delfolder&foldername=" + utf8enc(aSubFolders.get(i).toString()) + "'>删除</a></td></tr>");
			}
			...
			for (int i=nBeginPos; i<nEndPos; i++){
				...
				if (s_ViewMode.equals("list")){
					...
					m = n % 4;
					if (n == 1){
						out.print("<tr align=center><td colspan=5><table border=0 cellspacing=1 width='98%' class=list>");
					}
					if (m == 1){
						out.print("<tr>");
					}

					out.print("<td width='25%' valign=top align=center><table border=0 cellspacing=1>"+
			...
			if (nFileNum > 0) {
				...
				if (nCurrPage > 1) {
					out.print("<a href='?id=" + sStyleID + "&dir=" + utf8enc(sDir) + "&d_viewmode=" + s_ViewMode + "&page=1'>首页</a>&nbsp;&nbsp;<a href='?id=" + sStyleID + "&dir=" + utf8enc(sDir) + "&d_viewmode=" + s_ViewMode + "&page=" + String.valueOf(nCurrPage - 1) + "'>上一页</a>&nbsp;&nbsp;");
				}else{
					out.print("首页&nbsp;&nbsp;上一页&nbsp;&nbsp;");
				}
				if (nCurrPage < nPageNum) {
					out.print("<a href='?id=" + sStyleID + "&dir=" + utf8enc(sDir) + "&d_viewmode=" + s_ViewMode + "&page=" + String.valueOf(nCurrPage + 1) + "'>下一页</a>&nbsp;&nbsp;<a href='?id=" + sStyleID + "&dir=" + utf8enc(sDir) + "&d_viewmode=" + s_ViewMode + "&page=" + String.valueOf(nPageNum) + "'>尾页</a>");
				}else{
					out.print("下一页&nbsp;&nbsp;尾页");
				}
				...
				out.print("</td><td align=right><input type=checkbox id=b_selectall onclick='doCheckAll(this)'><label for=b_selectall>全选</label>&nbsp; <input type=submit name=b value='删除选定的文件'> <input type=button name=b1 value='清空本目录所有文件' onclick=\"javascript:if (confirm('你确定要清空所有文件吗？')) {location.href='?id=" + sStyleID + "&d_viewmode=" + s_ViewMode + "&dir=" + utf8enc(sDir) + "&action=delall';}\"></td></tr></table></td></tr>");
			}
